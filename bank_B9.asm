
set_inactive_kong_anim_handle_dixie:
	TAX					;$B9D000  \
	LDA current_kong_control_variables	;$B9D001   |
	PHA					;$B9D003   |
	LDA current_sprite			;$B9D004   |
	PHA					;$B9D006   |
	LDA inactive_kong_control_variables	;$B9D007   |
	STA current_kong_control_variables	;$B9D00A   |
	LDA inactive_kong_sprite		;$B9D00C   |
	STA current_sprite			;$B9D00F   |
	TXA					;$B9D011   |
	JSL set_anim_handle_dixie		;$B9D012   |
	PLA					;$B9D016   |
	STA current_sprite			;$B9D017   |
	PLA					;$B9D019   |
	STA current_kong_control_variables	;$B9D01A   |
	RTL					;$B9D01C  /

set_inactive_kong_anim:
	TAX					;$B9D01D  \
	LDA current_kong_control_variables	;$B9D01E   |
	PHA					;$B9D020   |
	LDA current_sprite			;$B9D021   |
	PHA					;$B9D023   |
	LDA inactive_kong_control_variables	;$B9D024   |
	STA current_kong_control_variables	;$B9D027   |
	LDA inactive_kong_sprite		;$B9D029   |
	STA current_sprite			;$B9D02C   |
	TXA					;$B9D02E   |
	JSL set_sprite_animation		;$B9D02F   |
	PLA					;$B9D033   |
	STA current_sprite			;$B9D034   |
	PLA					;$B9D036   |
	STA current_kong_control_variables	;$B9D037   |
	RTL					;$B9D039  /

set_anim_handle_throwable:
%local(.requested_animation_id, temp_26)
	STA .requested_animation_id		;$B9D03A  \
	LDX current_sprite			;$B9D03C   |
	LDA sprite.type,x			;$B9D03E   |
	SEC					;$B9D040   |
	SBC #!throwable_sprite_range_start	;$B9D041   |> Throwable base sprite id
	LSR A					;$B9D044   |
	LSR A					;$B9D045   |
	CLC					;$B9D046   |
	ADC .requested_animation_id		;$B9D047   |
	BRA set_sprite_animation		;$B9D049  /


set_riding_kong_anim_handle_dixie:
%local(.requested_animation_id, temp_32)
	STA .requested_animation_id		;$B9D04B  \
	LDA active_kong_control_variables	;$B9D04D   |
	STA current_kong_control_variables	;$B9D050   |
	TAX					;$B9D052   |
	LDA kong_control.animation_id,x		;$B9D053   |
	PHX					;$B9D055   |
	PHA					;$B9D056   |
	LDA current_sprite			;$B9D057   |
	PHA					;$B9D059   |
	LDA current_player_mount		;$B9D05A   |
	BEQ .is_animal				;$B9D05C   |
	STA current_sprite			;$B9D05E   |
	LDX current_sprite			;$B9D060   |
	LDA sprite.type,x			;$B9D062   |
	SEC					;$B9D064   |
	SBC #!animal_sprite_type_range_start	;$B9D065   |
	LSR A					;$B9D068   |
	LSR A					;$B9D069   |
	CLC					;$B9D06A   |
	ADC .requested_animation_id		;$B9D06B   |
	JSL set_anim_handle_dixie		;$B9D06D   |
.is_animal					;	   |
	PLA					;$B9D071   |
	STA current_sprite			;$B9D072   |
	PLA					;$B9D074   |
	PLX					;$B9D075   |
	STA kong_control.animation_id,x		;$B9D076   |
	LDX current_sprite			;$B9D078   |
	RTL					;$B9D07A  /

set_anim_handle_animal:
%local(.requested_animation_id, temp_32)
	STA .requested_animation_id		;$B9D07B  \
	LDX current_sprite			;$B9D07D   |
	LDA sprite.type,x			;$B9D07F   |
	SEC					;$B9D081   |
	SBC #!animal_sprite_type_range_start	;$B9D082   |
	LSR A					;$B9D085   |
	LSR A					;$B9D086   |
	CLC					;$B9D087   |
	ADC .requested_animation_id		;$B9D088   |
	BRA set_sprite_animation		;$B9D08A  /

set_anim_handle_animal_and_dixie:
%local(.requested_animation_id, temp_26)
	STA .requested_animation_id		;$B9D08C  \
	LDA animal_type				;$B9D08E   |
	SEC					;$B9D090   |
	SBC #!animal_sprite_type_range_start	;$B9D091   |
	LSR A					;$B9D094   |
	LSR A					;$B9D095   |
	CLC					;$B9D096   |
	ADC .requested_animation_id		;$B9D097   |
	BRA set_anim_handle_dixie		;$B9D099  /

set_alt_sprite_animation:
	TAX					;$B9D09B  \
	LDA current_sprite			;$B9D09C   |
	PHA					;$B9D09E   |
	LDA alternate_sprite			;$B9D09F   |
	PHA					;$B9D0A1   |
	STA current_sprite			;$B9D0A2   |
	TXA					;$B9D0A4   |
	JSL set_sprite_animation		;$B9D0A5   |
	PLA					;$B9D0A9   |
	STA alternate_sprite			;$B9D0AA   |
	PLA					;$B9D0AC   |
	STA current_sprite			;$B9D0AD   |
	RTL					;$B9D0AF  /

set_anim_once_handle_dixie:
	LDY current_kong_control_variables	;$B9D0B0  \
	CMP.w kong_control.animation_id,y	;$B9D0B2   |
	BNE set_anim_handle_dixie		;$B9D0B5   |
	RTL					;$B9D0B7  /

set_anim_handle_dixie:
	LDY current_kong_control_variables	;$B9D0B8  \
	STA.w kong_control.animation_id,y	;$B9D0BA   |
	CPY #diddy_control_variables		;$B9D0BD   |
	BEQ set_sprite_animation		;$B9D0C0   |
	CLC					;$B9D0C2   |
	ADC #!dixie_animation_id_start		;$B9D0C3   |
set_sprite_animation:				;	   |
%local(.animation_flags, temp_26)		;	   |
	PHB					;$B9D0C6   |\
	%pea_shift_dbr(animation_scripts_table)	;$B9D0C7   | | 
	PLB					;$B9D0CA   | |
	PLB					;$B9D0CB   |/
	LDX current_sprite			;$B9D0CC   |
	STA sprite.animation_id,x		;$B9D0CE   |\ set animation id then preserve it in Y
	TAY					;$B9D0D0   |/
	LDA #!default_animation_speed		;$B9D0D1   |\
	STA sprite.animation_speed,x		;$B9D0D4   | | initialize animation speed
	STZ sprite.animation_timer,x		;$B9D0D6   | | clear graphic display time
	STZ sprite.animation_routine,x		;$B9D0D8   | | clear animation routine call pointer
	TYA					;$B9D0DA   |/ retrieve animation id from Y
	ASL A					;$B9D0DB   |
	ASL A					;$B9D0DC   |
	TXY					;$B9D0DD   |
	TAX					;$B9D0DE   |
	LDA.l animation_flags_table,x		;$B9D0DF   |
	STA .animation_flags			;$B9D0E3   |
	LDA.l animation_scripts_table,x		;$B9D0E5   |\
	TYX					;$B9D0E9   | | update animation script address
	STA sprite.animation_address,x		;$B9D0EA   |/
	TAY					;$B9D0EC   |> preserve script address in Y
	LDA .animation_flags			;$B9D0ED   |
	STA sprite.animation_flags,x		;$B9D0EF   |
	BRA process_anim_script			;$B9D0F1  /

process_anim_preserve_state:
	LDX current_sprite			;$B9D0F3  \
	LDA sprite.state,x			;$B9D0F5   |
	PHA					;$B9D0F7   |
	JSL process_sprite_animation		;$B9D0F8   |
	PLA					;$B9D0FC   |
	STA sprite.state,x			;$B9D0FD   |
	RTL					;$B9D0FF  /

process_sprite_animation:
	LDX current_sprite			;$B9D100  \
	LDA sprite.animation_timer,x		;$B9D102   |
	SEC					;$B9D104   |
	SBC sprite.animation_speed,x		;$B9D105   |
	STA sprite.animation_timer,x		;$B9D107   |
	BEQ .CODE_B9D10D			;$B9D109   |
	BPL CODE_B9D13E				;$B9D10B   |
.CODE_B9D10D					;	   |
	PHB					;$B9D10D   |
	%pea_shift_dbr(animation_scripts_table)	;$B9D10E   |
	PLB					;$B9D111   |
	PLB					;$B9D112   |
	LDY sprite.animation_address,x		;$B9D113   |
process_anim_script:				;	   |
	DEY					;$B9D115   |\
	LDA $0000,y				;$B9D116   | |
	BMI animation_command_handler		;$B9D119   |/ if a command was loaded handle the command
	AND #$FF00				;$B9D11B   |\
	CLC					;$B9D11E   | | otherwise a graphic draw duration was loaded
	ADC sprite.animation_timer,x		;$B9D11F   | |
	STA sprite.animation_timer,x		;$B9D121   | | update graphic
	BPL CODE_B9D131				;$B9D123   |/
	INY					;$B9D125   |
	INY					;$B9D126   |
	INY					;$B9D127   |
	INY					;$B9D128   |
	BRA process_anim_script			;$B9D129  /

CODE_B9D12B:
	LDX current_sprite			;$B9D12B  \
	STZ sprite.animation_timer,x		;$B9D12D   |
	BRA CODE_B9D13D				;$B9D12F  /

CODE_B9D131:
	INY					;$B9D131  \
	INY					;$B9D132   |
	LDA $0000,y				;$B9D133   |
	STA sprite.current_graphic,x		;$B9D136   |
	INY					;$B9D138   |
	INY					;$B9D139   |
	TYA					;$B9D13A   |
	STA sprite.animation_address,x		;$B9D13B   |
CODE_B9D13D:					;	   |
	PLB					;$B9D13D   |
CODE_B9D13E:					;	   |
	LDA sprite.animation_routine,x		;$B9D13E   |
	BNE CODE_B9D143				;$B9D140   |
	RTL					;$B9D142  /

CODE_B9D143:
	STA temp_26				;$B9D143  \
	PHB					;$B9D145   |
	PHK					;$B9D146   |
	PLB					;$B9D147   |
	PHX					;$B9D148   |
	%return(CODE_B9D14F)			;$B9D149   |
	JMP (temp_26)				;$B9D14C  /

CODE_B9D14F:
	PLX					;$B9D14F  \
	PLB					;$B9D150   |
	RTL					;$B9D151  /

animation_command_handler:
	AND #$FF00				;$B9D152  \
	INY					;$B9D155   |
	XBA					;$B9D156   |
	SEC					;$B9D157   |
	SBC #$0080				;$B9D158   |
	ASL A					;$B9D15B   |
	TAX					;$B9D15C   |
	JMP (animation_command_table,x)		;$B9D15D  /

animation_command_80_94:
	LDX current_sprite			;$B9D160  \
	LDA sprite.animation_id,x		;$B9D162   |
	ASL A					;$B9D164   |
	ASL A					;$B9D165   |
	TXY					;$B9D166   |
	TAX					;$B9D167   |
	LDA.l animation_scripts_table,x		;$B9D168   |
	TYX					;$B9D16C   |
	STA sprite.animation_address,x		;$B9D16D   |
	TAY					;$B9D16F   |
	BRA process_anim_script			;$B9D170  /

animation_command_81:
	LDX current_sprite			;$B9D172  \> Get current sprite
	INY					;$B9D174   |\
	LDA $0000,y				;$B9D175   | | Get command param 1 (animation code execution pointer)
	STA temp_26				;$B9D178   |/
	INY					;$B9D17A   |\ Move to next command
	INY					;$B9D17B   |/
	TYA					;$B9D17C   |\
	PHA					;$B9D17D   |/ Preserve current animation script pointer on stack
	STA sprite.animation_address,x		;$B9D17E   |> Update animation script pointer for current sprite
	PHB					;$B9D180   |> Preserve data bank
	PHK					;$B9D181   |\
	PLB					;$B9D182   |/ Set data bank to this bank
	%return(.animation_code_return)		;$B9D183   |> Set return address for when RTS is executed
	JMP (temp_26)				;$B9D186  /> Execute animation code

.animation_code_return:
	PLB					;$B9D189  \> Retrieve data bank from stack
	PLA					;$B9D18A   |\ Retrieve last animation script pointer from stack and save it
	STA $52					;$B9D18B   |/
	LDX current_sprite			;$B9D18D   |\
	LDY sprite.animation_address,x		;$B9D18F   | | Get animation script pointer from current sprite
	TYA					;$B9D191   |/
	CMP $52					;$B9D192   |\ If animation script pointer for this sprite was changed stop processing the script
	BNE .stop_processing_script		;$B9D194   |/
	JMP process_anim_script			;$B9D196  /> Continue processing animation script

.stop_processing_script
	PLB					;$B9D199  \
	RTL					;$B9D19A  /

;animation command 82 (animation jump)
animation_command_82:
	LDX current_sprite			;$B9D19B  \> Get current sprite
	INY					;$B9D19D   |\
	LDA $0000,y				;$B9D19E   |/ Get command param 1 (animation script pointer)
	TAY					;$B9D1A1   |> Move animation script pointer to new animation jump address
	JMP process_anim_script			;$B9D1A2  /> Continue processing animation script from new address

;animation command 83 (execute code)
animation_command_83:
	LDX current_sprite			;$B9D1A5  \> Get current sprite
	TYA					;$B9D1A7   |\
	STA sprite.animation_address,x		;$B9D1A8   |/ Update animation script pointer for current sprite
	INY					;$B9D1AA   |\
	LDA $0000,y				;$B9D1AB   | | Get command param 1 (animation code execution pointer)
	STA temp_26				;$B9D1AE   |/
	INY					;$B9D1B0   |\ Move to next command
	INY					;$B9D1B1   |/
	JMP (temp_26)				;$B9D1B2  /> Execute animation code

;animation command 84 (execute code continuously)
animation_command_84:
	LDX current_sprite			;$B9D1B5  \> Get current sprite
	INY					;$B9D1B7   |\
	LDA $0000,y				;$B9D1B8   |/ Get command param 1 (animation code execution pointer)
	STA sprite.animation_routine,x		;$B9D1BB   |> Store animation code pointer in sprite for execution
	INY					;$B9D1BD   |\ Move to next command
	INY					;$B9D1BE   |/
	JMP process_anim_script			;$B9D1BF  /> Continue processing animation script

;animation command 8E (play sound effect)
animation_command_8E:
	LDX current_sprite			;$B9D1C2  \> Get current sprite
	INY					;$B9D1C4   |\
	LDA $0000,y				;$B9D1C5   |/ Get command param 1 (sound effect)
	PHY					;$B9D1C8   |\ Preserve animation script pointer
	JSL queue_sound_effect			;$B9D1C9   | | And queue sound effect
	PLY					;$B9D1CD   |/ Then retrieve animation script pointer
	INY					;$B9D1CE   |\ Move to next command
	INY					;$B9D1CF   |/
	LDX current_sprite			;$B9D1D0   |> Get current sprite
	JMP process_anim_script			;$B9D1D2  /> Continue processing animation script

;animation command 93 (play sound effect if not offscreen)
animation_command_93:
	LDX current_sprite			;$B9D1D5  \
	INY					;$B9D1D7   |
	LDA $0000,y				;$B9D1D8   |
	%pea_mirror_dbr()			;$B9D1DB   |
	PLB					;$B9D1DE   |
	PHY					;$B9D1DF   |
	PHA					;$B9D1E0   |
	JSL check_if_sprite_offscreen_global	;$B9D1E1   |
	PLA					;$B9D1E5   |
	BCS .CODE_B9D1EC			;$B9D1E6   |
	JSL queue_sound_effect			;$B9D1E8   |
.CODE_B9D1EC					;	   |
	PLY					;$B9D1EC   |
	PLB					;$B9D1ED   |
	INY					;$B9D1EE   |
	INY					;$B9D1EF   |
	LDX current_sprite			;$B9D1F0   |
	JMP process_anim_script			;$B9D1F2  /

;animation command 8F (animation jump if true)
animation_command_8F:
	LDX current_sprite			;$B9D1F5  \> Get current sprite
	INY					;$B9D1F7   |\
	LDA $0000,y				;$B9D1F8   | | Get command param 1 (animation code execution pointer)
	STA temp_26				;$B9D1FB   |/
	INY					;$B9D1FD   |\ Move to next parameter
	INY					;$B9D1FE   |/
	TYA					;$B9D1FF   |\
	STA sprite.animation_address,x		;$B9D200   |/ Update animation script pointer for current sprite
	PHB					;$B9D202   |> Preserve data bank
	PHK					;$B9D203   |\ Set data bank to this bank
	PLB					;$B9D204   |/
	%return(.animation_code_return)		;$B9D205   |> Set return address for when RTS is executed
	JMP (temp_26)				;$B9D208  /> Execute animation code

.animation_code_return
	PLB					;$B9D20B  \> Retrieve data bank from stack
	LDX current_sprite			;$B9D20C   |\
	LDY sprite.animation_address,x		;$B9D20E   |/ Get animation script pointer from current sprite
	LDA $0000,y				;$B9D210   |> Get animation jump address
	BCS .jump				;$B9D213   |> If the condition was met jump to new animation address
	INY					;$B9D215   |\ Move to next command
	INY					;$B9D216   |/
	JMP process_anim_script			;$B9D217  /> Condition wasn't met, Continue processing animation script

.jump
	TAY					;$B9D21A  \> Jump to new animation script address
	JMP process_anim_script			;$B9D21B  /> Continue processing animation script

;animation command 90 (execute code then set new animation id)
animation_command_90:
	LDX current_sprite			;$B9D21E  \> Get current sprite
	INY					;$B9D220   |\
	LDA $0000,y				;$B9D221   | | Get command param 1 (animation code execution pointer)
	STA temp_26				;$B9D224   |/
	INY					;$B9D226   |\ Move to next parameter
	INY					;$B9D227   |/
	TYA					;$B9D228   |\
	STA sprite.animation_address,x		;$B9D229   |/ Update animation script pointer for current sprite
	PHB					;$B9D22B   |> Preserve data bank
	PHK					;$B9D22C   |\ Set data bank to this bank
	PLB					;$B9D22D   |/
	%return(.animation_code_return)		;$B9D22E   |> Set return address for when RTS is executed
	JMP (temp_26)				;$B9D231  /> Execute animation code

.animation_code_return
	PLB					;$B9D234  \> Retrieve data bank from stack
	LDX current_sprite			;$B9D235   |\
	LDY sprite.animation_address,x		;$B9D237   |/ Get animation script pointer from current sprite
	LDA $0000,y				;$B9D239   |> Get new animation id
	PLB					;$B9D23C   |
	JMP set_sprite_animation		;$B9D23D  /> Set new sprite animation

;animation command 91 (call animation script subroutine)
animation_command_91:
	INY					;$B9D240  \ \
	LDA $0000,y				;$B9D241   | | Get command param 1 (variable to use for animation script return)
	AND #$00FF				;$B9D244   |/
	CLC					;$B9D247   |\
	ADC current_sprite			;$B9D248   | | Add offset to current sprite pointer to get actual address in RAM
	TAX					;$B9D24A   |/
	INY					;$B9D24B   |\
	LDA $0000,y				;$B9D24C   |/ Get command param 2 (animation script call address)
	INY					;$B9D24F   |\ Move to next parameter
	INY					;$B9D250   |/
	STY $00,x				;$B9D251   |> Store current animation script address in return variable of sprite
	TAY					;$B9D253   |> Jump to new animation script address
	LDX current_sprite			;$B9D254   |> Get current sprite
	JMP process_anim_script			;$B9D256  /> Continue processing animation script

;animation command 92 (return from animation script subroutine)
animation_command_92:
	INY					;$B9D259  \ \
	LDA $0000,y				;$B9D25A   | | Get command param 1 (variable to use for animation script return)
	AND #$00FF				;$B9D25D   |/
	CLC					;$B9D260   |\
	ADC current_sprite			;$B9D261   | | Add offset to current sprite pointer to get actual address in RAM
	TAX					;$B9D263   |/
	LDY $00,x				;$B9D264   |> Get animation return address from sprite variable and jump back to it
	LDX current_sprite			;$B9D266   |> Get current sprite
	JMP process_anim_script			;$B9D268  /> Continue processing animation script

;animation command 85 (draw graphic A with sprite and graphic B with kong if riding animal)
;parameters:
;00 command (85)
;01 draw duration
;02 graphic a
;04 graphic b

;this is only used in one animation script, when diddy mounts enguarde
;its probably leftover legacy code from DKC 1 that never needed to be changed
animation_command_85:
	LDX current_sprite			;$B9D26B  \ \
	LDA $0000,y				;$B9D26D   | | Get command param 1 (draw time)
	AND #$FF00				;$B9D270   |/
	CLC					;$B9D273   |\
	ADC sprite.animation_timer,x		;$B9D274   | | Add draw time to sprite graphic
	STA sprite.animation_timer,x		;$B9D276   |/
	BPL .apply_graphic			;$B9D278   |
	INY					;$B9D27A   |
	INY					;$B9D27B   |
	INY					;$B9D27C   |
	INY					;$B9D27D   |
	INY					;$B9D27E   |
	INY					;$B9D27F   |
	JMP process_anim_script			;$B9D280  /

.apply_graphic
	LDA $0002,y				;$B9D283  \ \
	STA sprite.current_graphic,x		;$B9D286   |/ Apply first graphic to current sprite
	LDA.l current_player_mount		;$B9D288   |\ Get animal buddy sprite
	BEQ .done				;$B9D28C   |/ If kong isnt mounted, dont apply second graphic
	TAX					;$B9D28E   |> Else get ready to apply a graphic to animal
	LDA.l active_kong_sprite		;$B9D28F   |\
	CMP current_sprite			;$B9D293   | |
	BNE .done				;$B9D295   |/ If this sprite isnt active kong, dont apply second graphic
	LDA $0004,y				;$B9D297   |\
	STA sprite.current_graphic,x		;$B9D29A   |/ Apply second graphic to animal
	STZ sprite.animation_speed,x		;$B9D29C   |> Prevent draw updates on animal
.done						;	   |
	LDX current_sprite			;$B9D29E   |> Get current sprite
	TYA					;$B9D2A0   |\
	CLC					;$B9D2A1   | |
	ADC #$0006				;$B9D2A2   | |
	STA sprite.animation_address,x		;$B9D2A5   |/ Update animation script address, move to next command
	JMP CODE_B9D13D				;$B9D2A7  /

;animation command 86 (draw graphic A with sprite and graphic B with kong if riding animal, with kong position offset)
;parameters:
;00 command (86)
;01 draw duration
;02 graphic a
;04 graphic b
;06 draw x offset
;08 draw y offset

;this command writes kong position offsets to $0D76 and $0D78 (kong animal riding offsets)
;aside from this it is exactly the same as command 85

animation_command_86:
	LDX current_sprite			;$B9D2AA  \ \
	LDA $0000,y				;$B9D2AC   | | Get command param 1 (draw time)
	AND #$FF00				;$B9D2AF   |/
	CLC					;$B9D2B2   |\
	ADC sprite.animation_timer,x		;$B9D2B3   | | Add draw time to sprite graphic
	STA sprite.animation_timer,x		;$B9D2B5   |/
	BPL .apply_graphic			;$B9D2B7   |
	INY					;$B9D2B9   |
	INY					;$B9D2BA   |
	INY					;$B9D2BB   |
	INY					;$B9D2BC   |
	INY					;$B9D2BD   |
	INY					;$B9D2BE   |
	INY					;$B9D2BF   |
	INY					;$B9D2C0   |
	INY					;$B9D2C1   |
	INY					;$B9D2C2   |
	JMP process_anim_script			;$B9D2C3  /

.apply_graphic
	LDA $0002,y				;$B9D2C6  \ \
	STA sprite.current_graphic,x		;$B9D2C9   |/ Apply first graphic to current sprite
	LDA.l current_player_mount		;$B9D2CB   |\ Get animal buddy sprite
	BEQ .done				;$B9D2CF   |/ If kong isnt mounted, dont apply second graphic
	TAX					;$B9D2D1   |> Else get ready to apply a graphic to animal
	LDA.l active_kong_sprite		;$B9D2D2   |\
	CMP current_sprite			;$B9D2D6   | |
	BNE .done				;$B9D2D8   |/ If this sprite isnt active kong, dont apply second graphic
	LDA $0006,y				;$B9D2DA   |\ Get x draw offset from animation
	CLC					;$B9D2DD   | |
	ADC $000D72				;$B9D2DE   | | Add to kong x offset on animal
	STA $000D76				;$B9D2E2   |/ Apply new true x offset of kong on animal
	LDA $0008,y				;$B9D2E6   |\ Get y draw offset from animation
	CLC					;$B9D2E9   | |
	ADC $000D74				;$B9D2EA   | | Add to kong y offset on animal
	STA $000D78				;$B9D2EE   |/ Apply new true y offset of kong on animal
	LDA $0004,y				;$B9D2F2   |\
	STA sprite.current_graphic,x		;$B9D2F5   |/ Apply second graphic to animal
	STZ sprite.animation_speed,x		;$B9D2F7   |> Prevent draw updates on animal
.done						;	   |
	LDX current_sprite			;$B9D2F9   |> Get current sprite
	TYA					;$B9D2FB   |\
	CLC					;$B9D2FC   | |
	ADC #$000A				;$B9D2FD   | |
	STA sprite.animation_address,x		;$B9D300   |/ Update animation script address, move to next command
	JMP CODE_B9D13D				;$B9D302  /

;animation command 87 (draw graphic with current sprite, with kong position offset)
;parameters:
;00 command (87)
;01 draw duration
;02 graphic a
;04 draw x offset
;06 draw y offset

;this command writes kong position offsets to $0D76 and $0D78 (kong animal riding offsets)

animation_command_87:
	LDX current_sprite			;$B9D305  \ \
	LDA $0000,y				;$B9D307   | | Get command param 1 (draw time)
	AND #$FF00				;$B9D30A   |/
	CLC					;$B9D30D   |\
	ADC sprite.animation_timer,x		;$B9D30E   | | Add draw time to sprite graphic
	STA sprite.animation_timer,x		;$B9D310   |/
	BPL .apply_graphic			;$B9D312   |
	INY					;$B9D314   |
	INY					;$B9D315   |
	INY					;$B9D316   |
	INY					;$B9D317   |
	INY					;$B9D318   |
	INY					;$B9D319   |
	INY					;$B9D31A   |
	INY					;$B9D31B   |
	JMP process_anim_script			;$B9D31C  /

.apply_graphic
	LDA $0002,y				;$B9D31F  \ \
	STA sprite.current_graphic,x		;$B9D322   |/ Apply first graphic to current sprite
	LDA.l current_player_mount		;$B9D324   |\ Get animal buddy sprite
	BEQ .done				;$B9D328   |/ If kong isnt mounted, dont apply draw offset
	LDA.l active_kong_sprite		;$B9D32A   |\ Else get active kong
	CMP current_sprite			;$B9D32E   | |
	BNE .done				;$B9D330   |/ If this sprite isnt active kong, dont apply draw offset
	LDA $0004,y				;$B9D332   |\ Get x draw offset from animation
	CLC					;$B9D335   | |
	ADC $000D72				;$B9D336   | | Add to kong x offset on animal
	STA $000D76				;$B9D33A   |/ Apply new true x offset of kong on animal
	LDA $0006,y				;$B9D33E   |\ Get y draw offset from animation
	CLC					;$B9D341   | |
	ADC $000D74				;$B9D342   | | Add to kong y offset on animal
	STA $000D78				;$B9D346   |/ Apply new true y offset of kong on animal
.done						;	   |
	LDX current_sprite			;$B9D34A   |> Get current sprite
	TYA					;$B9D34C   |\
	CLC					;$B9D34D   | |
	ADC #$0008				;$B9D34E   | |
	STA sprite.animation_address,x		;$B9D351   |/ Update animation script address, move to next command
	JMP CODE_B9D13D				;$B9D353  /

;animation command 88 (set kong animal position offset)
;parameters:
;00 command (88)
;01 draw x offset
;03 draw y offset

;this command writes kong position offsets to $0D76 and $0D78 (kong animal riding offsets)

animation_command_88:
	LDA.l current_player_mount		;$B9D356  \ \ Get animal buddy sprite
	BEQ .done				;$B9D35A   |/ If kong isnt mounted, dont apply second graphic
	LDA.l active_kong_sprite		;$B9D35C   |\ Else get active kong
	CMP current_sprite			;$B9D360   | |
	BNE .done				;$B9D362   |/ If this sprite isnt active kong, dont apply draw offset
	LDA $0001,y				;$B9D364   |\ Get x draw offset from animation
	CLC					;$B9D367   | |
	ADC $000D72				;$B9D368   | | Add to kong x offset on animal
	STA $000D76				;$B9D36C   |/ Apply new true x offset of kong on animal
	LDA $0003,y				;$B9D370   |\ Get y draw offset from animation
	CLC					;$B9D373   | |
	ADC $000D74				;$B9D374   | | Add to kong y offset on animal
	STA $000D78				;$B9D378   |/ Apply new true y offset of kong on animal
.done						;	   |
	LDX current_sprite			;$B9D37C   |> Get current sprite
	TYA					;$B9D37E   |\
	CLC					;$B9D37F   | |
	ADC #$0005				;$B9D380   | |
	TAY					;$B9D383   |/ Update animation script address, move to next command
	JMP process_anim_script			;$B9D384  /

;animation command 89 (draw graphic A with sprite and graphic B with held sprite if holding)
;parameters:
;00 command (89)
;01 draw duration
;02 graphic a
;04 graphic b

animation_command_89:
	LDX current_sprite			;$B9D387  \ \
	LDA $0000,y				;$B9D389   | | Get command param 1 (draw time)
	AND #$FF00				;$B9D38C   |/
	CLC					;$B9D38F   |\
	ADC sprite.animation_timer,x		;$B9D390   | | Add draw time to sprite graphic
	STA sprite.animation_timer,x		;$B9D392   |/
	BPL .apply_graphic			;$B9D394   |
	INY					;$B9D396   |
	INY					;$B9D397   |
	INY					;$B9D398   |
	INY					;$B9D399   |
	INY					;$B9D39A   |
	INY					;$B9D39B   |
	JMP process_anim_script			;$B9D39C  / 

.apply_graphic
	LDA $0002,y				;$B9D39F  \ \
	STA sprite.current_graphic,x		;$B9D3A2   |/ Apply first graphic to current sprite
	LDA.l current_held_sprite		;$B9D3A4   |\ Get held sprite
	BEQ .done				;$B9D3A8   |/ If not holding a sprite, dont apply second graphic
	LDA.l active_kong_sprite		;$B9D3AA   |\ Get active kong
	CMP current_sprite			;$B9D3AE   | |
	BNE .done				;$B9D3B0   |/ If this sprite isnt active kong, dont apply second graphic
	LDA.l current_held_sprite		;$B9D3B2   |\
	TAX					;$B9D3B6   |/ Get held sprite
	LDA $0004,y				;$B9D3B7   |\
	STA sprite.current_graphic,x		;$B9D3BA   |/ Apply second graphic to held sprite
	STZ sprite.animation_speed,x		;$B9D3BC   |> Prevent draw updates on held sprite
.done						;	   |
	LDX current_sprite			;$B9D3BE   |\
	TYA					;$B9D3C0   | |
	CLC					;$B9D3C1   | |
	ADC #$0006				;$B9D3C2   | |
	STA sprite.animation_address,x		;$B9D3C5   |/ Update animation script address, move to next command
	JMP CODE_B9D13D				;$B9D3C7  /

animation_command_8A:
	LDX current_sprite			;$B9D3CA  \ 
	LDA $0000,y				;$B9D3CC   |
	AND #$FF00				;$B9D3CF   |
	CLC					;$B9D3D2   |
	ADC sprite.animation_timer,x		;$B9D3D3   |
	STA sprite.animation_timer,x		;$B9D3D5   |
	BPL CODE_B9D3DF				;$B9D3D7   |
if !version == 0				;	   |
	INY					;$B9D3D8   |
	INY					;$B9D3D9   |
	INY					;$B9D3DA   |
	INY					;$B9D3DB   |
	INY					;$B9D3DC   |
	INY					;$B9D3DD   |
	INY					;$B9D3DE   |
	INY					;$B9D3DF   |
	INY					;$B9D3E0   |
	INY					;$B9D3E1   |
else						;	   |
	JSR CODE_B9D3E7				;$B9D3D9   |
endif						;	   |
	JMP process_anim_script			;$B9D3DC  /

CODE_B9D3DF:					;	  \
if !version == 1				;	   |
	JSR CODE_B9D3E7				;$B9D3DF   |
	STA sprite.animation_address,x		;$B9D3E2   |
	JMP CODE_B9D13D				;$B9D3E4   |
endif						;	  /

CODE_B9D3E7:					;	  \
	LDA $0002,y				;$B9D3E7   |
	STA sprite.current_graphic,x		;$B9D3EA   |
	LDA.l current_held_sprite		;$B9D3EC   |
	BEQ CODE_B9D414				;$B9D3F0   |
	LDA.l active_kong_sprite		;$B9D3F2   |
	CMP current_sprite			;$B9D3F6   |
	BNE CODE_B9D414				;$B9D3F8   |
	LDA.l current_held_sprite		;$B9D3FA   |
	TAX					;$B9D3FE   |
	LDA $0004,y				;$B9D3FF   |
	STA sprite.current_graphic,x		;$B9D402   |
	STZ sprite.animation_speed,x		;$B9D404   |
	LDA $0006,y				;$B9D406   |
	STA $000D7C				;$B9D409   |
	LDA $0008,y				;$B9D40D   |
	STA $000D7E				;$B9D410   |
CODE_B9D414:					;	   |
	LDX current_sprite			;$B9D414   |
	TYA					;$B9D416   |
	CLC					;$B9D417   |
	ADC #$000A				;$B9D418   |
if !version == 0				;	   |
	STA sprite.animation_address,x		;$B9D41B   |
	JMP CODE_B9D13D				;$B9D41D   |
else						;	   |
	TAY					;$B9D41B   |
	RTS					;$B9D41C  /
endif

animation_command_8B:
	LDX current_sprite			;$B9D41D  \
	LDA $0000,y				;$B9D41F   |
	AND #$FF00				;$B9D422   |
	CLC					;$B9D425   |
	ADC sprite.animation_timer,x		;$B9D426   |
	STA sprite.animation_timer,x		;$B9D428   |
	BPL CODE_B9D437				;$B9D42A   |
	INY					;$B9D42C   |
	INY					;$B9D42D   |
	INY					;$B9D42E   |
	INY					;$B9D42F   |
	INY					;$B9D430   |
	INY					;$B9D431   |
	INY					;$B9D432   |
	INY					;$B9D433   |
	JMP process_anim_script			;$B9D434  /

CODE_B9D437:
	LDA $0002,y				;$B9D437  \
	STA sprite.current_graphic,x		;$B9D43A   |
	LDA.l current_held_sprite		;$B9D43C   |
	BEQ CODE_B9D458				;$B9D440   |
	LDA.l active_kong_sprite		;$B9D442   |
	CMP current_sprite			;$B9D446   |
	BNE CODE_B9D458				;$B9D448   |
	LDA $0004,y				;$B9D44A   |
	STA $000D7C				;$B9D44D   |
	LDA $0006,y				;$B9D451   |
	STA $000D7E				;$B9D454   |
CODE_B9D458:					;	   |
	LDX current_sprite			;$B9D458   |
	TYA					;$B9D45A   |
	CLC					;$B9D45B   |
	ADC #$0008				;$B9D45C   |
	STA sprite.animation_address,x		;$B9D45F   |
	JMP CODE_B9D13D				;$B9D461  /

animation_command_8C:
	LDA.l current_held_sprite		;$B9D464  \
	BEQ CODE_B9D480				;$B9D468   |
	LDA.l active_kong_sprite		;$B9D46A   |
	CMP current_sprite			;$B9D46E   |
	BNE CODE_B9D480				;$B9D470   |
	LDA $0001,y				;$B9D472   |
	STA $000D7C				;$B9D475   |
	LDA $0003,y				;$B9D479   |
	STA $000D7E				;$B9D47C   |
CODE_B9D480:					;	   |
	LDX current_sprite			;$B9D480   |
	TYA					;$B9D482   |
	CLC					;$B9D483   |
	ADC #$0005				;$B9D484   |
	TAY					;$B9D487   |
	JMP process_anim_script			;$B9D488  /

animation_command_8D:
	LDX current_sprite			;$B9D48B  \
	LDA $0000,y				;$B9D48D   |
	AND #$FF00				;$B9D490   |
	CLC					;$B9D493   |
	ADC sprite.animation_timer,x		;$B9D494   |
	STA sprite.animation_timer,x		;$B9D496   |
	BPL CODE_B9D4A5				;$B9D498   |
	INY					;$B9D49A   |
	INY					;$B9D49B   |
	INY					;$B9D49C   |
	INY					;$B9D49D   |
	INY					;$B9D49E   |
	INY					;$B9D49F   |
	INY					;$B9D4A0   |
	INY					;$B9D4A1   |
	JMP process_anim_script			;$B9D4A2  /

CODE_B9D4A5:
	LDA $0002,y				;$B9D4A5  \
	STA sprite.current_graphic,x		;$B9D4A8   |
	LDA sprite.general_purpose_42,x		;$B9D4AA   |
	BEQ CODE_B9D4BB				;$B9D4AC   |
	TAX					;$B9D4AE   |
	LDA $0004,y				;$B9D4AF   |
	STA sprite.current_graphic,x		;$B9D4B2   |
	LDA $0006,y				;$B9D4B4   |
	STA sprite.general_purpose_44,x		;$B9D4B7   |
	LDX current_sprite			;$B9D4B9   |
CODE_B9D4BB:					;	   |
	TYA					;$B9D4BB   |
	CLC					;$B9D4BC   |
	ADC #$0008				;$B9D4BD   |
	STA sprite.animation_address,x		;$B9D4C0   |
	JMP CODE_B9D13D				;$B9D4C2  /

animation_command_table:
	dw animation_command_80_94		; 80
	dw animation_command_81			; 81
	dw animation_command_82			; 82
	dw animation_command_83			; 83
	dw animation_command_84			; 84
	dw animation_command_85			; 85
	dw animation_command_86			; 86
	dw animation_command_87			; 87
	dw animation_command_88			; 88
	dw animation_command_89			; 89
	dw animation_command_8A			; 8A
	dw animation_command_8B			; 8B
	dw animation_command_8C			; 8C
	dw animation_command_8D			; 8D
	dw animation_command_8E			; 8E
	dw animation_command_8F			; 8F
	dw animation_command_90			; 90
	dw animation_command_91			; 91
	dw animation_command_92			; 92
	dw animation_command_93			; 93
	dw animation_command_80_94		; 94

CODE_B9D4EF:
	LDA.l main_level.type			;$B9D4EF   |
	CMP #!boss_level_type			;$B9D4F3   |
	BNE CODE_B9D4FB				;$B9D4F6   |
	JMP CODE_B9D12B				;$B9D4F8  /

CODE_B9D4FB:
	JMP process_anim_script			;$B9D4FB  /

CODE_B9D4FE:
	LDA sprite.general_purpose_4E,x		;$B9D4FE  \
	AND #$0003				;$B9D500   |
	CMP #$0003				;$B9D503   |
	BEQ CODE_B9D50B				;$B9D506   |
	JMP CODE_B9D12B				;$B9D508  /

CODE_B9D50B:
	JMP process_anim_script			;$B9D50B  /

CODE_B9D50E:
	LDA.l active_kong_sprite		;$B9D50E  \
	TAX					;$B9D512   |
	LDA sprite.y_speed,x			;$B9D513   |
	BPL CODE_B9D51C				;$B9D515   |
	LDX current_sprite			;$B9D517   |
	JMP CODE_B9D12B				;$B9D519  /

CODE_B9D51C:
	LDX current_sprite			;$B9D51C  \
	JMP process_anim_script			;$B9D51E  /

CODE_B9D521:
	LDA sprite.state,x			;$B9D521  \
	CMP #!kong_state_2C			;$B9D523   |
	BNE CODE_B9D52B				;$B9D526   |
	JMP CODE_B9D12B				;$B9D528  /

CODE_B9D52B:
	JMP process_anim_script			;$B9D52B  /

CODE_B9D52E:
	LDA.l active_kong_sprite		;$B9D52E  \
	TAX					;$B9D532   |
	LDA sprite.y_speed,x			;$B9D533   |
	BMI CODE_B9D53C				;$B9D535   |
	LDX current_sprite			;$B9D537   |
	JMP CODE_B9D12B				;$B9D539  /

CODE_B9D53C:
	LDX current_sprite			;$B9D53C  \
	JMP process_anim_script			;$B9D53E  /

CODE_B9D541:
	LDA sprite.terrain_interaction,x	;$B9D541  \
	AND #$0101				;$B9D543   |
	BNE CODE_B9D557				;$B9D546   |
	LDA sprite.y_position,x			;$B9D548   |
	SEC					;$B9D54A   |
	SBC $0017C0				;$B9D54B   |
	CMP #$00F0				;$B9D54F   |
	BPL CODE_B9D557				;$B9D552   |
	JMP CODE_B9D12B				;$B9D554  /

CODE_B9D557:
	JMP process_anim_script			;$B9D557  /

CODE_B9D55A:
	LDA sprite.y_speed,x			;$B9D55A  \
	BPL CODE_B9D561				;$B9D55C   |
	JMP CODE_B9D12B				;$B9D55E  /

CODE_B9D561:
	JMP process_anim_script			;$B9D561  /

CODE_B9D564:
	LDA sprite.y_speed,x			;$B9D564  \
	BMI CODE_B9D56B				;$B9D566   |
	JMP CODE_B9D12B				;$B9D568  /

CODE_B9D56B:
	JMP process_anim_script			;$B9D56B  /

CODE_B9D56E:
	LDA sprite.terrain_interaction,x	;$B9D56E  \
	AND #$1000				;$B9D570   |
	BNE CODE_B9D586				;$B9D573   |
	LDA sprite.ground_y_position,x		;$B9D575   |
	BMI CODE_B9D583				;$B9D577   |
	SEC					;$B9D579   |
	SBC sprite.y_position,x			;$B9D57A   |
	BMI CODE_B9D586				;$B9D57C   |
	CMP #$0048				;$B9D57E   |
	BMI CODE_B9D586				;$B9D581   |
CODE_B9D583:					;	   |
	JMP CODE_B9D12B				;$B9D583  /

CODE_B9D586:
	JMP process_anim_script			;$B9D586  /

CODE_B9D589:
	LDA sprite.terrain_interaction,x	;$B9D589  \
	AND #$1000				;$B9D58B   |
	BNE CODE_B9D5A1				;$B9D58E   |
	LDA sprite.ground_y_position,x		;$B9D590   |
	BMI CODE_B9D59E				;$B9D592   |
	SEC					;$B9D594   |
	SBC sprite.y_position,x			;$B9D595   |
	BMI CODE_B9D5A1				;$B9D597   |
	CMP #$0010				;$B9D599   |
	BMI CODE_B9D5A1				;$B9D59C   |
CODE_B9D59E:					;	   |
	JMP CODE_B9D12B				;$B9D59E  /

CODE_B9D5A1:
	JMP process_anim_script			;$B9D5A1  /

stall_animation_until_grounded:
	LDA sprite.terrain_interaction,x	;$B9D5A4  \
	AND #$0001				;$B9D5A6   |
	BNE CODE_B9D5AE				;$B9D5A9   |
	JMP CODE_B9D12B				;$B9D5AB  /

CODE_B9D5AE:
	JMP process_anim_script			;$B9D5AE  /

CODE_B9D5B1:
	LDA sprite.terrain_interaction,x	;$B9D5B1  \
	AND #$0001				;$B9D5B3   |
	BEQ CODE_B9D5C3				;$B9D5B6   |
	LDA sprite.terrain_attributes,x		;$B9D5B8   |
	AND #$0007				;$B9D5BA   |
	CMP $000088				;$B9D5BD   |
	BNE CODE_B9D5C6				;$B9D5C1   |
CODE_B9D5C3:					;	   |
	JMP CODE_B9D12B				;$B9D5C3  /

CODE_B9D5C6:
	JMP process_anim_script			;$B9D5C6  /

CODE_B9D5C9:
	LDA sprite.x_speed,x			;$B9D5C9  \
	BEQ CODE_B9D5D0				;$B9D5CB   |
	JMP CODE_B9D12B				;$B9D5CD  /

CODE_B9D5D0:
	JMP process_anim_script			;$B9D5D0  /

CODE_B9D5D3:
	LDA.l active_kong_sprite		;$B9D5D3  \
	TAX					;$B9D5D7   |
	LDA sprite.max_x_speed,x		;$B9D5D8   |
	BNE CODE_B9D5E1				;$B9D5DA   |
CODE_B9D5DC:					;	   |
	LDX current_sprite			;$B9D5DC   |
	JMP CODE_B9D12B				;$B9D5DE  /

CODE_B9D5E1:
	LDA sprite.x_speed,x			;$B9D5E1  \
	BPL CODE_B9D5E9				;$B9D5E3   |
	EOR #$FFFF				;$B9D5E5   |
	INC A					;$B9D5E8   |
CODE_B9D5E9:					;	   |
	CMP #$0180				;$B9D5E9   |
	BMI CODE_B9D5DC				;$B9D5EC   |
	LDX current_sprite			;$B9D5EE   |
	JMP process_anim_script			;$B9D5F0  /

check_if_boss_level:
	LDA.l main_level.type			;$B9D5F3  \
	CMP #!boss_level_type			;$B9D5F7   |
	BNE .boss_level				;$B9D5FA   |
	CLC					;$B9D5FC   |
	RTS					;$B9D5FD  /

.boss_level:
	SEC					;$B9D5FE  \
	RTS					;$B9D5FF  /

check_if_not_bonus_or_sub_level:
	LDA.l main_level.type			;$B9D600  \
	CMP #!small_level_type			;$B9D604   |
	BEQ .bonus_or_sub_level			;$B9D607   |
	CMP #!bonus_level_type			;$B9D609   |
	BNE .normal_level			;$B9D60C   |
	LDA.l main_level.bonus_type		;$B9D60E   |
	BEQ .normal_level			;$B9D612   |
.bonus_or_sub_level:				;	   |
	CLC					;$B9D614   |
	RTS					;$B9D615  /

.normal_level:
	SEC					;$B9D616  \
	RTS					;$B9D617  /

check_if_no_web_platforms_are_shooting:
	LDA.l $000B02				;$B9D618  \
	AND #$0040				;$B9D61C   |
	BEQ .no_web_platforms_shooting		;$B9D61F   |
	CLC					;$B9D621   |
	RTS					;$B9D622  /

.no_web_platforms_shooting:
	SEC					;$B9D623  \
	RTS					;$B9D624  /

check_if_player_not_mounted:
	LDA.l current_player_mount		;$B9D625  \
	BNE .not_mounted			;$B9D629   |
	SEC					;$B9D62B   |
	RTS					;$B9D62C  /

.not_mounted:
	CLC					;$B9D62D  \
	RTS					;$B9D62E  /

check_if_current_kong_is_diddy:
	LDA.l active_kong_number		;$B9D62F  \
	BNE .player_is_dixie			;$B9D633   |
	SEC					;$B9D635   |
	RTS					;$B9D636  /

.player_is_dixie:
	CLC					;$B9D637  \
	RTS					;$B9D638  /

check_if_active_kong_has_no_max_x_speed:
	LDA.l active_kong_sprite		;$B9D639  \
	TAX					;$B9D63D   |
	LDA sprite.max_x_speed,x		;$B9D63E   |
	BNE .has_target_x_speed			;$B9D640   |
	SEC					;$B9D642   |
	RTS					;$B9D643  /

.has_target_x_speed:
	CLC					;$B9D644  \
	RTS					;$B9D645  /

;unused
.check_if_away_from_ground:
	LDA sprite.ground_y_position,x		;$B9D646  \
	BMI .away_from_ground			;$B9D648   |
	SEC					;$B9D64A   |
	SBC sprite.y_position,x			;$B9D64B   |
	BMI .close_to_ground			;$B9D64D   |
	CMP #$0030				;$B9D64F   |
	BPL .away_from_ground			;$B9D652   |
.close_to_ground:				;	   |
	CLC					;$B9D654   |
	RTS					;$B9D655  /

.away_from_ground:
	SEC					;$B9D656  \
	RTS					;$B9D657  /

CODE_B9D658:
	PHY					;$B9D658  \
	LDY current_kong_control_variables	;$B9D659   |
	LDA.w kong_control.input_held,y		;$B9D65B   |
	AND #$4000				;$B9D65E   |
	BEQ CODE_B9D671				;$B9D661   |
	LDA active_frame_counter		;$B9D663   |
	SEC					;$B9D665   |
	SBC.w kong_control.y_press_time,y	;$B9D666   |
	CMP #$0015				;$B9D669   |
	BMI CODE_B9D671				;$B9D66C   |
	PLY					;$B9D66E   |
	SEC					;$B9D66F   |
	RTS					;$B9D670  /

CODE_B9D671:
	PLY					;$B9D671  \
	CLC					;$B9D672   |
	RTS					;$B9D673  /

;Called by kong rolling animations
CODE_B9D674:
	LDA sprite.terrain_interaction,x	;$B9D674  \
	AND #$1001				;$B9D676   |
	BNE CODE_B9D67D				;$B9D679   |
	CLC					;$B9D67B   |
	RTS					;$B9D67C  /

CODE_B9D67D:
	SEC					;$B9D67D  \
	RTS					;$B9D67E  /

check_for_vertical_wind_stop:
	LDA.l $0019C4				;$B9D67F  \
	TAX					;$B9D683   |
	LDA sprite.general_purpose_46,x		;$B9D684   |
	BEQ .no_wind				;$B9D686   |
	CLC					;$B9D688   |
	RTS					;$B9D689  /

.no_wind:
	SEC					;$B9D68A  \
	RTS					;$B9D68B  /

;unused
CODE_B9D68C:
	LDA sprite.terrain_interaction,x	;$B9D68C  \
	AND #$1000				;$B9D68E   |
	BNE CODE_B9D6A3				;$B9D691   |
	LDA sprite.ground_y_position,x		;$B9D693   |
	BMI CODE_B9D6A1				;$B9D695   |
	SEC					;$B9D697   |
	SBC sprite.y_position,x			;$B9D698   |
	BMI CODE_B9D6A3				;$B9D69A   |
	CMP #$0048				;$B9D69C   |
	BMI CODE_B9D6A3				;$B9D69F   |
CODE_B9D6A1:					;	   |
	CLC					;$B9D6A1   |
	RTS					;$B9D6A2  /

CODE_B9D6A3:
	SEC					;$B9D6A3  \
	RTS					;$B9D6A4  /

;Called by dixie rolling animation
CODE_B9D6A5:
	LDA sprite.terrain_interaction,x	;$B9D6A5  \
	AND #$1001				;$B9D6A7   |
	BNE CODE_B9D6AE				;$B9D6AA   |
	SEC					;$B9D6AC   |
	RTS					;$B9D6AD  /

CODE_B9D6AE:
	CLC					;$B9D6AE  \
	RTS					;$B9D6AF  /

check_if_moving_down:
	LDX current_sprite			;$B9D6B0  \
	LDA sprite.y_speed,x			;$B9D6B2   |
	BMI .moving_up				;$B9D6B4   |
	SEC					;$B9D6B6   |
	RTS					;$B9D6B7  /

.moving_up:
	CLC					;$B9D6B8  \
	RTS					;$B9D6B9  /

CODE_B9D6BA:
	LDX current_sprite			;$B9D6BA  \
	LDA sprite.max_x_speed,x		;$B9D6BC   |
	BEQ CODE_B9D6C2				;$B9D6BE   |
CODE_B9D6C0:					;	   |
	SEC					;$B9D6C0   |
	RTS					;$B9D6C1  /

CODE_B9D6C2:
	LDA sprite.x_speed,x			;$B9D6C2  \
	BPL CODE_B9D6CA				;$B9D6C4   |
	EOR #$FFFF				;$B9D6C6   |
	INC A					;$B9D6C9   |
CODE_B9D6CA:					;	   |
	AND #$FFC0				;$B9D6CA   |
	BNE CODE_B9D6C0				;$B9D6CD   |
	CLC					;$B9D6CF   |
	RTS					;$B9D6D0  /

CODE_B9D6D1:
	LDY current_kong_control_variables	;$B9D6D1  \
	LDA.w kong_control.unknown_0C,y		;$B9D6D3   |
	AND #$0001				;$B9D6D6   |
	BNE CODE_B9D6DD				;$B9D6D9   |
	SEC					;$B9D6DB   |
	RTS					;$B9D6DC  /

CODE_B9D6DD:
	CLC					;$B9D6DD  \
	RTS					;$B9D6DE  /

CODE_B9D6DF:
	LDX current_sprite			;$B9D6DF  \
	LDA sprite.x_speed,x			;$B9D6E1   |
	BPL CODE_B9D6E9				;$B9D6E3   |
	EOR #$FFFF				;$B9D6E5   |
	INC A					;$B9D6E8   |
CODE_B9D6E9:					;	   |
	CMP #$0201				;$B9D6E9   |
	BPL CODE_B9D6F0				;$B9D6EC   |
	CLC					;$B9D6EE   |
	RTS					;$B9D6EF  /

CODE_B9D6F0:
	SEC					;$B9D6F0  \
	RTS					;$B9D6F1  /

CODE_B9D6F2:
	LDX current_sprite			;$B9D6F2  \
	LDA sprite.x_speed,x			;$B9D6F4   |
	BPL CODE_B9D6FC				;$B9D6F6   |
	EOR #$FFFF				;$B9D6F8   |
	INC A					;$B9D6FB   |
CODE_B9D6FC:					;	   |
	CMP #$00C0				;$B9D6FC   |
	BPL CODE_B9D703				;$B9D6FF   |
	CLC					;$B9D701   |
	RTS					;$B9D702  /

CODE_B9D703:
	SEC					;$B9D703  \
	RTS					;$B9D704  /

CODE_B9D705:
	LDX current_sprite			;$B9D705  \
	CPX inactive_kong_sprite		;$B9D707   |
	BEQ CODE_B9D724				;$B9D70A   |
	LDA current_held_sprite			;$B9D70C   |
	BNE CODE_B9D74E				;$B9D70F   |
	LDA animal_type				;$B9D711   |
	BNE CODE_B9D72D				;$B9D713   |
	LDA game_state_flags			;$B9D715   |
	BMI CODE_B9D726				;$B9D718   |
	STZ sprite.state,x			;$B9D71A   |
CODE_B9D71C:					;	   |
	LDA #$0001				;$B9D71C   |
	JSL set_anim_handle_dixie		;$B9D71F   |
	RTS					;$B9D723  /

CODE_B9D724:
	BRA CODE_B9D71C				;$B9D724  /

CODE_B9D726:
	LDA #!kong_state_1D			;$B9D726  \
	STA sprite.state,x			;$B9D729   |
	BRA CODE_B9D71C				;$B9D72B  /

CODE_B9D72D:
	CMP #!sprite_squawks			;$B9D72D  \
	BEQ CODE_B9D744				;$B9D730   |
	CMP #!sprite_enguarde			;$B9D732   |
	BEQ CODE_B9D749				;$B9D735   |
	LDA #$0009				;$B9D737   |
CODE_B9D73A:					;	   |
	STA sprite.state,x			;$B9D73A   |
	LDA #$005E				;$B9D73C   |
	JSL set_anim_handle_animal_and_dixie	;$B9D73F   |
	RTS					;$B9D743  /

CODE_B9D744:
	LDA #$0010				;$B9D744  \
	BRA CODE_B9D73A				;$B9D747  /

CODE_B9D749:
	LDA #$0050				;$B9D749  \
	BRA CODE_B9D73A				;$B9D74C  /

CODE_B9D74E:
	CMP inactive_kong_sprite		;$B9D74E  \
	BEQ CODE_B9D760				;$B9D751   |
	LDA #$000C				;$B9D753   |
	STA sprite.state,x			;$B9D756   |
	LDA #$0010				;$B9D758   |
	JSL set_anim_handle_dixie		;$B9D75B   |
	RTS					;$B9D75F  /

CODE_B9D760:
	LDA #$0017				;$B9D760  \
	STA sprite.state,x			;$B9D763   |
	LDA #$001D				;$B9D765   |
	JSL set_anim_handle_dixie		;$B9D768   |
	RTS					;$B9D76C  /

CODE_B9D76D:
	LDX current_sprite			;$B9D76D  \
	CPX inactive_kong_sprite		;$B9D76F   |
	BEQ CODE_B9D78C				;$B9D772   |
	LDA current_held_sprite			;$B9D774   |
	BNE CODE_B9D7C7				;$B9D777   |
	LDA animal_type				;$B9D779   |
	BNE CODE_B9D7A6				;$B9D77B   |
	LDA game_state_flags			;$B9D77D   |
	BMI CODE_B9D79F				;$B9D780   |
	STZ sprite.state,x			;$B9D782   |
CODE_B9D784:					;	   |
	LDA #$0003				;$B9D784   |
	JSL set_anim_handle_dixie		;$B9D787   |
	RTS					;$B9D78B  /

CODE_B9D78C:
	LDA game_state_flags			;$B9D78C  \
	BMI CODE_B9D798				;$B9D78F   |
	LDA #!kong_state_22			;$B9D791   |
	STA sprite.state,x			;$B9D794   |
	BRA CODE_B9D784				;$B9D796  /

CODE_B9D798:
	LDA #!kong_state_1E			;$B9D798  \
	STA sprite.state,x			;$B9D79B   |
	BRA CODE_B9D784				;$B9D79D  /

CODE_B9D79F:
	LDA #!kong_state_1D			;$B9D79F  \
	STA sprite.state,x			;$B9D7A2   |
	BRA CODE_B9D784				;$B9D7A4  /

CODE_B9D7A6:
	CMP #!sprite_squawks			;$B9D7A6  \
	BEQ CODE_B9D7BD				;$B9D7A9   |
	CMP #!sprite_enguarde			;$B9D7AB   |
	BEQ CODE_B9D7C2				;$B9D7AE   |
	LDA #$0009				;$B9D7B0   |
CODE_B9D7B3:					;	   |
	STA sprite.state,x			;$B9D7B3   |
	LDA #$0068				;$B9D7B5   |
	JSL set_anim_handle_animal_and_dixie	;$B9D7B8   |
	RTS					;$B9D7BC  /

CODE_B9D7BD:
	LDA #$0010				;$B9D7BD  \
	BRA CODE_B9D7B3				;$B9D7C0  /

CODE_B9D7C2:
	LDA #$0050				;$B9D7C2  \
	BRA CODE_B9D7B3				;$B9D7C5  /

CODE_B9D7C7:
	CMP inactive_kong_sprite		;$B9D7C7  \
	BEQ CODE_B9D7D9				;$B9D7CA   |
	LDA #$000C				;$B9D7CC   |
	STA sprite.state,x			;$B9D7CF   |
	LDA #$0011				;$B9D7D1   |
	JSL set_anim_handle_dixie		;$B9D7D4   |
	RTS					;$B9D7D8  /

CODE_B9D7D9:
	LDA #$0017				;$B9D7D9  \
	STA sprite.state,x			;$B9D7DC   |
	LDA #$001F				;$B9D7DE   |
	JSL set_anim_handle_dixie		;$B9D7E1   |
	RTS					;$B9D7E5  /

CODE_B9D7E6:
	LDX current_sprite			;$B9D7E6  \
	CPX inactive_kong_sprite		;$B9D7E8   |
	BEQ CODE_B9D80C				;$B9D7EB   |
	LDA current_held_sprite			;$B9D7ED   |
	BNE CODE_B9D834				;$B9D7F0   |
	LDA animal_type				;$B9D7F2   |
	BNE CODE_B9D813				;$B9D7F4   |
	LDA game_state_flags			;$B9D7F6   |
	BMI CODE_B9D805				;$B9D7F9   |
	STZ sprite.state,x			;$B9D7FB   |
CODE_B9D7FD:					;	   |
	LDA #$0004				;$B9D7FD   |
	JSL set_anim_handle_dixie		;$B9D800   |
	RTS					;$B9D804  /

CODE_B9D805:
	LDA #!kong_state_1D			;$B9D805  \
	STA sprite.state,x			;$B9D808   |
	BRA CODE_B9D7FD				;$B9D80A  /

CODE_B9D80C:
	LDA #!kong_state_22			;$B9D80C  \
	STA sprite.state,x			;$B9D80F   |
	BRA CODE_B9D7FD				;$B9D811  /

CODE_B9D813:
	CMP #!sprite_squawks			;$B9D813  \
	BEQ CODE_B9D82A				;$B9D816   |
	CMP #!sprite_enguarde			;$B9D818   |
	BEQ CODE_B9D82F				;$B9D81B   |
	LDA #$0009				;$B9D81D   |
CODE_B9D820:					;	   |
	STA sprite.state,x			;$B9D820   |
	LDA #$0068				;$B9D822   |
	JSL set_anim_handle_animal_and_dixie	;$B9D825   |
	RTS					;$B9D829  /

CODE_B9D82A:
	LDA #$0010				;$B9D82A  \
	BRA CODE_B9D820				;$B9D82D  /

CODE_B9D82F:
	LDA #$0050				;$B9D82F  \
	BRA CODE_B9D820				;$B9D832  /

CODE_B9D834:
	CMP inactive_kong_sprite		;$B9D834  \
	BEQ CODE_B9D846				;$B9D837   |
	LDA #$000C				;$B9D839   |
	STA sprite.state,x			;$B9D83C   |
	LDA #$0011				;$B9D83E   |
	JSL set_anim_handle_dixie		;$B9D841   |
	RTS					;$B9D845  /

CODE_B9D846:
	LDA #$0017				;$B9D846  \
	STA sprite.state,x			;$B9D849   |
	LDA #$001F				;$B9D84B   |
	JSL set_anim_handle_dixie		;$B9D84E   |
	RTS					;$B9D852  /

CODE_B9D853:
	CPX inactive_kong_sprite		;$B9D853  \
	BEQ CODE_B9D85C				;$B9D856   |
	JSL CODE_B8B9B8				;$B9D858   |
CODE_B9D85C:					;	   |
	RTS					;$B9D85C  /

CODE_B9D85D:
	LDA main_level.type			;$B9D85D  \
	CMP #!boss_level_type			;$B9D860   |
	BNE CODE_B9D86A				;$B9D863   |
	JSL set_level_end_fade_global		;$B9D865   |
	RTS					;$B9D869  /

CODE_B9D86A:
	RTS					;$B9D86A  /

spawn_diddy_boombox:
	LDA current_held_sprite			;$B9D86B  \ Check if a sprite is being held
	BEQ .spawn				;$B9D86E   | If not, spawn the boombox
	RTS					;$B9D870  / Else return

.spawn:
	LDY #!special_sprite_spawn_id_0004	;$B9D871  \
	JSL spawn_special_sprite_index		;$B9D874   | Spawn diddy boombox
	BCS .return				;$B9D878   | If spawn failed, return
	LDA alternate_sprite			;$B9D87A   |
	STA current_held_sprite			;$B9D87C   | Else set boombox as current held sprite
.return:					;	   |
	RTS					;$B9D87F  / Return

CODE_B9D880:
	LDY #!special_sprite_spawn_id_001E	;$B9D880  \
	JSL spawn_special_sprite_index		;$B9D883   | Spawn rain cloud (spawn animation)
	RTS					;$B9D887  /

CODE_B9D888:
	LDA #$0001				;$B9D888  \
	STA sprite.general_purpose_42,x		;$B9D88B   |
	RTS					;$B9D88D  /

CODE_B9D88E:
	STZ sprite.general_purpose_42,x		;$B9D88E  \
	RTS					;$B9D890  /

CODE_B9D891:
	STZ sprite.x_speed,x			;$B9D891  \
	STZ sprite.max_x_speed,x		;$B9D893   |
	RTS					;$B9D895  /

CODE_B9D896:
	LDY current_held_sprite			;$B9D896  \
	BEQ CODE_B9D8A1				;$B9D899   |
	LDA #$0001				;$B9D89B   |
	STA.w sprite.carry_or_defeat_flags,y	;$B9D89E   |
CODE_B9D8A1:					;	   |
	TYX					;$B9D8A1   |
	JSL CODE_B8D4AE				;$B9D8A2   |
	STZ current_held_sprite			;$B9D8A6   |
	RTS					;$B9D8A9  /

CODE_B9D8AA:
	LDX current_held_sprite			;$B9D8AA  \
	BEQ CODE_B9D8BB				;$B9D8AD   |
	LDA #$0002				;$B9D8AF   |
	STA sprite.carry_or_defeat_flags,x	;$B9D8B2   |
	LDX current_sprite			;$B9D8B4   |
	LDA #$000F				;$B9D8B6   |
	STA sprite.state,x			;$B9D8B9   |
CODE_B9D8BB:					;	   |
	RTS					;$B9D8BB  /

CODE_B9D8BC:
	LDY current_held_sprite			;$B9D8BC  \
	BEQ CODE_B9D91B				;$B9D8BF   |
	LDX current_kong_control_variables	;$B9D8C1   |
	LDA kong_control.input_held,x		;$B9D8C3   |
	AND #$0800				;$B9D8C5   |
	BEQ CODE_B9D91C				;$B9D8C8   |
	LDX current_sprite			;$B9D8CA   |
	LDA #$002C				;$B9D8CC   |
	STA sprite.state,x			;$B9D8CF   |
	LDA #$0010				;$B9D8D1   |
	ORA.w sprite.interaction_flags,y	;$B9D8D4   |
	STA.w sprite.interaction_flags,y	;$B9D8D7   |
	LDA #$002B				;$B9D8DA   |
	STA.w sprite.state,y			;$B9D8DD   |
	LDA #$0008				;$B9D8E0   |
	ORA.w sprite.interaction_flags,y	;$B9D8E3   |
	STA.w sprite.interaction_flags,y	;$B9D8E6   |
	LDY #$0040				;$B9D8E9   |
	LDA [current_sprite_constants],y	;$B9D8EC   |
	LDY current_held_sprite			;$B9D8EE   |
	BIT sprite.oam_property,x		;$B9D8F1   |
	BVC CODE_B9D8F9				;$B9D8F3   |
	EOR #$FFFF				;$B9D8F5   |
	INC A					;$B9D8F8   |
CODE_B9D8F9:					;	   |
	STA.w sprite.x_speed,y			;$B9D8F9   |
	LDA #$0000				;$B9D8FC   |
	STA.w sprite.max_x_speed,y		;$B9D8FF   |
	LDY #$0042				;$B9D902   |
	LDA [current_sprite_constants],y	;$B9D905   |
	LDY current_held_sprite			;$B9D907   |
	STA.w sprite.y_speed,y			;$B9D90A   |
	LDA #$0000				;$B9D90D   |
	STA.w sprite.terrain_interaction,y	;$B9D910   |
	TYX					;$B9D913   |
	JSL CODE_B8D4AE				;$B9D914   |
	STZ current_held_sprite			;$B9D918   |
CODE_B9D91B:					;	   |
	RTS					;$B9D91B  /

CODE_B9D91C:
	LDX current_sprite			;$B9D91C  \
	LDA #$0020				;$B9D91E   |
	STA sprite.state,x			;$B9D921   |
	LDA #$001F				;$B9D923   |
	STA.w sprite.state,y			;$B9D926   |
	LDA #$0010				;$B9D929   |
	ORA.w sprite.interaction_flags,y	;$B9D92C   |
	STA.w sprite.interaction_flags,y	;$B9D92F   |
	LDY #$003C				;$B9D932   |
	LDA [current_sprite_constants],y	;$B9D935   |
	LDY current_held_sprite			;$B9D937   |
	BIT sprite.oam_property,x		;$B9D93A   |
	BVC CODE_B9D942				;$B9D93C   |
	EOR #$FFFF				;$B9D93E   |
	INC A					;$B9D941   |
CODE_B9D942:					;	   |
	STA.w sprite.x_speed,y			;$B9D942   |
	LDA #$0000				;$B9D945   |
	STA.w sprite.max_x_speed,y		;$B9D948   |
	LDY #$003E				;$B9D94B   |
	LDA [current_sprite_constants],y	;$B9D94E   |
	LDY current_held_sprite			;$B9D950   |
	STA.w sprite.y_speed,y			;$B9D953   |
	LDA #$0000				;$B9D956   |
	STA.w sprite.terrain_interaction,y	;$B9D959   |
	TYX					;$B9D95C   |
	JSL CODE_B8D4AE				;$B9D95D   |
	STZ current_held_sprite			;$B9D961   |
	RTS					;$B9D964  /

CODE_B9D965:
	LDA #$003F				;$B9D965  \
	STA sprite.state,x			;$B9D968   |
	LDY current_held_sprite			;$B9D96A   |
	BEQ CODE_B9D9AE				;$B9D96D   |
	LDX current_kong_control_variables	;$B9D96F   |> Get kong control variables for current kong
	LDA kong_control.input_held,x		;$B9D971   |\
	AND #$0800				;$B9D973   | |
	BEQ CODE_B9D9AF				;$B9D976   |/ If UP isnt pressed then throwing sprite sideways
	LDX current_sprite			;$B9D978   |> Else get current sprite
	LDA #$0006				;$B9D97A   |\
	STA.w sprite.carry_or_defeat_flags,y	;$B9D97D   |/ Set sprite carry interaction to thrown upwards
	LDY #$0038				;$B9D980   |
	LDA [current_sprite_constants],y	;$B9D983   |
	LDY current_held_sprite			;$B9D985   |
	BIT sprite.oam_property,x		;$B9D988   |
	BVC .no_flip				;$B9D98A   |
	EOR #$FFFF				;$B9D98C   |
	INC A					;$B9D98F   |
.no_flip					;	   |
	STA.w sprite.x_speed,y			;$B9D990   |
	STA.w sprite.max_x_speed,y		;$B9D993   |
	PHY					;$B9D996   |
	LDY #$003A				;$B9D997   |
	LDA [current_sprite_constants],y	;$B9D99A   |
	PLY					;$B9D99C   |
	STA.w sprite.y_speed,y			;$B9D99D   |
	LDA #$0000				;$B9D9A0   |
	STA.w sprite.terrain_interaction,y	;$B9D9A3   |
	TYX					;$B9D9A6   |
	JSL CODE_B8D4AE				;$B9D9A7   |
	STZ current_held_sprite			;$B9D9AB   |
CODE_B9D9AE:					;	   |
	RTS					;$B9D9AE  /

CODE_B9D9AF:
	LDX current_sprite			;$B9D9AF  \
	LDA #$0004				;$B9D9B1   |\
	STA.w sprite.carry_or_defeat_flags,y	;$B9D9B4   |/ Set sprite carry interaction to thrown sideways
	LDY #$0034				;$B9D9B7   |
	LDA [current_sprite_constants],y	;$B9D9BA   |
	LDY current_held_sprite			;$B9D9BC   |
	BIT sprite.oam_property,x		;$B9D9BF   |
	BVC CODE_B9D9C7				;$B9D9C1   |
	EOR #$FFFF				;$B9D9C3   |
	INC A					;$B9D9C6   |
CODE_B9D9C7:					;	   |
	STA.w sprite.x_speed,y			;$B9D9C7   |
	STA.w sprite.max_x_speed,y		;$B9D9CA   |
	PHY					;$B9D9CD   |
	LDY #$0036				;$B9D9CE   |
	LDA [current_sprite_constants],y	;$B9D9D1   |
	PLY					;$B9D9D3   |
	STA.w sprite.y_speed,y			;$B9D9D4   |
	LDA #$0000				;$B9D9D7   |
	STA.w sprite.terrain_interaction,y	;$B9D9DA   |
	TYX					;$B9D9DD   |
	JSL CODE_B8D4AE				;$B9D9DE   |
	STZ current_held_sprite			;$B9D9E2   |
	RTS					;$B9D9E5  /

CODE_B9D9E6:
	STZ sprite.max_x_speed,x		;$B9D9E6  \
	STZ sprite.x_speed,x			;$B9D9E8   |
	STZ sprite.general_purpose_4E,x		;$B9D9EA   |
	RTS					;$B9D9EC  /

CODE_B9D9ED:
	LDA #$0400				;$B9D9ED  \
	BIT sprite.oam_property,x		;$B9D9F0   |
	BVC CODE_B9D9F8				;$B9D9F2   |
	EOR #$FFFF				;$B9D9F4   |
	INC A					;$B9D9F7   |
CODE_B9D9F8:					;	   |
	STA sprite.max_x_speed,x		;$B9D9F8   |
	STA sprite.general_purpose_4E,x		;$B9D9FA   |
	RTS					;$B9D9FC  /

CODE_B9D9FD:
	NOP					;$B9D9FD  \
	NOP					;$B9D9FE   |
CODE_B9D9FF:					;	   |
	LDA main_level.type			;$B9D9FF   |
	CMP #!small_level_type			;$B9DA02   |
	BEQ CODE_B9DA18				;$B9DA05   |
	CMP #!bonus_level_type			;$B9DA07   |
	BNE CODE_B9DA11				;$B9DA0A   |
	LDA main_level.bonus_type		;$B9DA0C   |
	BEQ CODE_B9DA18				;$B9DA0F   |
CODE_B9DA11:					;	   |
	LDA #$0001				;$B9DA11   |
	JSL transition_song			;$B9DA14   |
CODE_B9DA18:					;	   |
	RTS					;$B9DA18  /

CODE_B9DA19:
	STZ sprite.x_speed,x			;$B9DA19  \
	STZ sprite.max_x_speed,x		;$B9DA1B   |
	STZ sprite.y_speed,x			;$B9DA1D   |
	LDA sprite.x_position,x			;$B9DA1F   |
	STA sprite.general_purpose_44,x		;$B9DA21   |
	LDA #!kong_state_25			;$B9DA23   |
	STA sprite.state,x			;$B9DA26   |
	LDA #$003D				;$B9DA28   |
	JSL set_anim_handle_dixie		;$B9DA2B   |
	RTS					;$B9DA2F  /

CODE_B9DA30:
	JSR CODE_B9E06F				;$B9DA30  \
	LDX current_sprite			;$B9DA33   |
	LDA sprite.x_speed,x			;$B9DA35   |
	BPL CODE_B9DA3D				;$B9DA37   |
	EOR #$FFFF				;$B9DA39   |
	INC A					;$B9DA3C   |
CODE_B9DA3D:					;	   |
	LSR A					;$B9DA3D   |
	LSR A					;$B9DA3E   |
	STA $5E					;$B9DA3F   |
	LSR A					;$B9DA41   |
	CLC					;$B9DA42   |
	ADC $5E					;$B9DA43   |
	CMP #!default_animation_speed		;$B9DA45   |
	BCS CODE_B9DA4D				;$B9DA48   |
	LDA #!default_animation_speed		;$B9DA4A   |
CODE_B9DA4D:					;	   |
	STA sprite.animation_speed,x		;$B9DA4D   |
	RTS					;$B9DA4F  /

CODE_B9DA50:
	LDA #$F900				;$B9DA50  \
	STA sprite.y_speed,x			;$B9DA53   |
	LDA #$0400				;$B9DA55   |
	STA sprite.max_x_speed,x		;$B9DA58   |
	RTS					;$B9DA5A  /

CODE_B9DA5B:
	RTS					;$B9DA5B  |

CODE_B9DA5C:
	JSL CODE_B883D5				;$B9DA5C  \
	RTS					;$B9DA60  /

exit_level_on_death:
	LDA #$810F				;$B9DA61  \
	JSL set_fade_global			;$B9DA64   |
	LDA #$0002				;$B9DA68   |
	TSB game_state_flags			;$B9DA6B   |
	LDA #$FFFF				;$B9DA6E   |
	STA level_destination_number		;$B9DA71   |
	RTS					;$B9DA74  /

CODE_B9DA75:
	LDA #$0003				;$B9DA75  \
if !version == 0				;	   |
	LDY #$0280				;$B9DA78   |
else						;	   |
	LDY #$0100				;$B9DA78   |
endif						;	   |
	JSL enable_bullet_time_global		;$B9DA7B   |
	RTS					;$B9DA7F  /

CODE_B9DA80:
	RTS					;$B9DA80  |

;Unreferenced, would essentially exit the level keeping entrance + no fadeout
CODE_B9DA81:
	LDA level_number			;$B9DA81  \
	STA level_destination_number		;$B9DA83   |
	LDA level_entrance_number		;$B9DA86   |
	STA destination_level_entrance_number	;$B9DA89   |
	LDA #!player_interaction_2D		;$B9DA8C   |
	JSL set_player_interaction_global	;$B9DA8F   |
	RTS					;$B9DA93  /

spawn_diddy_hurt_stars:
	%lda_sound(5, diddy_dizzy)		;$B9DA94  \
	JSL queue_sound_effect			;$B9DA97   | Play dizzy stars sound effect
	LDY #!special_sprite_spawn_id_0016	;$B9DA9B   |
	JSL spawn_special_sprite_index		;$B9DA9E   | Spawn diddy hurt stars
	RTS					;$B9DAA2  /

CODE_B9DAA3:
	LDX current_player_mount		;$B9DAA3  \
	BEQ CODE_B9DAAE				;$B9DAA5   |
	LDA #$0054				;$B9DAA7   |
	JSL set_riding_kong_anim_handle_dixie	;$B9DAAA   |
CODE_B9DAAE:					;	   |
	RTS					;$B9DAAE  /

CODE_B9DAAF:
	LDA #$002C				;$B9DAAF  \
	JSL set_anim_handle_dixie		;$B9DAB2   |
	RTS					;$B9DAB6  /

CODE_B9DAB7:
	CPX inactive_kong_sprite		;$B9DAB7  \
	BEQ CODE_B9DADF				;$B9DABA   |
	LDA sprite.y_speed,x			;$B9DABC   |
	BPL CODE_B9DACD				;$B9DABE   |
	EOR #$FFFF				;$B9DAC0   |
	INC A					;$B9DAC3   |
	LSR A					;$B9DAC4   |
	STA $32					;$B9DAC5   |
	LSR A					;$B9DAC7   |
	CLC					;$B9DAC8   |
	ADC $32					;$B9DAC9   |
	BRA CODE_B9DAD5				;$B9DACB  /

CODE_B9DACD:
	LSR A					;$B9DACD  \
	LSR A					;$B9DACE   |
	STA $32					;$B9DACF   |
	LSR A					;$B9DAD1   |
	CLC					;$B9DAD2   |
	ADC $32					;$B9DAD3   |
CODE_B9DAD5:					;	   |
	CMP #$0070				;$B9DAD5   |
	BPL CODE_B9DADD				;$B9DAD8   |
	LDA #$0070				;$B9DADA   |
CODE_B9DADD:					;	   |
	STA sprite.animation_speed,x		;$B9DADD   |
CODE_B9DADF:					;	   |
	RTS					;$B9DADF  /

CODE_B9DAE0:
	CPX inactive_kong_sprite		;$B9DAE0  \
	BEQ CODE_B9DB00				;$B9DAE3   |
	LDA sprite.max_x_speed,x		;$B9DAE5   |
	BEQ CODE_B9DAF3				;$B9DAE7   |
	LDA sprite.oam_property,x		;$B9DAE9   |
	ASL A					;$B9DAEB   |
	EOR #$FFFF				;$B9DAEC   |
	EOR sprite.max_x_speed,x		;$B9DAEF   |
	BMI CODE_B9DB09				;$B9DAF1   |
CODE_B9DAF3:					;	   |
	LDA sprite.max_y_speed,x		;$B9DAF3   |
	BEQ CODE_B9DB00				;$B9DAF5   |
	BMI CODE_B9DB01				;$B9DAF7   |
	LDA #$0031				;$B9DAF9   |
	JSL set_anim_handle_dixie		;$B9DAFC   |
CODE_B9DB00:					;	   |
	RTS					;$B9DB00  /

CODE_B9DB01:
	LDA #$0030				;$B9DB01  \
	JSL set_anim_handle_dixie		;$B9DB04   |
	RTS					;$B9DB08  /

CODE_B9DB09:
	LDA #$0032				;$B9DB09  \
	JSL set_anim_handle_dixie		;$B9DB0C   |
	RTS					;$B9DB10  /

CODE_B9DB11:
	LDA #$002F				;$B9DB11  \
	JSL set_anim_handle_dixie		;$B9DB14   |
	RTS					;$B9DB18  /

CODE_B9DB19:
	CPX inactive_kong_sprite		;$B9DB19  \
	BEQ CODE_B9DB35				;$B9DB1C   |
	LDA sprite.max_y_speed,x		;$B9DB1E   |
	BNE CODE_B9DB25				;$B9DB20   |
	JMP CODE_B9DB11				;$B9DB22  /

CODE_B9DB25:
	BMI CODE_B9DB36				;$B9DB25  \
	LDY current_kong_control_variables	;$B9DB27   |
	LDA #$0031				;$B9DB29   |
	CMP.w kong_control.animation_id,y	;$B9DB2C   |
	BEQ CODE_B9DB35				;$B9DB2F   |
	JSL set_anim_handle_dixie		;$B9DB31   |
CODE_B9DB35:					;	   |
	RTS					;$B9DB35  /

CODE_B9DB36:
	LDY current_kong_control_variables	;$B9DB36  \
	LDA #$0030				;$B9DB38   |
	CMP.w kong_control.animation_id,y	;$B9DB3B   |
	BEQ CODE_B9DB35				;$B9DB3E   |
	JSL set_anim_handle_dixie		;$B9DB40   |
	RTS					;$B9DB44  /

CODE_B9DB45:
	CPX inactive_kong_sprite		;$B9DB45  \
	BEQ CODE_B9DB5B				;$B9DB48   |
	JSR CODE_B9DAB7				;$B9DB4A   |
	LDA sprite.max_x_speed,x		;$B9DB4D   |
	BEQ CODE_B9DB5B				;$B9DB4F   |
	LDA sprite.oam_property,x		;$B9DB51   |
	ASL A					;$B9DB53   |
	EOR #$FFFF				;$B9DB54   |
	EOR sprite.max_x_speed,x		;$B9DB57   |
	BMI CODE_B9DB5C				;$B9DB59   |
CODE_B9DB5B:					;	   |
	RTS					;$B9DB5B  /

CODE_B9DB5C:
	LDA #$0032				;$B9DB5C  \
	JSL set_anim_handle_dixie		;$B9DB5F   |
	RTS					;$B9DB63  /

CODE_B9DB64:
	LDA #$0043				;$B9DB64  \
	JSL set_anim_handle_dixie		;$B9DB67   |
	RTS					;$B9DB6B  /

CODE_B9DB6C:
	CPX inactive_kong_sprite		;$B9DB6C  \
	BEQ CODE_B9DB87				;$B9DB6F   |
	JSR CODE_B9DB88				;$B9DB71   |
	LDA sprite.max_x_speed,x		;$B9DB74   |
	BEQ CODE_B9DB87				;$B9DB76   |
	LSR A					;$B9DB78   |
	EOR sprite.oam_property,x		;$B9DB79   |
	AND #$4000				;$B9DB7B   |
	BEQ CODE_B9DB87				;$B9DB7E   |
	LDA #$0044				;$B9DB80   |
	JSL set_anim_handle_dixie		;$B9DB83   |
CODE_B9DB87:					;	   |
	RTS					;$B9DB87  /

CODE_B9DB88:
	LDX current_sprite			;$B9DB88  \
	LDA sprite.animation_speed,x		;$B9DB8A   |
	SEC					;$B9DB8C   |
	SBC #$0010				;$B9DB8D   |
	CMP #!default_animation_speed		;$B9DB90   |
	BPL CODE_B9DB98				;$B9DB93   |
	LDA #!default_animation_speed		;$B9DB95   |
CODE_B9DB98:					;	   |
	STA sprite.animation_speed,x		;$B9DB98   |
	RTS					;$B9DB9A  /

CODE_B9DB9B:
	LDA sprite.x_speed,x			;$B9DB9B  \
	BPL CODE_B9DBA3				;$B9DB9D   |
	EOR #$FFFF				;$B9DB9F   |
	INC A					;$B9DBA2   |
CODE_B9DBA3:					;	   |
	CMP #$0140				;$B9DBA3   |
	BCS CODE_B9DBAB				;$B9DBA6   |
	LDA #$0140				;$B9DBA8   |
CODE_B9DBAB:					;	   |
	STA sprite.animation_speed,x		;$B9DBAB   |
	JMP CODE_B9E06C				;$B9DBAD  /

CODE_B9DBB0:
	LDA sprite.ground_y_position,x		;$B9DBB0  \
	BMI CODE_B9DBBE				;$B9DBB2   |
	SEC					;$B9DBB4   |
	SBC sprite.y_position,x			;$B9DBB5   |
	BMI CODE_B9DBBE				;$B9DBB7   |
	CMP #$0010				;$B9DBB9   |
	BMI CODE_B9DBBF				;$B9DBBC   |
CODE_B9DBBE:					;	   |
	RTS					;$B9DBBE  /

CODE_B9DBBF:
	LDA #$0200				;$B9DBBF  \
	STA sprite.animation_speed,x		;$B9DBC2   |
	RTS					;$B9DBC4  /

CODE_B9DBC5:
	STZ sprite.x_speed,x			;$B9DBC5  \
	RTS					;$B9DBC7  /

CODE_B9DBC8:
	LDY current_kong_control_variables	;$B9DBC8  \
	LDA.w kong_control.fast_flag,y		;$B9DBCA   |
	AND #$0004				;$B9DBCD   |
	BNE CODE_B9DBE3				;$B9DBD0   |
	LDX current_sprite			;$B9DBD2   |
	LDA #$FB80				;$B9DBD4   |
	STA sprite.y_speed,x			;$B9DBD7   |
	LDY #$009C				;$B9DBD9   |
	LDA [current_sprite_constants],y	;$B9DBDC   |
	LDX current_kong_control_variables	;$B9DBDE   |
	STA kong_control.gravity_force,x	;$B9DBE0   |
	RTS					;$B9DBE2  /

CODE_B9DBE3:
	LDX current_sprite			;$B9DBE3  \
	LDA #$FB00				;$B9DBE5   |
	STA sprite.y_speed,x			;$B9DBE8   |
	LDY #$009C				;$B9DBEA   |
	LDA [current_sprite_constants],y	;$B9DBED   |
	LDX current_kong_control_variables	;$B9DBEF   |
	STA kong_control.gravity_force,x	;$B9DBF1   |
	RTS					;$B9DBF3  /

CODE_B9DBF4:
	LDA #$FEC0				;$B9DBF4  \
	STA sprite.y_speed,x			;$B9DBF7   |
CODE_B9DBF9:					;	   |
	LDA #$005E				;$B9DBF9   |
	JSL set_anim_handle_animal_and_dixie	;$B9DBFC   |
	RTS					;$B9DC00  /

CODE_B9DC01:
	LDA sprite.max_x_speed,x		;$B9DC01  \
	BEQ CODE_B9DC0F				;$B9DC03   |
	LSR A					;$B9DC05   |
	EOR sprite.oam_property,x		;$B9DC06   |
	AND #$4000				;$B9DC08   |
	EOR sprite.oam_property,x		;$B9DC0B   |
	STA sprite.oam_property,x		;$B9DC0D   |
CODE_B9DC0F:					;	   |
	RTS					;$B9DC0F  /

CODE_B9DC10:
	LDA #$0001				;$B9DC10  \
	TRB RAM_0B02				;$B9DC13   |
	RTS					;$B9DC16  /

CODE_B9DC17:
	LDA sprite.max_x_speed,x		;$B9DC17  \
	BEQ CODE_B9DC25				;$B9DC19   |
	LSR A					;$B9DC1B   |
	EOR sprite.oam_property,x		;$B9DC1C   |
	AND #$4000				;$B9DC1E   |
	EOR sprite.oam_property,x		;$B9DC21   |
	STA sprite.oam_property,x		;$B9DC23   |
CODE_B9DC25:					;	   |
	LDA #$0001				;$B9DC25   |
	TRB RAM_0B02				;$B9DC28   |
	BNE CODE_B9DC2E				;$B9DC2B   |
	RTS					;$B9DC2D  /

CODE_B9DC2E:
	LDA #$008B				;$B9DC2E  \
	JSL set_anim_handle_animal_and_dixie	;$B9DC31   |
	RTS					;$B9DC35  /

CODE_B9DC36:
	LDY #!special_sprite_spawn_id_002A	;$B9DC36  \
	JSL spawn_no_gfx_special_sprite_index	;$B9DC39   |
	BCC CODE_B9DC54				;$B9DC3D   |
	RTS					;$B9DC3F  /

CODE_B9DC40:
	LDY #!special_sprite_spawn_id_0028	;$B9DC40  \
	JSL spawn_no_gfx_special_sprite_index	;$B9DC43   |
	BCC CODE_B9DC54				;$B9DC47   |
	RTS					;$B9DC49  /

CODE_B9DC4A:
	LDY #!special_sprite_spawn_id_002C	;$B9DC4A  \
	JSL spawn_no_gfx_special_sprite_index	;$B9DC4D   |
	BCC CODE_B9DC54				;$B9DC51   |
	RTS					;$B9DC53  /

CODE_B9DC54:
	LDX alternate_sprite			;$B9DC54  \
	LDY current_sprite			;$B9DC56   |
	LDA.w sprite.x_speed,y			;$B9DC58   |
	CLC					;$B9DC5B   |
	ADC sprite.x_speed,x			;$B9DC5C   |
	STA sprite.x_speed,x			;$B9DC5E   |
	%lda_sound(5, squawks_spit)		;$B9DC60   |
	JSL queue_sound_effect			;$B9DC63   |
	RTS					;$B9DC67  /

CODE_B9DC68:
	LDA sprite.x_speed,x			;$B9DC68  \
	BPL CODE_B9DC70				;$B9DC6A   |
	EOR #$FFFF				;$B9DC6C   |
	INC A					;$B9DC6F   |
CODE_B9DC70:					;	   |
	CMP #$0600				;$B9DC70   |
	BMI CODE_B9DC7C				;$B9DC73   |
	LDY #!special_sprite_spawn_id_0054	;$B9DC75   |
	JSL spawn_no_gfx_special_sprite_index	;$B9DC78   |
CODE_B9DC7C:					;	   |
	RTS					;$B9DC7C  /

CODE_B9DC7D:
	LDA sprite.x_speed,x			;$B9DC7D  \
	BPL CODE_B9DC85				;$B9DC7F   |
	EOR #$FFFF				;$B9DC81   |
	INC A					;$B9DC84   |
CODE_B9DC85:					;	   |
	CMP #$0140				;$B9DC85   |
	BCS CODE_B9DC8D				;$B9DC88   |
	LDA #$0140				;$B9DC8A   |
CODE_B9DC8D:					;	   |
	STA sprite.animation_speed,x		;$B9DC8D   |
	JMP CODE_B9E06C				;$B9DC8F  /

CODE_B9DC92:
	RTS					;$B9DC92  |

CODE_B9DC93:
	RTS					;$B9DC93  \

CODE_B9DC94:
	STZ current_held_sprite			;$B9DC94  \
	RTS					;$B9DC97  /

spawn_dixie_guitar:
	LDA current_held_sprite			;$B9DC98  \ Check if a sprite is being held
	BEQ .spawn				;$B9DC9B   | If not, spawn guitar
	RTS					;$B9DC9D  / Else return

.spawn:
	LDY #!special_sprite_spawn_id_0006	;$B9DC9E  \
	JSL spawn_special_sprite_index		;$B9DCA1   | Spawn dixie guitar
	BCS .return				;$B9DCA5   | If spawn failed, return
	LDA alternate_sprite			;$B9DCA7   |
	STA current_held_sprite			;$B9DCA9   | Else set guitar as current held sprite
.return:					;	   |
	RTS					;$B9DCAC  / Return

;Unreferenced
CODE_B9DCAD:
	LDY #!special_sprite_spawn_id_0020	;$B9DCAD  \
	JSL spawn_special_sprite_index		;$B9DCB0   | Spawn unused dixie hurt tears
	RTS					;$B9DCB4  /

CODE_B9DCB5:
	LDA sprite.x_speed,x			;$B9DCB5  \
	BPL CODE_B9DCBD				;$B9DCB7   |
	EOR #$FFFF				;$B9DCB9   |
	INC A					;$B9DCBC   |
CODE_B9DCBD:					;	   |
	CMP #$00E0				;$B9DCBD   |
	BCS CODE_B9DCC5				;$B9DCC0   |
	LDA #$00E0				;$B9DCC2   |
CODE_B9DCC5:					;	   |
	STA sprite.animation_speed,x		;$B9DCC5   |
	JMP CODE_B9E101				;$B9DCC7  /

CODE_B9DCCA:
	JSR CODE_B9E06F				;$B9DCCA  \
	LDX current_sprite			;$B9DCCD   |
	LDA sprite.x_speed,x			;$B9DCCF   |
	BPL CODE_B9DCD7				;$B9DCD1   |
	EOR #$FFFF				;$B9DCD3   |
	INC A					;$B9DCD6   |
CODE_B9DCD7:					;	   |
	CMP #$0140				;$B9DCD7   |
	BCS CODE_B9DCDF				;$B9DCDA   |
	LDA #$0140				;$B9DCDC   |
CODE_B9DCDF:					;	   |
	STA sprite.animation_speed,x		;$B9DCDF   |
	RTS					;$B9DCE1  /

CODE_B9DCE2:
	LDA #!default_animation_speed		;$B9DCE2  \
	STA sprite.animation_speed,x		;$B9DCE5   |
	RTS					;$B9DCE7  /

CODE_B9DCE8:
	LDA #$0027				;$B9DCE8  \
	JSL set_inactive_kong_anim_handle_dixie	;$B9DCEB   |
	RTS					;$B9DCEF  /

CODE_B9DCF0:
	LDA #$0001				;$B9DCF0  \
	STA $0D80				;$B9DCF3   |
	RTS					;$B9DCF6  /

CODE_B9DCF7:
	LDA #$FFFF				;$B9DCF7  \
	STA $0D80				;$B9DCFA   |
	RTS					;$B9DCFD  /

CODE_B9DCFE:
	LDA sprite.x_speed,x			;$B9DCFE  \
	BPL CODE_B9DD06				;$B9DD00   |
	EOR #$FFFF				;$B9DD02   |
	INC A					;$B9DD05   |
CODE_B9DD06:					;	   |
	STA $32					;$B9DD06   |
	LSR A					;$B9DD08   |
	CLC					;$B9DD09   |
	ADC $32					;$B9DD0A   |
	CMP #$0140				;$B9DD0C   |
	BPL CODE_B9DD14				;$B9DD0F   |
	LDA #$0140				;$B9DD11   |
CODE_B9DD14:					;	   |
	STA sprite.animation_speed,x		;$B9DD14   |
	JMP CODE_B9E021				;$B9DD16  /

CODE_B9DD19:
	RTS					;$B9DD19  |

CODE_B9DD1A:
	LDA #$0001				;$B9DD1A  \
	STA $0D80				;$B9DD1D   |
	RTS					;$B9DD20  /

CODE_B9DD21:
	LDY #$0028				;$B9DD21  \
	LDA [current_sprite_constants],y	;$B9DD24   |
	STA sprite.y_speed,x			;$B9DD26   |
	STZ sprite.max_x_speed,x		;$B9DD28   |
	RTS					;$B9DD2A  /

spawn_dixie_hurt_tears:
	%lda_sound(5, dixie_cry)		;$B9DD2B  \
	JSL queue_sound_effect			;$B9DD2E   | Play dixie crying sound effect
	LDY #!special_sprite_spawn_id_0014	;$B9DD32   |
	JSL spawn_special_sprite_index		;$B9DD35   | Spawn dixie hurt tears
	RTS					;$B9DD39  /

CODE_B9DD3A:
	LDA sprite.terrain_interaction,x	;$B9DD3A  \
	AND #$1001				;$B9DD3C   |
	BNE CODE_B9DD5D				;$B9DD3F   |
	LDY current_kong_control_variables	;$B9DD41   |
	LDA.w kong_control.input_held,y		;$B9DD43   |
	AND #$4000				;$B9DD46   |
	BEQ CODE_B9DD4C				;$B9DD49   |
	RTS					;$B9DD4B  /

CODE_B9DD4C:
	LDA #$0001				;$B9DD4C  \
	STA sprite.state,x			;$B9DD4F   |
	JSL set_player_terminal_velocity_global	;$B9DD51   |
	LDA #$0007				;$B9DD55   |
	JSL set_anim_handle_dixie		;$B9DD58   |
	RTS					;$B9DD5C  /

CODE_B9DD5D:
	JSR CODE_B9DFE1				;$B9DD5D  \
	RTS					;$B9DD60  /

CODE_B9DD61:
	CPX inactive_kong_sprite		;$B9DD61  \
	BEQ CODE_B9DD75				;$B9DD64   |
	LDY current_kong_control_variables	;$B9DD66   |
	LDA.w kong_control.fast_flag,y		;$B9DD68   |
	AND #$0004				;$B9DD6B   |
	BNE CODE_B9DD76				;$B9DD6E   |
	LDA #!default_animation_speed		;$B9DD70   |
	STA sprite.animation_speed,x		;$B9DD73   |
CODE_B9DD75:					;	   |
	RTS					;$B9DD75  /

CODE_B9DD76:
	LDA #$0200				;$B9DD76  \
	STA sprite.animation_speed,x		;$B9DD79   |
	RTS					;$B9DD7B  /

CODE_B9DD7C:
	CPX inactive_kong_sprite		;$B9DD7C  \
	BEQ CODE_B9DD8D				;$B9DD7F   |
	LDA #$0037				;$B9DD81   |
	STA sprite.state,x			;$B9DD84   |
	LDA #$0036				;$B9DD86   |
	JSL set_anim_handle_dixie		;$B9DD89   |
CODE_B9DD8D:					;	   |
	RTS					;$B9DD8D  /

CODE_B9DD8E:
	CPX inactive_kong_sprite		;$B9DD8E  \
	BEQ CODE_B9DD9B				;$B9DD91   |
	LDA #$0035				;$B9DD93   |
	STA sprite.state,x			;$B9DD96   |
	JMP CODE_B9DB11				;$B9DD98  /

CODE_B9DD9B:
	RTS					;$B9DD9B  /

CODE_B9DD9C:
	CPX inactive_kong_sprite		;$B9DD9C  \
	BEQ CODE_B9DDAE				;$B9DD9F   |
	LDA sprite.max_y_speed,x		;$B9DDA1   |
	BEQ CODE_B9DDAE				;$B9DDA3   |
	BPL CODE_B9DDAF				;$B9DDA5   |
	LDA #$0037				;$B9DDA7   |
	JSL set_anim_handle_dixie		;$B9DDAA   |
CODE_B9DDAE:					;	   |
	RTS					;$B9DDAE  /

CODE_B9DDAF:
	LDA #$0038				;$B9DDAF  \
	JSL set_anim_handle_dixie		;$B9DDB2   |
	RTS					;$B9DDB6  /

CODE_B9DDB7:
	CPX inactive_kong_sprite		;$B9DDB7  \
	BEQ CODE_B9DDC0				;$B9DDBA   |
	LDA sprite.max_x_speed,x		;$B9DDBC   |
	BNE CODE_B9DDC1				;$B9DDBE   |
CODE_B9DDC0:					;	   |
	RTS					;$B9DDC0  /

CODE_B9DDC1:
	LDA #$003A				;$B9DDC1  \
	JSL set_anim_handle_dixie		;$B9DDC4   |
	RTS					;$B9DDC8  /

CODE_B9DDC9:
	CPX inactive_kong_sprite		;$B9DDC9  \
	BEQ CODE_B9DDD2				;$B9DDCC   |
	LDA sprite.max_x_speed,x		;$B9DDCE   |
	BEQ CODE_B9DDD3				;$B9DDD0   |
CODE_B9DDD2:					;	   |
	RTS					;$B9DDD2  /

CODE_B9DDD3:
	LDA sprite.x_speed,x			;$B9DDD3  \
	BPL CODE_B9DDDB				;$B9DDD5   |
	EOR #$FFFF				;$B9DDD7   |
	INC A					;$B9DDDA   |
CODE_B9DDDB:					;	   |
	CMP #$0080				;$B9DDDB   |
	BPL CODE_B9DDD2				;$B9DDDE   |
	LDA #$0039				;$B9DDE0   |
	JSL set_anim_handle_dixie		;$B9DDE3   |
	RTS					;$B9DDE7  /

CODE_B9DDE8:
	CPX inactive_kong_sprite		;$B9DDE8  \
	BEQ CODE_B9DE0E				;$B9DDEB   |
	LDA sprite.max_x_speed,x		;$B9DDED   |
	BEQ CODE_B9DDFB				;$B9DDEF   |
	LSR A					;$B9DDF1   |
	EOR sprite.oam_property,x		;$B9DDF2   |
	AND #$4000				;$B9DDF4   |
	EOR sprite.oam_property,x		;$B9DDF7   |
	STA sprite.oam_property,x		;$B9DDF9   |
CODE_B9DDFB:					;	   |
	LDA sprite.x_speed,x			;$B9DDFB   |
	BPL CODE_B9DE03				;$B9DDFD   |
	EOR #$FFFF				;$B9DDFF   |
	INC A					;$B9DE02   |
CODE_B9DE03:					;	   |
	LSR A					;$B9DE03   |
	CMP #$0070				;$B9DE04   |
	BPL CODE_B9DE0C				;$B9DE07   |
	LDA #$0070				;$B9DE09   |
CODE_B9DE0C:					;	   |
	STA sprite.animation_speed,x		;$B9DE0C   |
CODE_B9DE0E:					;	   |
	RTS					;$B9DE0E  /

CODE_B9DE0F:
	LDA #$0036				;$B9DE0F  \
	JSL set_anim_handle_dixie		;$B9DE12   |
	RTS					;$B9DE16  /

CODE_B9DE17:
	CPX inactive_kong_sprite		;$B9DE17  \
	BEQ CODE_B9DE33				;$B9DE1A   |
	LDA sprite.max_y_speed,x		;$B9DE1C   |
	BNE CODE_B9DE23				;$B9DE1E   |
	JMP CODE_B9DE0F				;$B9DE20  /

CODE_B9DE23:
	BPL CODE_B9DE34				;$B9DE23  \
	LDY current_kong_control_variables	;$B9DE25   |
	LDA #$0037				;$B9DE27   |
	CMP.w kong_control.animation_id,y	;$B9DE2A   |
	BEQ CODE_B9DE33				;$B9DE2D   |
	JSL set_anim_handle_dixie		;$B9DE2F   |
CODE_B9DE33:					;	   |
	RTS					;$B9DE33  /

CODE_B9DE34:
	LDY current_kong_control_variables	;$B9DE34  \
	LDA #$0038				;$B9DE36   |
	CMP.w kong_control.animation_id,y	;$B9DE39   |
	BEQ CODE_B9DE33				;$B9DE3C   |
	JSL set_anim_handle_dixie		;$B9DE3E   |
	RTS					;$B9DE42  /

CODE_B9DE43:
	LDA sprite.state,x			;$B9DE43  \
	CMP #$0019				;$B9DE45   |
	BEQ CODE_B9DE78				;$B9DE48   |
	CMP #$0002				;$B9DE4A   |
	BNE CODE_B9DE67				;$B9DE4D   |
	LDY current_kong_control_variables	;$B9DE4F   |
	LDA.w kong_control.input_held,y		;$B9DE51   |
	AND #$4000				;$B9DE54   |
	BEQ CODE_B9DE67				;$B9DE57   |
	LDA.w kong_control.roll_event_time,y	;$B9DE59   |
	CMP.w kong_control.y_press_time,y	;$B9DE5C   |
	BNE CODE_B9DE67				;$B9DE5F   |
	LDX current_sprite			;$B9DE61   |
	LDA sprite.max_x_speed,x		;$B9DE63   |
	BNE CODE_B9DE78				;$B9DE65   |
CODE_B9DE67:					;	   |
	STZ sprite.x_speed,x			;$B9DE67   |
	STZ sprite.max_x_speed,x		;$B9DE69   |
	LDA #$0004				;$B9DE6B   |
	STA sprite.state,x			;$B9DE6E   |
	LDA sprite.animation_flags,x		;$B9DE70   |
	AND #$B7FF				;$B9DE72   |
	STA sprite.animation_flags,x		;$B9DE75   |
	RTS					;$B9DE77  /

CODE_B9DE78:
	JSR CODE_B9DFE5				;$B9DE78  \
	RTS					;$B9DE7B  /

CODE_B9DE7C:
	LDA sprite.state,x			;$B9DE7C  \
	CMP #$0019				;$B9DE7E   |
	BEQ CODE_B9DEAA				;$B9DE81   |
	CMP #$0002				;$B9DE83   |
	BNE CODE_B9DEA0				;$B9DE86   |
	LDY current_kong_control_variables	;$B9DE88   |
	LDA.w kong_control.input_held,y		;$B9DE8A   |
	AND #$4000				;$B9DE8D   |
	BEQ CODE_B9DEA0				;$B9DE90   |
	LDA.w kong_control.roll_event_time,y	;$B9DE92   |
	CMP.w kong_control.y_press_time,y	;$B9DE95   |
	BNE CODE_B9DEA0				;$B9DE98   |
	LDX current_sprite			;$B9DE9A   |
	LDA sprite.max_x_speed,x		;$B9DE9C   |
	BNE CODE_B9DEAA				;$B9DE9E   |
CODE_B9DEA0:					;	   |
	STZ sprite.x_speed,x			;$B9DEA0   |
	STZ sprite.max_x_speed,x		;$B9DEA2   |
	LDA #$0004				;$B9DEA4   |
	STA sprite.state,x			;$B9DEA7   |
	RTS					;$B9DEA9  /

CODE_B9DEAA:
	JSR CODE_B9DFE5				;$B9DEAA  \
	RTS					;$B9DEAD  /

CODE_B9DEAE:
	LDY current_kong_control_variables	;$B9DEAE  \
	LDA.w kong_control.roll_speed,y		;$B9DEB0   |
	BIT sprite.oam_property,x		;$B9DEB3   |
	BVC CODE_B9DEBB				;$B9DEB5   |
	EOR #$FFFF				;$B9DEB7   |
	INC A					;$B9DEBA   |
CODE_B9DEBB:					;	   |
	STA sprite.max_x_speed,x		;$B9DEBB   |
	RTS					;$B9DEBD  /

CODE_B9DEBE:
	STZ sprite.x_speed,x			;$B9DEBE  \
	RTS					;$B9DEC0  /

CODE_B9DEC1:
	LDA sprite.y_speed,x			;$B9DEC1  \
	BPL CODE_B9DEC6				;$B9DEC3   |
	RTS					;$B9DEC5  /

CODE_B9DEC6:
	LDA #$0200				;$B9DEC6  \
	STA sprite.animation_speed,x		;$B9DEC9   |
	RTS					;$B9DECB  /

CODE_B9DECC:
	LDA sprite.ground_y_position,x		;$B9DECC  \
	BMI CODE_B9DEDA				;$B9DECE   |
	SEC					;$B9DED0   |
	SBC sprite.y_position,x			;$B9DED1   |
	BMI CODE_B9DEE1				;$B9DED3   |
	CMP #$0010				;$B9DED5   |
	BMI CODE_B9DEDB				;$B9DED8   |
CODE_B9DEDA:					;	   |
	RTS					;$B9DEDA  /

CODE_B9DEDB:
	LDA #$0200				;$B9DEDB  \
	STA sprite.animation_speed,x		;$B9DEDE   |
	RTS					;$B9DEE0  /

CODE_B9DEE1:
	LDA #$0300				;$B9DEE1  \
	STA sprite.animation_speed,x		;$B9DEE4   |
	RTS					;$B9DEE6  /

CODE_B9DEE7:
	LDA #$000D				;$B9DEE7  \
	JSL set_anim_handle_dixie		;$B9DEEA   |
	RTS					;$B9DEEE  /

CODE_B9DEEF:
	LDA current_held_sprite			;$B9DEEF  \
	BNE CODE_B9DF32				;$B9DEF2   |
	LDA animal_type				;$B9DEF4   |
	BNE CODE_B9DF13				;$B9DEF6   |
	STZ sprite.state,x			;$B9DEF8   |
	LDA #$0008				;$B9DEFA   |
	JSL set_anim_handle_dixie		;$B9DEFD   |
	LDY current_kong_control_variables	;$B9DF01   |
	LDA.w kong_control.fast_flag,y		;$B9DF03   |
	AND #$0004				;$B9DF06   |
	BEQ CODE_B9DF12				;$B9DF09   |
	LDX current_sprite			;$B9DF0B   |
	LDA #$0200				;$B9DF0D   |
	STA sprite.animation_speed,x		;$B9DF10   |
CODE_B9DF12:					;	   |
	RTS					;$B9DF12  /

CODE_B9DF13:
	LDA sprite.state,x			;$B9DF13  \
	CMP #!kong_state_4F			;$B9DF15   |
	BEQ CODE_B9DF27				;$B9DF18   |
	LDA #!kong_state_09			;$B9DF1A   |
	STA sprite.state,x			;$B9DF1D   |
	LDA #$007C				;$B9DF1F   |
	JSL set_anim_handle_animal_and_dixie	;$B9DF22   |
	RTS					;$B9DF26  /

CODE_B9DF27:
	LDA #$0068				;$B9DF27  \
	JSL set_anim_handle_animal_and_dixie	;$B9DF2A   |
	JSR CODE_B9DAA3				;$B9DF2E   |
	RTS					;$B9DF31  /

CODE_B9DF32:
	CMP inactive_kong_sprite		;$B9DF32  \
	BEQ CODE_B9DF44				;$B9DF35   |
	LDA #$000C				;$B9DF37   |
	STA sprite.state,x			;$B9DF3A   |
	LDA #$0018				;$B9DF3C   |
	JSL set_anim_handle_dixie		;$B9DF3F   |
	RTS					;$B9DF43  /

CODE_B9DF44:
	LDA #$0017				;$B9DF44  \
	STA sprite.state,x			;$B9DF47   |
	LDA #$0024				;$B9DF49   |
	JSL set_anim_handle_dixie		;$B9DF4C   |
	RTS					;$B9DF50  /

CODE_B9DF51:
	LDA current_held_sprite			;$B9DF51  \
	BNE CODE_B9DF7A				;$B9DF54   |
	LDA animal_type				;$B9DF56   |
	BNE CODE_B9DF6A				;$B9DF58   |
	JSR CODE_B9DFB6				;$B9DF5A   |
	LDA #$0006				;$B9DF5D   |
	STA sprite.state,x			;$B9DF60   |
	LDA #$0006				;$B9DF62   |
	JSL set_anim_handle_dixie		;$B9DF65   |
	RTS					;$B9DF69  /

CODE_B9DF6A:
	JSR CODE_B9DFB6				;$B9DF6A  \
	LDA #$000B				;$B9DF6D   |
	STA sprite.state,x			;$B9DF70   |
	LDA #$0072				;$B9DF72   |
	JSL set_anim_handle_animal_and_dixie	;$B9DF75   |
	RTS					;$B9DF79  /

CODE_B9DF7A:
	CMP inactive_kong_sprite		;$B9DF7A  \
	BEQ CODE_B9DF9E				;$B9DF7D   |
	LDA active_kong_number			;$B9DF7F   |
	BEQ CODE_B9DF8A				;$B9DF82   |
	LDA #$0001				;$B9DF84   |
	STA $0D80				;$B9DF87   |
CODE_B9DF8A:					;	   |
	LDY #$0008				;$B9DF8A   |
	LDA [current_sprite_constants],y	;$B9DF8D   |
	STA sprite.y_speed,x			;$B9DF8F   |
	LDA #$000E				;$B9DF91   |
	STA sprite.state,x			;$B9DF94   |
	LDA #$0016				;$B9DF96   |
	JSL set_anim_handle_dixie		;$B9DF99   |
	RTS					;$B9DF9D  /

CODE_B9DF9E:
	LDY #$0008				;$B9DF9E  \
	LDA [current_sprite_constants],y	;$B9DFA1   |
	STA sprite.y_speed,x			;$B9DFA3   |
	LDA #$001B				;$B9DFA5   |
	STA sprite.state,x			;$B9DFA8   |
	LDA #$0022				;$B9DFAA   |
	JSL set_anim_handle_dixie		;$B9DFAD   |
	RTS					;$B9DFB1  /

CODE_B9DFB2:
	JSR CODE_B9DFB6				;$B9DFB2  \
	RTL					;$B9DFB5  /

CODE_B9DFB6:
	LDX current_sprite			;$B9DFB6  \
	LDA animal_type				;$B9DFB8   |
	BNE CODE_B9DFC4				;$B9DFBA   |
	LDY #$0008				;$B9DFBC   |
	LDA [current_sprite_constants],y	;$B9DFBF   |
	STA sprite.y_speed,x			;$B9DFC1   |
	RTS					;$B9DFC3  /

CODE_B9DFC4:
	SEC					;$B9DFC4  \
	SBC #$000C				;$B9DFC5   |
	LSR A					;$B9DFC8   |
	TAY					;$B9DFC9   |
	LDA [current_sprite_constants],y	;$B9DFCA   |
	STA sprite.y_speed,x			;$B9DFCC   |
	RTS					;$B9DFCE  /

CODE_B9DFCF:
	JSR CODE_B9DFD3				;$B9DFCF  \
	RTL					;$B9DFD2  /

CODE_B9DFD3:
	LDX current_sprite			;$B9DFD3  \
	LDA sprite.terrain_interaction,x	;$B9DFD5   |
	AND #$0001				;$B9DFD7   |
	BNE CODE_B9DFE5				;$B9DFDA   |
	JSL CODE_B8B9B4				;$B9DFDC   |
	RTS					;$B9DFE0  /

CODE_B9DFE1:
	JSL set_player_terminal_velocity_global	;$B9DFE1  \
CODE_B9DFE5:					;	   |
	LDX current_sprite			;$B9DFE5   |
	CPX inactive_kong_sprite		;$B9DFE7   |
	BEQ CODE_B9E000				;$B9DFEA   |
	JSR CODE_B9E128				;$B9DFEC   |
	TXY					;$B9DFEF   |
	TAX					;$B9DFF0   |
	JMP (DATA_B9DFF4,x)			;$B9DFF1  /

DATA_B9DFF4:
	dw CODE_B9E000
	dw CODE_B9E003
	dw CODE_B9E003
	dw CODE_B9E003
	dw CODE_B9E003
	dw CODE_B9E003


CODE_B9E000:
	JMP CODE_B9D705				;$B9E000  /

CODE_B9E003:
	LDY current_kong_control_variables	;$B9E003  \
	LDA.w kong_control.fast_flag,y		;$B9E005   |
	AND #$0004				;$B9E008   |
	BNE CODE_B9E010				;$B9E00B   |
	JMP CODE_B9D76D				;$B9E00D  /

CODE_B9E010:
	JMP CODE_B9D7E6				;$B9E010  /

CODE_B9E013:
	CPX inactive_kong_sprite		;$B9E013  \
	BNE CODE_B9E019				;$B9E016   |
	RTS					;$B9E018  /

CODE_B9E019:
	LDA sprite.oam_property,x		;$B9E019  \
	EOR #$4000				;$B9E01B   |
	STA sprite.oam_property,x		;$B9E01E   |
	RTS					;$B9E020  /

CODE_B9E021:
	LDX current_sprite			;$B9E021  \
	LDA sprite.max_x_speed,x		;$B9E023   |
	BEQ CODE_B9E06A				;$B9E025   |
	LSR A					;$B9E027   |
	EOR sprite.oam_property,x		;$B9E028   |
	AND #$4000				;$B9E02A   |
	BEQ CODE_B9E06A				;$B9E02D   |
	LDA current_held_sprite			;$B9E02F   |
	BNE CODE_B9E041				;$B9E032   |
	LDA animal_type				;$B9E034   |
	BNE CODE_B9E058				;$B9E036   |
	LDA #$0002				;$B9E038   |
	JSL set_anim_handle_dixie		;$B9E03B   |
	SEC					;$B9E03F   |
	RTS					;$B9E040  /

CODE_B9E041:
	CMP inactive_kong_sprite		;$B9E041  \
	BEQ CODE_B9E04F				;$B9E044   |
	LDA #$0012				;$B9E046   |
	JSL set_anim_handle_dixie		;$B9E049   |
	SEC					;$B9E04D   |
	RTS					;$B9E04E  /

CODE_B9E04F:
	LDA #$001E				;$B9E04F  \
	JSL set_anim_handle_dixie		;$B9E052   |
	SEC					;$B9E056   |
	RTS					;$B9E057  /

CODE_B9E058:
	CMP #!sprite_rattly			;$B9E058  \
	BEQ CODE_B9E066				;$B9E05B   |
	LDA #$0063				;$B9E05D   |
	JSL set_anim_handle_animal_and_dixie	;$B9E060   |
	SEC					;$B9E064   |
	RTS					;$B9E065  /

CODE_B9E066:
	JSR CODE_B9E019				;$B9E066  \
	RTS					;$B9E069  /

CODE_B9E06A:
	CLC					;$B9E06A  \
	RTS					;$B9E06B  /

CODE_B9E06C:
	JMP CODE_B9E021				;$B9E06C  /

CODE_B9E06F:
	JSR CODE_B9E021				;$B9E06F  \
	BCC CODE_B9E079				;$B9E072   |
	LDA #$0180				;$B9E074   |
	STA sprite.animation_speed,x		;$B9E077   |
CODE_B9E079:					;	   |
	RTS					;$B9E079  /

CODE_B9E07A:
	LDY current_kong_control_variables	;$B9E07A  \
	LDA.w kong_control.fast_flag,y		;$B9E07C   |
	AND #$0004				;$B9E07F   |
	STA $5E					;$B9E082   |
	JSR CODE_B9E128				;$B9E084   |
	TXY					;$B9E087   |
	TAX					;$B9E088   |
	JMP (DATA_B9E08C,x)			;$B9E089  /

DATA_B9E08C:
	dw CODE_B9E098
	dw CODE_B9E09B
	dw CODE_B9E0A1
	dw CODE_B9E0A1
	dw CODE_B9E0A1
	dw CODE_B9E0A1


CODE_B9E098:
	JMP CODE_B9D705				;$B9E098  /

CODE_B9E09B:
	JSR CODE_B9E0F3				;$B9E09B  \
	BEQ CODE_B9E098				;$B9E09E   |
CODE_B9E0A0:					;	   |
	RTS					;$B9E0A0  /

CODE_B9E0A1:
	LDA $5E					;$B9E0A1  \
	BNE CODE_B9E0A0				;$B9E0A3   |
	JMP CODE_B9D76D				;$B9E0A5  /

CODE_B9E0A8:
	LDX current_sprite			;$B9E0A8  \
	CPX inactive_kong_sprite		;$B9E0AA   |
	BEQ CODE_B9E0C4				;$B9E0AD   |
CODE_B9E0AF:					;	   |
	JSR CODE_B9E128				;$B9E0AF   |
	TXY					;$B9E0B2   |
	TAX					;$B9E0B3   |
	JMP (DATA_B9E0B7,x)			;$B9E0B4  /

DATA_B9E0B7:
	dw CODE_B9E0C3
	dw CODE_B9E0C3
	dw CODE_B9E0D0
	dw CODE_B9E0D0
	dw CODE_B9E0D0
	dw CODE_B9E0D0


CODE_B9E0C3:
	RTS					;$B9E0C3  /

CODE_B9E0C4:
	LDA game_state_flags			;$B9E0C4  \
	LDA #$C000				;$B9E0C7   |
	CMP #$C000				;$B9E0CA   |
	BEQ CODE_B9E0AF				;$B9E0CD   |
	RTS					;$B9E0CF  /

CODE_B9E0D0:
	LDY current_kong_control_variables	;$B9E0D0  \
	LDA.w kong_control.fast_flag,y		;$B9E0D2   |
	AND #$0004				;$B9E0D5   |
	BNE CODE_B9E0DD				;$B9E0D8   |
	JMP CODE_B9D76D				;$B9E0DA  /

CODE_B9E0DD:
	JMP CODE_B9D7E6				;$B9E0DD  /

CODE_B9E0E0:
	JSR CODE_B9E128				;$B9E0E0  \
	BEQ CODE_B9E0F0				;$B9E0E3   |
	CMP #$0002				;$B9E0E5   |
	BEQ CODE_B9E0EB				;$B9E0E8   |
CODE_B9E0EA:					;	   |
	RTS					;$B9E0EA  /

CODE_B9E0EB:
	JSR CODE_B9E0F3				;$B9E0EB  \
	BNE CODE_B9E0EA				;$B9E0EE   |
CODE_B9E0F0:					;	   |
	JMP CODE_B9D705				;$B9E0F0  /

CODE_B9E0F3:
	LDX current_sprite			;$B9E0F3  \
	LDA sprite.x_speed,x			;$B9E0F5   |
	BPL CODE_B9E0FD				;$B9E0F7   |
	EOR #$FFFF				;$B9E0F9   |
	INC A					;$B9E0FC   |
CODE_B9E0FD:					;	   |
	AND #$FFE0				;$B9E0FD   |
	RTS					;$B9E100  /

CODE_B9E101:
	JSR CODE_B9E021				;$B9E101  \
	BCS CODE_B9E127				;$B9E104   |
	JSR CODE_B9E128				;$B9E106   |
	TXY					;$B9E109   |
	TAX					;$B9E10A   |
	JMP (DATA_B9E10E,x)			;$B9E10B  /

DATA_B9E10E:
	dw CODE_B9E127
	dw CODE_B9E127
	dw CODE_B9E11A
	dw CODE_B9E11A
	dw CODE_B9E11A
	dw CODE_B9E11A


CODE_B9E11A:
	LDY current_kong_control_variables	;$B9E11A  \
	LDA.w kong_control.fast_flag,y		;$B9E11D   |
	AND #$0004				;$B9E11F   |
	BEQ CODE_B9E127				;$B9E122   |
	JMP CODE_B9D7E6				;$B9E124  /

CODE_B9E127:
	RTS					;$B9E127  /

CODE_B9E128:
	LDA sprite.max_x_speed,x		;$B9E128  \
	BNE CODE_B9E138				;$B9E12A   |
	LDA sprite.x_speed,x			;$B9E12C   |
	BNE CODE_B9E134				;$B9E12E   |
	LDA #$0000				;$B9E130   |
	RTS					;$B9E133  /

CODE_B9E134:
	LDA #$0002				;$B9E134  \
	RTS					;$B9E137  /

CODE_B9E138:
	LDA sprite.x_speed,x			;$B9E138  \
	BNE CODE_B9E140				;$B9E13A   |
	LDA #$0004				;$B9E13C   |
	RTS					;$B9E13F  /

CODE_B9E140:
	BPL CODE_B9E146				;$B9E140  \
	EOR #$FFFF				;$B9E142   |
	INC A					;$B9E145   |
CODE_B9E146:					;	   |
	STA $32					;$B9E146   |
	LDA sprite.max_x_speed,x		;$B9E148   |
	BPL CODE_B9E150				;$B9E14A   |
	EOR #$FFFF				;$B9E14C   |
	INC A					;$B9E14F   |
CODE_B9E150:					;	   |
	CMP $32					;$B9E150   |
	BEQ CODE_B9E15E				;$B9E152   |
	BCS CODE_B9E15A				;$B9E154   |
	LDA #$0006				;$B9E156   |
	RTS					;$B9E159  /

CODE_B9E15A:
	LDA #$0008				;$B9E15A  \
	RTS					;$B9E15D  /

CODE_B9E15E:
	LDA #$000A				;$B9E15E  \
	RTS					;$B9E161  /

CODE_B9E162:
	STZ current_held_sprite			;$B9E162  \
	LDX inactive_kong_sprite		;$B9E165   |
	LDY active_kong_sprite			;$B9E168   |
	LDA.w sprite.y_position,y		;$B9E16B   |
	SEC					;$B9E16E   |
	SBC #$0038				;$B9E16F   |
	STA sprite.y_position,x			;$B9E172   |
	LDA.w sprite.x_position,y		;$B9E174   |
	STA sprite.x_position,x			;$B9E177   |
	LDA #$FD40				;$B9E179   |
	STA sprite.y_speed,x			;$B9E17C   |
	LDA #$FFF0				;$B9E17E   |
	BIT sprite.oam_property,x		;$B9E181   |
	BVC CODE_B9E189				;$B9E183   |
	EOR #$FFFF				;$B9E185   |
	INC A					;$B9E188   |
CODE_B9E189:					;	   |
	STA sprite.x_speed,x			;$B9E189   |
	LDA #$004D				;$B9E18B   |
	STA sprite.state,x			;$B9E18E   |
	LDA #$004B				;$B9E190   |
	JSL set_inactive_kong_anim_handle_dixie	;$B9E193   |
	RTS					;$B9E197  /

CODE_B9E198:
	LDA $0D66				;$B9E198  \
	SEC					;$B9E19B   |
	SBC sprite.x_position,x			;$B9E19C   |
	JSR CODE_B9E1B4				;$B9E19E   |
	STA sprite.x_speed,x			;$B9E1A1   |
	LDA $0D6A				;$B9E1A3   |
	SEC					;$B9E1A6   |
	SBC sprite.y_position,x			;$B9E1A7   |
	JSR CODE_B9E1B4				;$B9E1A9   |
	STA sprite.y_speed,x			;$B9E1AC   |
	LDA #$004B				;$B9E1AE   |
	STA sprite.state,x			;$B9E1B1   |
	RTS					;$B9E1B3  /

CODE_B9E1B4:
	BPL CODE_B9E1C2				;$B9E1B4  \
	EOR #$FFFF				;$B9E1B6   |
	INC A					;$B9E1B9   |
	JSR CODE_B9E1C2				;$B9E1BA   |
	EOR #$FFFF				;$B9E1BD   |
	INC A					;$B9E1C0   |
	RTS					;$B9E1C1  /

CODE_B9E1C2:
	ASL A					;$B9E1C2  \
	ASL A					;$B9E1C3   |
	ASL A					;$B9E1C4   |
	ASL A					;$B9E1C5   |
	CMP #$0004				;$B9E1C6   |
	BPL CODE_B9E1CE				;$B9E1C9   |
	LDA #$0004				;$B9E1CB   |
CODE_B9E1CE:					;	   |
	RTS					;$B9E1CE  /

CODE_B9E1CF:
	LDA #$0010				;$B9E1CF  \
	BIT sprite.oam_property,x		;$B9E1D2   |
	BVC CODE_B9E1DA				;$B9E1D4   |
	EOR #$FFFF				;$B9E1D6   |
	INC A					;$B9E1D9   |
CODE_B9E1DA:					;	   |
	CLC					;$B9E1DA   |
	ADC sprite.x_position,x			;$B9E1DB   |
	STA sprite.x_position,x			;$B9E1DD   |
	JMP CODE_B9E198				;$B9E1DF  /

CODE_B9E1E2:
	STZ current_held_sprite			;$B9E1E2  \
	LDX inactive_kong_sprite		;$B9E1E5   |
	LDA sprite.y_position,x			;$B9E1E8   |
	SEC					;$B9E1EA   |
	SBC #$001D				;$B9E1EB   |
	STA sprite.y_position,x			;$B9E1EE   |
	LDA #$FB00				;$B9E1F0   |
	STA sprite.y_speed,x			;$B9E1F3   |
	LDA #$FF40				;$B9E1F5   |
	BIT sprite.oam_property,x		;$B9E1F8   |
	BVC CODE_B9E200				;$B9E1FA   |
	EOR #$FFFF				;$B9E1FC   |
	INC A					;$B9E1FF   |
CODE_B9E200:					;	   |
	STA sprite.x_speed,x			;$B9E200   |
	LDA #$004D				;$B9E202   |
	STA sprite.state,x			;$B9E205   |
	LDA #$0019				;$B9E207   |
	JSL set_inactive_kong_anim_handle_dixie	;$B9E20A   |
	RTS					;$B9E20E  /

CODE_B9E20F:
	LDX current_sprite			;$B9E20F  \
	LDA #$000C				;$B9E211   |
	JSL disable_enemy_damage_global		;$B9E214   |
	RTS					;$B9E218  /

;Rambi attack related, possible special hitbox injection
CODE_B9E219:
	LDA #$0008				;$B9E219  \
	JSL init_special_player_clipping	;$B9E21C   |
	RTS					;$B9E220  /

CODE_B9E221:
	LDA sprite.terrain_interaction,x	;$B9E221  \
	AND #$1001				;$B9E223   |
	CMP #$0001				;$B9E226   |
	BEQ CODE_B9E22C				;$B9E229   |
	RTS					;$B9E22B  /

CODE_B9E22C:
	LDA #$FC00				;$B9E22C  \
	STA sprite.y_speed,x			;$B9E22F   |
	LDA #$000A				;$B9E231   |
	STA sprite.state,x			;$B9E234   |
	RTS					;$B9E236  /

CODE_B9E237:
	LDY #!special_sprite_spawn_id_0054	;$B9E237  \
	JSL spawn_no_gfx_special_sprite_index	;$B9E23A   |
	RTS					;$B9E23E  /

CODE_B9E23F:
	LDA sprite.max_x_speed,x		;$B9E23F  \
	BNE CODE_B9E24F				;$B9E241   |
	LDY active_kong_control_variables	;$B9E243   |
	LDA.w kong_control.input_held,y		;$B9E246   |
	AND #$0080				;$B9E249   |
	BEQ CODE_B9E24F				;$B9E24C   |
	RTS					;$B9E24E  /

CODE_B9E24F:
	JMP CODE_B9D705				;$B9E24F  /

CODE_B9E252:
	LDA sprite.max_x_speed,x		;$B9E252  \
	BNE CODE_B9E262				;$B9E254   |
	LDY active_kong_control_variables	;$B9E256   |
	LDA.w kong_control.input_held,y		;$B9E259   |
	AND #$0080				;$B9E25C   |
	BEQ CODE_B9E26A				;$B9E25F   |
	RTS					;$B9E261  /

CODE_B9E262:
	LDA #$005E				;$B9E262  \
	JSL set_anim_handle_animal_and_dixie	;$B9E265   |
	RTS					;$B9E269  /

CODE_B9E26A:
	LDA animal_type				;$B9E26A  \
	CMP #!sprite_enguarde			;$B9E26C   |
	BEQ CODE_B9E28F				;$B9E26F   |
	LDA #$0700				;$B9E271   |
	BIT sprite.oam_property,x		;$B9E274   |
	BVC CODE_B9E27C				;$B9E276   |
	EOR #$FFFF				;$B9E278   |
	INC A					;$B9E27B   |
CODE_B9E27C:					;	   |
	STA sprite.general_purpose_42,x		;$B9E27C   |
	STA sprite.x_speed,x			;$B9E27E   |
	STA sprite.max_x_speed,x		;$B9E280   |
	LDA #$004F				;$B9E282   |
	STA sprite.state,x			;$B9E285   |
	LDA #$0068				;$B9E287   |
	JSL set_anim_handle_animal_and_dixie	;$B9E28A   |
	RTS					;$B9E28E  /

CODE_B9E28F:
	LDA #$0700				;$B9E28F  \
	BIT sprite.oam_property,x		;$B9E292   |
	BVC CODE_B9E29A				;$B9E294   |
	EOR #$FFFF				;$B9E296   |
	INC A					;$B9E299   |
CODE_B9E29A:					;	   |
	STA sprite.general_purpose_42,x		;$B9E29A   |
	STA sprite.x_speed,x			;$B9E29C   |
	STA sprite.max_x_speed,x		;$B9E29E   |
	LDA #$0052				;$B9E2A0   |
	STA sprite.state,x			;$B9E2A3   |
	LDA #$0068				;$B9E2A5   |
	JSL set_anim_handle_animal_and_dixie	;$B9E2A8   |
	LDX current_sprite			;$B9E2AC   |
	LDA #$A000				;$B9E2AE   |
	ORA sprite.animation_flags,x		;$B9E2B1   |
	STA sprite.animation_flags,x		;$B9E2B3   |
	RTS					;$B9E2B5  /

CODE_B9E2B6:
	JSR CODE_B9E2D0				;$B9E2B6  \
	LDX current_sprite			;$B9E2B9   |
	LDA sprite.max_x_speed,x		;$B9E2BB   |
	BEQ CODE_B9E2CF				;$B9E2BD   |
	LSR A					;$B9E2BF   |
	EOR sprite.oam_property,x		;$B9E2C0   |
	AND #$4000				;$B9E2C2   |
	BNE CODE_B9E2C8				;$B9E2C5   |
	RTS					;$B9E2C7  /

CODE_B9E2C8:
	LDA #$0063				;$B9E2C8  \
	JSL set_anim_handle_animal_and_dixie	;$B9E2CB   |
CODE_B9E2CF:					;	   |
	RTS					;$B9E2CF  /

CODE_B9E2D0:
	LDX current_sprite			;$B9E2D0  \
	LDA sprite.y_speed,x			;$B9E2D2   |
	BPL CODE_B9E2DA				;$B9E2D4   |
	EOR #$FFFF				;$B9E2D6   |
	INC A					;$B9E2D9   |
CODE_B9E2DA:					;	   |
	STA $34					;$B9E2DA   |
	LDA sprite.x_speed,x			;$B9E2DC   |
	BPL CODE_B9E2E4				;$B9E2DE   |
	EOR #$FFFF				;$B9E2E0   |
	INC A					;$B9E2E3   |
CODE_B9E2E4:					;	   |
	CMP $34					;$B9E2E4   |
	BPL CODE_B9E2EA				;$B9E2E6   |
	LDA $34					;$B9E2E8   |
CODE_B9E2EA:					;	   |
	CMP #$00C0				;$B9E2EA   |
	BPL CODE_B9E2F2				;$B9E2ED   |
	LDA #$00C0				;$B9E2EF   |
CODE_B9E2F2:					;	   |
	STA sprite.animation_speed,x		;$B9E2F2   |
	RTS					;$B9E2F4  /

CODE_B9E2F5:
	LDA #$005E				;$B9E2F5  \
	JSL set_anim_handle_animal_and_dixie	;$B9E2F8   |
	RTS					;$B9E2FC  /

CODE_B9E2FD:
	LDA #$0051				;$B9E2FD  \
	STA sprite.state,x			;$B9E300   |
	LDA #$A000				;$B9E302   |
	ORA sprite.animation_flags,x		;$B9E305   |
	STA sprite.animation_flags,x		;$B9E307   |
	RTS					;$B9E309  /

CODE_B9E30A:
	LDA RAM_0B02				;$B9E30A  \
	AND #$0001				;$B9E30D   |
	BNE CODE_B9E313				;$B9E310   |
	RTS					;$B9E312  /

CODE_B9E313:
	EOR RAM_0B02				;$B9E313  \
	STA RAM_0B02				;$B9E316   |
	LDX current_sprite			;$B9E319   |
	LDA sprite.max_x_speed,x		;$B9E31B   |
	BEQ CODE_B9E329				;$B9E31D   |
	LSR A					;$B9E31F   |
	EOR sprite.oam_property,x		;$B9E320   |
	AND #$4000				;$B9E322   |
	EOR sprite.oam_property,x		;$B9E325   |
	STA sprite.oam_property,x		;$B9E327   |
CODE_B9E329:					;	   |
	LDA #$008B				;$B9E329   |
	JSL set_anim_handle_animal_and_dixie	;$B9E32C   |
	RTS					;$B9E330  /

CODE_B9E331:
	LDA active_kong_number			;$B9E331  \
	BNE CODE_B9E33D				;$B9E334   |
	LDA #$0059				;$B9E336   |
	JSL set_riding_kong_anim_handle_dixie	;$B9E339   |
CODE_B9E33D:					;	   |
	RTS					;$B9E33D  /

CODE_B9E33E:
	LDY current_kong_control_variables	;$B9E33E  \
	LDA.w kong_control.input_held,y		;$B9E340   |
	AND #$4000				;$B9E343   |
	BEQ CODE_B9E354				;$B9E346   |
	LDA active_frame_counter		;$B9E348   |
	SEC					;$B9E34A   |
	SBC.w kong_control.y_press_time,y	;$B9E34B   |
	CMP #$0014				;$B9E34E   |
	BMI CODE_B9E354				;$B9E351   |
	RTS					;$B9E353  /

CODE_B9E354:
	JMP CODE_B9D705				;$B9E354  /

CODE_B9E357:
	LDA #$0011				;$B9E357  \
	JSL disable_enemy_damage_global		;$B9E35A   |
	RTS					;$B9E35E  /

CODE_B9E35F:
	%lda_sound(5, enguarde_charge)		;$B9E35F  \
	JSL queue_sound_effect			;$B9E362   |
	LDX current_sprite			;$B9E366   |
	LDY #$0400				;$B9E368   |
	LDA sprite.state,x			;$B9E36B   |
	CMP #!kong_state_6C			;$B9E36D   |
	BNE CODE_B9E375				;$B9E370   |
	LDY #$0180				;$B9E372   |
CODE_B9E375:					;	   |
	BIT sprite.oam_property,x		;$B9E375   |
	BVC CODE_B9E37F				;$B9E377   |
	TYA					;$B9E379   |
	EOR #$FFFF				;$B9E37A   |
	INC A					;$B9E37D   |
	TAY					;$B9E37E   |
CODE_B9E37F:					;	   |
	TYA					;$B9E37F   |
	STA sprite.x_speed,x			;$B9E380   |
	STA sprite.max_x_speed,x		;$B9E382   |
	RTS					;$B9E384  /

CODE_B9E385:
	LDY #$0000				;$B9E385  \
	LDA sprite.animation_speed,x		;$B9E388   |
	CLC					;$B9E38A   |
	ADC #$000C				;$B9E38B   |
	CMP #$0480				;$B9E38E   |
	BMI CODE_B9E397				;$B9E391   |
	INY					;$B9E393   |
	LDA #$0480				;$B9E394   |
CODE_B9E397:					;	   |
	STA sprite.animation_speed,x		;$B9E397   |
	STY $32					;$B9E399   |
	LDY active_kong_control_variables	;$B9E39B   |
	LDA.w kong_control.input_held,y		;$B9E39E   |
	AND #$0080				;$B9E3A1   |
	BEQ CODE_B9E3A7				;$B9E3A4   |
	RTS					;$B9E3A6  /

CODE_B9E3A7:
	LDA $32					;$B9E3A7  \
	BNE CODE_B9E3AF				;$B9E3A9   |
	JMP CODE_B9D705				;$B9E3AB  /

CODE_B9E3AE:
	RTS					;$B9E3AE  \

CODE_B9E3AF:
	LDX current_sprite			;$B9E3AF  \
	LDY #$0098				;$B9E3B1   |
	LDA [current_sprite_constants],y	;$B9E3B4   |
	STA sprite.y_speed,x			;$B9E3B6   |
	LDA #$0062				;$B9E3B8   |
	STA sprite.state,x			;$B9E3BB   |
	LDY current_kong_control_variables	;$B9E3BD   |
	LDA active_frame_counter		;$B9E3BF   |
	STA.w kong_control.jump_event_time,y	;$B9E3C1   |
	LDA #$0072				;$B9E3C4   |
	JSL set_anim_handle_animal_and_dixie	;$B9E3C7   |
	%lda_sound(5, rattly_leap)		;$B9E3CB   |
	JSL queue_sound_effect			;$B9E3CE   |
	RTS					;$B9E3D2  /

CODE_B9E3D3:
	LDA #$14EC				;$B9E3D3  \
	BRL CODE_B9E486				;$B9E3D6  /

CODE_B9E3D9:
	LDA #$01C0				;$B9E3D9  \
	BRA CODE_B9E3E1				;$B9E3DC  /

CODE_B9E3DE:
	LDA #$02C0				;$B9E3DE  \
CODE_B9E3E1:					;	   |
	BIT sprite.general_purpose_4A,x		;$B9E3E1   |
	BNE CODE_B9E3EC				;$B9E3E3   |
CODE_B9E3E5:					;	   |
	LDA sprite.general_purpose_48,x		;$B9E3E5   |
	CMP #$0009				;$B9E3E7   |
	BEQ CODE_B9E401				;$B9E3EA   |
CODE_B9E3EC:					;	   |
namespace animation
	LDA #dixie_skull_cart			;$B9E3EC   |
namespace off
	STA sprite.animation_address,x		;$B9E3EF   |
	STZ sprite.animation_routine,x		;$B9E3F1   |
	STZ sprite.animation_timer,x		;$B9E3F3   |
	LDA sprite.general_purpose_48,x		;$B9E3F5   |
	AND #$00FF				;$B9E3F7   |
	ASL A					;$B9E3FA   |
	ASL A					;$B9E3FB   |
	ADC #$14EC				;$B9E3FC   |
	STA sprite.current_graphic,x		;$B9E3FF   |
CODE_B9E401:					;	   |
	RTS					;$B9E401  /

CODE_B9E402:
	LDA sprite.general_purpose_46,x		;$B9E402  \
	CMP #$8005				;$B9E404   |
	BNE CODE_B9E3EC				;$B9E407   |
	RTS					;$B9E409  /

CODE_B9E40A:
	LDA #$1454				;$B9E40A  \
	BRL CODE_B9E486				;$B9E40D  /

CODE_B9E410:
	LDA #$01C0				;$B9E410  \
	BRA CODE_B9E418				;$B9E413  /

CODE_B9E415:
	LDA #$02C0				;$B9E415  \
CODE_B9E418:					;	   |
	BIT sprite.general_purpose_4A,x		;$B9E418   |
	BNE CODE_B9E423				;$B9E41A   |
CODE_B9E41C:					;	   |
	LDA sprite.general_purpose_48,x		;$B9E41C   |
	CMP #$0009				;$B9E41E   |
	BEQ CODE_B9E438				;$B9E421   |
CODE_B9E423:					;	   |
namespace animation
	LDA #diddy_skull_cart			;$B9E423   |
namespace off
	STA sprite.animation_address,x		;$B9E426   |
	STZ sprite.animation_routine,x		;$B9E428   |
	STZ sprite.animation_timer,x		;$B9E42A   |
	LDA sprite.general_purpose_48,x		;$B9E42C   |
	AND #$00FF				;$B9E42E   |
	ASL A					;$B9E431   |
	ASL A					;$B9E432   |
	ADC #$1454				;$B9E433   |
	STA sprite.current_graphic,x		;$B9E436   |
CODE_B9E438:					;	   |
	RTS					;$B9E438  /

CODE_B9E439:
	LDA sprite.ground_distance,x		;$B9E439  \
	BNE CODE_B9E423				;$B9E43B   |
	RTS					;$B9E43D  /

CODE_B9E43E:
	LDA sprite.general_purpose_46,x		;$B9E43E  \
	CMP #$8005				;$B9E440   |
	BNE CODE_B9E480				;$B9E443   |
	LDA #$0040				;$B9E445   |
	BIT sprite.general_purpose_4A,x		;$B9E448   |
	BNE CODE_B9E483				;$B9E44A   |
	BRL process_anim_script			;$B9E44C  /

CODE_B9E44F:
	LDA sprite.general_purpose_4A,x		;$B9E44F  \
	BIT #$0008				;$B9E451   |
	BNE CODE_B9E45D				;$B9E454   |
	AND #$0200				;$B9E456   |
	BEQ CODE_B9E45D				;$B9E459   |
	CLC					;$B9E45B   |
	RTS					;$B9E45C  /

CODE_B9E45D:
	STZ sprite.animation_routine,x		;$B9E45D  \
	STZ sprite.animation_timer,x		;$B9E45F   |
	SEC					;$B9E461   |
	RTS					;$B9E462  /

CODE_B9E463:
	CLC					;$B9E463  \
	LDA #$0080				;$B9E464   |
	BRA CODE_B9E46D				;$B9E467  /

CODE_B9E469:
	SEC					;$B9E469  \
	LDA #$0100				;$B9E46A   |
CODE_B9E46D:					;	   |
	BIT sprite.general_purpose_4A,x		;$B9E46D   |
	BNE CODE_B9E483				;$B9E46F   |
	LDA #$0008				;$B9E471   |
	BIT sprite.general_purpose_4A,x		;$B9E474   |
	BEQ CODE_B9E480				;$B9E476   |
	ROL A					;$B9E478   |
	EOR $000D58				;$B9E479   |
	LSR A					;$B9E47D   |
	BCC CODE_B9E483				;$B9E47E   |
CODE_B9E480:					;	   |
	BRL process_anim_script			;$B9E480  /

CODE_B9E483:
	BRL CODE_B9D12B				;$B9E483  /

CODE_B9E486:
	STZ sprite.animation_routine,x		;$B9E486  \
	STA $32					;$B9E488   |
	LDA sprite.general_purpose_4A,x		;$B9E48A   |
	BIT #$0040				;$B9E48C   |
	BNE CODE_B9E4D0				;$B9E48F   |
	BIT #$0008				;$B9E491   |
	BNE CODE_B9E508				;$B9E494   |
	BIT #$0100				;$B9E496   |
	BNE CODE_B9E4F5				;$B9E499   |
	BIT #$0080				;$B9E49B   |
	BNE CODE_B9E4EC				;$B9E49E   |
	BIT #$0200				;$B9E4A0   |
	BNE CODE_B9E4FE				;$B9E4A3   |
CODE_B9E4A5:					;	   |
	LDA sprite.general_purpose_4A,x		;$B9E4A5   |
	BIT #$0010				;$B9E4A7   |
	BNE CODE_B9E4BA				;$B9E4AA   |
CODE_B9E4AC:					;	   |
	LDA sprite.general_purpose_48,x		;$B9E4AC   |
	AND #$00FF				;$B9E4AE   |
	ASL A					;$B9E4B1   |
	ASL A					;$B9E4B2   |
	ADC $32					;$B9E4B3   |
	STA sprite.current_graphic,x		;$B9E4B5   |
	BRL CODE_B9D12B				;$B9E4B7  /

CODE_B9E4BA:
	LDA active_frame_counter		;$B9E4BA  \
	AND #$000F				;$B9E4BC   |
	BNE CODE_B9E4AC				;$B9E4BF   |
	JSL CODE_B4C175				;$B9E4C1   |
	AND #$000F				;$B9E4C5   |
	CMP #$0005				;$B9E4C8   |
	BCS CODE_B9E4AC				;$B9E4CB   |
	BRL process_anim_script			;$B9E4CD  /

CODE_B9E4D0:
	LDA #$8005				;$B9E4D0  \
	CMP sprite.general_purpose_44,x		;$B9E4D3   |
	BNE CODE_B9E4AC				;$B9E4D5   |
	CMP sprite.general_purpose_46,x		;$B9E4D7   |
	BNE CODE_B9E4AC				;$B9E4D9   |
	LDA sprite.ground_distance,x		;$B9E4DB   |
	BNE CODE_B9E4AC				;$B9E4DD   |
	LDA sprite.y_speed,x			;$B9E4DF   |
	BMI CODE_B9E4AC				;$B9E4E1   |
	TYA					;$B9E4E3   |
	CLC					;$B9E4E4   |
	ADC #$0009				;$B9E4E5   |
	TAY					;$B9E4E8   |
	BRL process_anim_script			;$B9E4E9  /

CODE_B9E4EC:
	TYA					;$B9E4EC  \
	CLC					;$B9E4ED   |
	ADC #$0006				;$B9E4EE   |
	TAY					;$B9E4F1   |
	BRL process_anim_script			;$B9E4F2  /

CODE_B9E4F5:
	TYA					;$B9E4F5  \
	CLC					;$B9E4F6   |
	ADC #$0003				;$B9E4F7   |
	TAY					;$B9E4FA   |
	BRL process_anim_script			;$B9E4FB  /

CODE_B9E4FE:
	TXA					;$B9E4FE  \
	CMP.l active_kong_sprite		;$B9E4FF   |
	BNE CODE_B9E4A5				;$B9E503   |
	BRL process_anim_script			;$B9E505  /

CODE_B9E508:
	LDA.l $000D58				;$B9E508  \
	LSR A					;$B9E50C   |
	BCS CODE_B9E4F5				;$B9E50D   |
	BRA CODE_B9E4EC				;$B9E50F  /

anim_delete_sprite:
	JSL delete_sprite_handle_deallocation	;$B9E511  \
	RTS					;$B9E515  /

CODE_B9E516:
	SEP #$20				;$B9E516  \
	INC $19AF				;$B9E518   |
	REP #$20				;$B9E51B   |
	RTS					;$B9E51D  /

CODE_B9E51E:
	LDA time_stop_flags			;$B9E51E  \
	BIT #$0004				;$B9E521   |
	BNE CODE_B9E534				;$B9E524   |
	JSL CODE_BCFB58				;$B9E526   |
	LDA #$0100				;$B9E52A   |
	LDY #$0100				;$B9E52D   |
	JSL CODE_BEBD8E				;$B9E530   |
CODE_B9E534:					;	   |
	RTS					;$B9E534  /

CODE_B9E535:
	LDA sprite.state,x			;$B9E535  \
	AND #$00FF				;$B9E537   |
	INC A					;$B9E53A   |
	STA sprite.state,x			;$B9E53B   |
	RTS					;$B9E53D  /

CODE_B9E53E:
	INC sprite.sub_state,x			;$B9E53E  \
	RTS					;$B9E540  /

CODE_B9E541:
	DEC sprite.sub_state,x			;$B9E541  \
	RTS					;$B9E543  /

CODE_B9E544:
	INC sprite.state,x			;$B9E544  \
	RTS					;$B9E546  /

CODE_B9E547:
	LDA sprite.y_speed,x			;$B9E547  \
	BMI CODE_B9E54E				;$B9E549   |
CODE_B9E54B:					;	   |
	JMP process_anim_script			;$B9E54B  /

CODE_B9E54E:
	JMP CODE_B9D12B				;$B9E54E  /

CODE_B9E551:
	LDA sprite.y_speed,x			;$B9E551  \
	BPL CODE_B9E54B				;$B9E553   |
	CMP #$FE00				;$B9E555   |
	BCC CODE_B9E54E				;$B9E558   |
	JMP process_anim_script			;$B9E55A  /

CODE_B9E55D:
	LDA sprite.ground_distance,x		;$B9E55D  \
	BNE CODE_B9E54E				;$B9E55F   |
	JMP process_anim_script			;$B9E561  /

turn_sprite_if_needed:
	LDA sprite.max_x_speed,x		;$B9E564  \
	BEQ CODE_B9E575				;$B9E566   |
	LSR A					;$B9E568   |
	EOR sprite.oam_property,x		;$B9E569   |
	AND #$4000				;$B9E56B   |
	BEQ CODE_B9E575				;$B9E56E   |
	TYA					;$B9E570   |
	JSL set_sprite_animation		;$B9E571   |
CODE_B9E575:					;	   |
	RTS					;$B9E575  /

CODE_B9E576:
	LDA game_state_flags			;$B9E576  \
	AND #$4000				;$B9E579   |
	BNE CODE_B9E592				;$B9E57C   |
	LDY active_kong_sprite			;$B9E57E   |
	LDA.w sprite.state,y			;$B9E581   |
	ASL A					;$B9E584   |
	ASL A					;$B9E585   |
	TAX					;$B9E586   |
	LDA.l kong_state_flags_table,x		;$B9E587   |
	AND #$0001				;$B9E58B   |
	BEQ CODE_B9E592				;$B9E58E   |
	CLC					;$B9E590   |
	RTS					;$B9E591  /

CODE_B9E592:
	SEC					;$B9E592  \
	RTS					;$B9E593  /

;Ship water splash related
CODE_B9E594:
	LDA sprite.general_purpose_46,x		;$B9E594  \
	LSR A					;$B9E596   |
	%lda_sound(5, splash_1)			;$B9E597   |
	BCC CODE_B9E59F				;$B9E59A   |
	%lda_sound(5, splash_2)			;$B9E59C   |
CODE_B9E59F:					;	   |
	JSL queue_sound_effect			;$B9E59F   |
	LDX current_sprite			;$B9E5A3   |
	LDA #$FFFF				;$B9E5A5   |
	STA sprite.general_purpose_46,x		;$B9E5A8   |
	RTS					;$B9E5AA  /

CODE_B9E5AB:
	LDA active_frame_counter		;$B9E5AB  \
	AND #$003F				;$B9E5AD   |
	BNE turn_neek_if_needed			;$B9E5B0   |
	%lda_sound(5, neek_squeak)		;$B9E5B2   |
	JSL queue_sfx_if_on_screen_global	;$B9E5B5   |
turn_neek_if_needed:				;	   |
	LDY #$0170				;$B9E5B9   |
	JMP turn_sprite_if_needed		;$B9E5BC  /

turn_click_clack_if_needed:
	LDY #$0165				;$B9E5BF  \
	JMP turn_sprite_if_needed		;$B9E5C2  /

CODE_B9E5C5:
	LDA #$FB00				;$B9E5C5  \
	STA sprite.y_speed,x			;$B9E5C8   |
	LDA sprite.x_position,x			;$B9E5CA   |
	STA contact_flash_x_position		;$B9E5CC   |
	LDA sprite.y_position,x			;$B9E5CF   |
	SBC #$0008				;$B9E5D1   |
	STA contact_flash_y_position		;$B9E5D4   |
	LDY #!special_sprite_spawn_id_0034	;$B9E5D7   |
	JSL spawn_special_sprite_index		;$B9E5DA   |
	RTS					;$B9E5DE  /

CODE_B9E5DF:
	LDA sprite.terrain_interaction,x	;$B9E5DF  \
	AND #$0001				;$B9E5E1   |
	BEQ CODE_B9E5ED				;$B9E5E4   |
	%lda_sound(6, click_clack_walk)		;$B9E5E6   |
	JSL queue_sfx_if_on_screen_global	;$B9E5E9   |
CODE_B9E5ED:					;	   |
	RTS					;$B9E5ED  /

CODE_B9E5EE:
	LDA #$0000				;$B9E5EE  \
	CMP sprite.general_purpose_4E,x		;$B9E5F1   |
	RTS					;$B9E5F3  /

CODE_B9E5F4:
	JSR CODE_B9E601				;$B9E5F4  \
	BCS CODE_B9E613				;$B9E5F7   |
	LDX current_sprite			;$B9E5F9   |
	LDY #$01F6				;$B9E5FB   |
	JMP turn_sprite_if_needed		;$B9E5FE  /

CODE_B9E601:
	LDA sprite.state,x			;$B9E601  \
	AND #$00FF				;$B9E603   |
	CMP #$0003				;$B9E606   |
	BCC CODE_B9E613				;$B9E609   |
	LDA #$01F7				;$B9E60B   |
	JSL set_sprite_animation		;$B9E60E   |
	SEC					;$B9E612   |
CODE_B9E613:					;	   |
	RTS					;$B9E613  /

CODE_B9E614:
	LDY #$0006				;$B9E614  \
	LDA [current_sprite_constants],y	;$B9E617   |
	BIT sprite.oam_property,x		;$B9E619   |
	BVC CODE_B9E621				;$B9E61B   |
	EOR #$FFFF				;$B9E61D   |
	INC A					;$B9E620   |
CODE_B9E621:					;	   |
	STA sprite.max_x_speed,x		;$B9E621   |
	LDA #$0200				;$B9E623   |
	STA sprite.general_purpose_4A,x		;$B9E626   |
	LDA #$0001				;$B9E628   |
	STA sprite.general_purpose_50,x		;$B9E62B   |
	RTS					;$B9E62D  /

CODE_B9E62E:
	LDA #$FC80				;$B9E62E  \
	STA sprite.y_speed,x			;$B9E631   |
	RTS					;$B9E633  /

CODE_B9E634:
	LDA sprite.animation_speed,x		;$B9E634  \
	CMP #!default_animation_speed+1		;$B9E636   |
	BCC CODE_B9E613				;$B9E639   |
	LDA sprite.terrain_interaction,x	;$B9E63B   |
	AND #$0101				;$B9E63D   |
	BEQ CODE_B9E613				;$B9E640   |
	LDA sprite.max_x_speed,x		;$B9E642   |
	EOR sprite.x_speed,x			;$B9E644   |
	BMI CODE_B9E655				;$B9E646   |
	LDA sprite.x_speed,x			;$B9E648   |
	BPL CODE_B9E650				;$B9E64A   |
	EOR #$FFFF				;$B9E64C   |
	INC A					;$B9E64F   |
CODE_B9E650:					;	   |
	CMP #$0200				;$B9E650   |
	BCS CODE_B9E613				;$B9E653   |
CODE_B9E655:					;	   |
	JSL check_if_sprite_offscreen_global	;$B9E655   |
	BCS CODE_B9E613				;$B9E659   |
	LDX current_sprite			;$B9E65B   |
	LDY #$004E				;$B9E65D   |
	BIT sprite.oam_property,x		;$B9E660   |
	BVC CODE_B9E667				;$B9E662   |
	LDY #!special_sprite_spawn_id_0050	;$B9E664   |
CODE_B9E667:					;	   |
	JSL spawn_no_gfx_special_sprite_index	;$B9E667   |
	LDX current_sprite			;$B9E66B   |
	DEC sprite.general_purpose_50,x		;$B9E66D   |
	BNE CODE_B9E613				;$B9E66F   |
	%lda_sound(7, klobber_skid)		;$B9E671   |
	JSL queue_sound_effect			;$B9E674   |
	RTS					;$B9E678  /

;This will effectively ASL/LSR his current X velocity twice, depending if moving left/right respectively.
delay_klomp_walk:
	LDA sprite.x_speed,x			;$B9E679  \
	CMP #$8000				;$B9E67B   |
	ROR sprite.x_speed,x			;$B9E67E   |
	CMP #$8000				;$B9E680   |
	ROR sprite.x_speed,x			;$B9E683   |
	RTS					;$B9E685  /

;Similar to above.
CODE_B9E686:
	LDA sprite.x_speed,x			;$B9E686  \
	CMP #$8000				;$B9E688   |
	ROR A					;$B9E68B   |
	CLC					;$B9E68C   |
	ADC sprite.x_speed,x			;$B9E68D   |
	STA sprite.x_speed,x			;$B9E68F   |
	RTS					;$B9E691  /

turn_klomp_if_needed:
	LDY #$0161				;$B9E692  \
	JMP turn_sprite_if_needed		;$B9E695  /

turn_kruncha_if_needed:
	LDY #$0173				;$B9E698  \
	JMP turn_sprite_if_needed		;$B9E69B  /

CODE_B9E69E:
	LDA sprite.y_speed,x			;$B9E69E  \
	BMI CODE_B9E6AE				;$B9E6A0   |
	CMP #$0040				;$B9E6A2   |
	BCC CODE_B9E6AE				;$B9E6A5   |
CODE_B9E6A7:					;	   |
	BRL CODE_B9D12B				;$B9E6A7  /

CODE_B9E6AA:
	BIT sprite.max_y_speed,x		;$B9E6AA  \
	BPL CODE_B9E6A7				;$B9E6AC   |
CODE_B9E6AE:					;	   |
	BRL process_anim_script			;$B9E6AE  /

CODE_B9E6B1:
	LDY active_kong_sprite			;$B9E6B1  \
	LDA.w sprite.x_position,y		;$B9E6B4   |
	CMP sprite.x_position,x			;$B9E6B7   |
	ROR A					;$B9E6B9   |
	ROR A					;$B9E6BA   |
	EOR sprite.oam_property,x		;$B9E6BB   |
	ASL A					;$B9E6BD   |
	ASL A					;$B9E6BE   |
	RTS					;$B9E6BF  /

turn_klampon_if_needed:
	LDY #$01A0				;$B9E6C0  \
	JMP turn_sprite_if_needed		;$B9E6C3  /

CODE_B9E6C6:
	LDA.l $000A36				;$B9E6C6  \
	AND #$0004				;$B9E6CA   |
	SEC					;$B9E6CD   |
	SBC #$0004				;$B9E6CE   |
	RTS					;$B9E6D1  /

turn_spiny_if_needed:
	LDY #$01A4				;$B9E6D2  \
	JMP turn_sprite_if_needed		;$B9E6D5  /

set_credits_snapjaw_animation:
	LDA sprite.x_position,x			;$B9E6D8  \
	CMP #$0160				;$B9E6DA   | Check if snapjaw has reached target X position
	BCC .return				;$B9E6DD   | If not, return
	STZ sprite.animation_routine,x		;$B9E6DF   | Else clear anim subroutine pointer
namespace animation
	LDA #DATA_F956E6			;$B9E6E1   |
namespace off
	STA sprite.animation_address,x		;$B9E6E4   | Set new animation script pointer
.return:					;	   |
	RTS					;$B9E6E6  / Return

turn_lockjaw_if_needed:
	LDY #$0178				;$B9E6E7  \
	JMP turn_sprite_if_needed		;$B9E6EA  /

CODE_B9E6ED:
	LDA sprite.oam_property,x		;$B9E6ED  \
	ASL A					;$B9E6EF   |
	EOR sprite.max_x_speed,x		;$B9E6F0   |
	ASL A					;$B9E6F2   |
	RTS					;$B9E6F3  /

CODE_B9E6F4:
	LDA sprite.max_x_speed,x		;$B9E6F4  \
	BNE CODE_B9E704				;$B9E6F6   |
namespace animation
	LDA #DATA_F95798			;$B9E6F8   |
namespace off
	STA sprite.animation_address,x		;$B9E6FB   |
	LDA #CODE_B9E705			;$B9E6FD   |
	STA sprite.animation_routine,x		;$B9E700   |
	STZ sprite.animation_timer,x		;$B9E702   |
CODE_B9E704:					;	   |
	RTS					;$B9E704  /

CODE_B9E705:
	LDA sprite.max_x_speed,x		;$B9E705  \
	BEQ CODE_B9E704				;$B9E707   |
	STZ sprite.animation_routine,x		;$B9E709   |
	STZ sprite.animation_timer,x		;$B9E70B   |
namespace animation
	LDA #DATA_F957AA			;$B9E70D   |
namespace off
	STA sprite.animation_address,x		;$B9E710   |
	RTS					;$B9E712  /

CODE_B9E713:
	LDA sprite.state,x			;$B9E713  \
	CMP #$0402				;$B9E715   |
	BNE CODE_B9E71D				;$B9E718   |
	BRL process_anim_script			;$B9E71A  /

CODE_B9E71D:
	BRL CODE_B9D12B				;$B9E71D  /

CODE_B9E720:
	LDA #$0080				;$B9E720  \
	BIT sprite.oam_property,x		;$B9E723   |
	BVS CODE_B9E72A				;$B9E725   |
	LDA #$FF80				;$B9E727   |
CODE_B9E72A:					;	   |
	STA sprite.max_x_speed,x		;$B9E72A   |
	STZ sprite.max_y_speed,x		;$B9E72C   |
	RTS					;$B9E72E  /

CODE_B9E72F:
	LDA sprite.oam_property,x		;$B9E72F  \
	ASL A					;$B9E731   |
	EOR sprite.max_x_speed,x		;$B9E732   |
	EOR #$8000				;$B9E734   |
	ASL A					;$B9E737   |
	STZ sprite.max_x_speed,x		;$B9E738   |
	RTS					;$B9E73A  /

CODE_B9E73B:
	LDA sprite.display_mode,x		;$B9E73B  \
	LSR A					;$B9E73D   |
	RTS					;$B9E73E  /

CODE_B9E73F:
	LDA sprite.display_mode,x		;$B9E73F  \
	ORA #$0002				;$B9E741   |
	STA sprite.display_mode,x		;$B9E744   |
	LSR A					;$B9E746   |
	BNE CODE_B9E75F				;$B9E747   |
	LDA.l $000915				;$B9E749   |
	BNE CODE_B9E75F				;$B9E74D   |
	JSR CODE_B9E576				;$B9E74F   |
	BCC CODE_B9E75F				;$B9E752   |
	LDX active_kong_sprite			;$B9E754   |
	LDA sprite.y_position,x			;$B9E757   |
	CMP $0D4E				;$B9E759   |
	BCC CODE_B9E75F				;$B9E75C   |
	RTS					;$B9E75E  /

CODE_B9E75F:
	TYX					;$B9E75F  \
	LDY #$0010				;$B9E760   |
	LDA [current_sprite_constants],y	;$B9E763   |
	TXY					;$B9E765   |
	LDX current_sprite			;$B9E766   |
	STA sprite.general_purpose_4E,x		;$B9E768   |
	LDA #$FD00				;$B9E76A   |
	CMP sprite.max_x_speed,x		;$B9E76D   |
	BCC CODE_B9E77F				;$B9E76F   |
	LDA sprite.max_x_speed,x		;$B9E771   |
	BMI CODE_B9E77D				;$B9E773   |
	CMP #$0300				;$B9E775   |
	BCC CODE_B9E77F				;$B9E778   |
	LDA #$0300				;$B9E77A   |
CODE_B9E77D:					;	   |
	STA sprite.max_x_speed,x		;$B9E77D   |
CODE_B9E77F:					;	   |
	CLC					;$B9E77F   |
	RTS					;$B9E780  /

CODE_B9E781:
	LDX active_kong_sprite			;$B9E781  \
	LDA sprite.x_position,x			;$B9E784   |
	SEC					;$B9E786   |
	LDX current_sprite			;$B9E787   |
	SBC sprite.x_position,x			;$B9E789   |
	LSR A					;$B9E78B   |
	EOR sprite.oam_property,x		;$B9E78C   |
	EOR #$4000				;$B9E78E   |
	ASL A					;$B9E791   |
	ASL A					;$B9E792   |
	RTS					;$B9E793  /

CODE_B9E794:
	LDA.l $000915				;$B9E794  \
	BNE CODE_B9E7B0				;$B9E798   |
	LDA.l active_kong_sprite		;$B9E79A   |
	TAX					;$B9E79E   |
	LDA sprite.x_position,x			;$B9E79F   |
	LDX current_sprite			;$B9E7A1   |
	SBC sprite.x_position,x			;$B9E7A3   |
	BPL CODE_B9E7B3				;$B9E7A5   |
	BIT sprite.oam_property,x		;$B9E7A7   |
	BVC CODE_B9E7D1				;$B9E7A9   |
	CMP #$FFF0				;$B9E7AB   |
	BCC CODE_B9E7BC				;$B9E7AE   |
CODE_B9E7B0:					;	   |
	JMP process_anim_script			;$B9E7B0  /

CODE_B9E7B3:
	BIT sprite.oam_property,x		;$B9E7B3  \
	BVS CODE_B9E7D1				;$B9E7B5   |
	CMP #$0010				;$B9E7B7   |
	BCC CODE_B9E7B0				;$B9E7BA   |
CODE_B9E7BC:					;	   |
	LDA sprite.display_mode,x		;$B9E7BC   |
	LSR A					;$B9E7BE   |
	BCS CODE_B9E7B0				;$B9E7BF   |
	LDA.l active_kong_sprite		;$B9E7C1   |
	TAX					;$B9E7C5   |
	LDA sprite.y_position,x			;$B9E7C6   |
	CMP $000D4E				;$B9E7C8   |
	BCC CODE_B9E7B0				;$B9E7CC   |
	JMP CODE_B9D12B				;$B9E7CE  /

CODE_B9E7D1:
	LDA #CODE_B9E7D9			;$B9E7D1  \
	STA sprite.animation_routine,x		;$B9E7D4   |
	JMP process_anim_script			;$B9E7D6  /

CODE_B9E7D9:
	STZ sprite.max_x_speed,x		;$B9E7D9  \
	STZ sprite.max_y_speed,x		;$B9E7DB   |
	RTS					;$B9E7DD  /

turn_flotsam_if_needed:
	LDY #$01AA				;$B9E7DE  \
	JMP turn_sprite_if_needed		;$B9E7E1  /

CODE_B9E7E4:
	STZ sprite.animation_routine,x		;$B9E7E4  \
	LDA sprite.max_x_speed,x		;$B9E7E6   |
	CMP #$8000				;$B9E7E8   |
	ROR A					;$B9E7EB   |
	CLC					;$B9E7EC   |
	ADC sprite.max_x_speed,x		;$B9E7ED   |
	STA sprite.x_speed,x			;$B9E7EF   |
	RTS					;$B9E7F1  /

CODE_B9E7F2:
	LDA sprite.oam_property,x		;$B9E7F2  \
	EOR #$4000				;$B9E7F4   |
	STA sprite.oam_property,x		;$B9E7F7   |
	AND #$4000				;$B9E7F9   |
	CMP #$4000				;$B9E7FC   |
	LDA #$0008				;$B9E7FF   |
	BCC CODE_B9E807				;$B9E802   |
	LDA #$FFF9				;$B9E804   |
CODE_B9E807:					;	   |
	ADC sprite.x_position,x			;$B9E807   |
	STA sprite.x_position,x			;$B9E809   |
	RTS					;$B9E80B  /

CODE_B9E80C:
	LDA sprite.animation_speed,x		;$B9E80C  \
	CMP #!default_animation_speed		;$B9E80E   |
	BEQ CODE_B9E815				;$B9E811   |
	DEC sprite.animation_speed,x		;$B9E813   |
CODE_B9E815:					;	   |
	RTS					;$B9E815  /

CODE_B9E816:
	LDA sprite.movement_state,x		;$B9E816  \
	AND #$FF00				;$B9E818   |
	BNE CODE_B9E824				;$B9E81B   |
namespace animation
	LDA #DATA_F9596D			;$B9E81D   |
namespace off
	STA sprite.animation_address,x		;$B9E820   |
	STZ sprite.animation_routine,x		;$B9E822   |
CODE_B9E824:					;	   |
	RTS					;$B9E824  /

CODE_B9E825:
	%lda_sound(5, knock)			;$B9E825  \
	JSL queue_sound_effect			;$B9E828   |
	LDX current_sprite			;$B9E82C   |
	LDA sprite.max_y_speed,x		;$B9E82E   |
	EOR #$FFFF				;$B9E830   |
	INC A					;$B9E833   |
	ASL A					;$B9E834   |
	STA CPU.dividen				;$B9E835   |
	LDA sprite.unknown_22,x			;$B9E838   |
	SEP #$20				;$B9E83A   |
	STA CPU.divisor				;$B9E83C   |
	REP #$20				;$B9E83F   |
	LDA sprite.general_purpose_50,x		;$B9E841   |
	STA sprite.max_x_speed,x		;$B9E843   |
	LDA #$3C00				;$B9E845   |
	STA CPU.dividen				;$B9E848   |
	LDA CPU.divide_result			;$B9E84B   |
	STA CPU.divisor				;$B9E84E   |
	LDA sprite.max_y_speed,x		;$B9E851   |
	STA sprite.y_speed,x			;$B9E853   |
	NOP					;$B9E855   |
	NOP					;$B9E856   |
	NOP					;$B9E857   |
	LDA CPU.divide_result			;$B9E858   |
	STA sprite.animation_speed,x		;$B9E85B   |
	INC sprite.sub_state,x			;$B9E85D   |
	RTS					;$B9E85F  /

CODE_B9E860:
	%lda_sound(5, knock)			;$B9E860  \
	JSL queue_sound_effect			;$B9E863   |
	LDX current_sprite			;$B9E867   |
	LDA #!default_animation_speed		;$B9E869   |
	STA sprite.animation_speed,x		;$B9E86C   |
	LDA sprite.general_purpose_50,x		;$B9E86E   |
	BEQ CODE_B9E87C				;$B9E870   |
	LSR A					;$B9E872   |
	EOR sprite.oam_property,x		;$B9E873   |
	AND #$4000				;$B9E875   |
	BEQ CODE_B9E87C				;$B9E878   |
	SEC					;$B9E87A   |
	RTS					;$B9E87B  /

CODE_B9E87C:
	CLC					;$B9E87C  \
	RTS					;$B9E87D  /

CODE_B9E87E:
	LDY #$0198				;$B9E87E  \
	LDA #$0007				;$B9E881   |
	JMP CODE_B9E898				;$B9E884  /

CODE_B9E887:
	LDA sprite.unknown_22,x			;$B9E887  \
	EOR sprite.oam_property,x		;$B9E889   |
	AND #$4000				;$B9E88B   |
	BEQ CODE_B9E897				;$B9E88E   |
	LDA #$02DC				;$B9E890   |
	JSL set_sprite_animation		;$B9E893   |
CODE_B9E897:					;	   |
	RTS					;$B9E897  /

CODE_B9E898:
	STA CPU.multiply_A			;$B9E898  \
	LDA sprite.movement_state,x		;$B9E89B   |
	AND #$00FF				;$B9E89D   |
	CMP #$000A				;$B9E8A0   |
	BCC CODE_B9E8E0				;$B9E8A3   |
	CMP #$000C				;$B9E8A5   |
	BCS CODE_B9E8E0				;$B9E8A8   |
	LDA sprite.max_x_speed,x		;$B9E8AA   |
	BPL CODE_B9E8B2				;$B9E8AC   |
	EOR #$FFFF				;$B9E8AE   |
	INC A					;$B9E8B1   |
CODE_B9E8B2:					;	   |
	ASL A					;$B9E8B2   |
	ASL A					;$B9E8B3   |
	XBA					;$B9E8B4   |
	STA CPU.multiply_B			;$B9E8B5   |
	LDA sprite.general_purpose_46,x		;$B9E8B8   |
	CLC					;$B9E8BA   |
	ADC #$4000				;$B9E8BB   |
	STA $5E					;$B9E8BE   |
	LDA CPU.multiply_result			;$B9E8C0   |
	XBA					;$B9E8C3   |
	LSR A					;$B9E8C4   |
	LSR A					;$B9E8C5   |
	BIT sprite.max_x_speed,x		;$B9E8C6   |
	BPL CODE_B9E8CE				;$B9E8C8   |
	EOR #$FFFF				;$B9E8CA   |
	SEC					;$B9E8CD   |
CODE_B9E8CE:					;	   |
	ADC $5E					;$B9E8CE   |
	EOR sprite.max_x_speed,x		;$B9E8D0   |
	LSR A					;$B9E8D2   |
	EOR sprite.oam_property,x		;$B9E8D3   |
	AND #$4000				;$B9E8D5   |
	BEQ CODE_B9E8DF				;$B9E8D8   |
	TYA					;$B9E8DA   |
	JSL set_sprite_animation		;$B9E8DB   |
CODE_B9E8DF:					;	   |
	RTS					;$B9E8DF  /

CODE_B9E8E0:
	JMP turn_sprite_if_needed		;$B9E8E0  /

CODE_B9E8E3:
	LDY #$019C				;$B9E8E3  \
	LDA #$0008				;$B9E8E6   |
	JMP CODE_B9E898				;$B9E8E9  /

CODE_B9E8EC:
	BIT sprite.y_speed,x			;$B9E8EC  \
	BPL CODE_B9E8F1				;$B9E8EE   |
	RTS					;$B9E8F0  /

CODE_B9E8F1:
	STZ sprite.y_speed,x			;$B9E8F1  \
	RTS					;$B9E8F3  /

CODE_B9E8F4:
	LDA sprite.y_speed,x			;$B9E8F4  \
	ASL A					;$B9E8F6   |
	CLC					;$B9E8F7   |
	ADC sprite.y_speed,x			;$B9E8F8   |
	STA sprite.y_speed,x			;$B9E8FA   |
	RTS					;$B9E8FC  /

CODE_B9E8FD:
	ASL sprite.y_speed,x			;$B9E8FD  \
	ASL sprite.y_speed,x			;$B9E8FF   |
	RTS					;$B9E901  /

CODE_B9E902:
	LDA sprite.max_y_speed,x		;$B9E902  \
	EOR #$8000				;$B9E904   |
	ASL A					;$B9E907   |
	RTS					;$B9E908  /

CODE_B9E909:
	LDA sprite.max_y_speed,x		;$B9E909  \
	ASL A					;$B9E90B   |
	BCC CODE_B9E912				;$B9E90C   |
	LDA sprite.general_purpose_4E,x		;$B9E90E   |
	STA sprite.animation_speed,x		;$B9E910   |
CODE_B9E912:					;	   |
	RTS					;$B9E912  /

CODE_B9E913:
	LDA sprite.max_y_speed,x		;$B9E913  \
	BMI CODE_B9E91E				;$B9E915   |
	ASL A					;$B9E917   |
	ASL A					;$B9E918   |
	STA sprite.y_speed,x			;$B9E919   |
	STZ sprite.y_sub_position,x		;$B9E91B   |
	RTS					;$B9E91D  /

CODE_B9E91E:
	BIT sprite.y_speed,x			;$B9E91E  \
	BPL CODE_B9E928				;$B9E920   |
	LDA #$FFFF				;$B9E922   |
	STA sprite.y_speed,x			;$B9E925   |
	RTS					;$B9E927  /

CODE_B9E928:
	LDA active_frame_counter		;$B9E928  \
	AND #$0007				;$B9E92A   |
	BNE CODE_B9E936				;$B9E92D   |
	LDY #!special_sprite_spawn_id_0052	;$B9E92F   |
	JSL spawn_no_gfx_special_sprite_index	;$B9E932   |
CODE_B9E936:					;	   |
	RTS					;$B9E936  /

CODE_B9E937:
	LDA #$FD80				;$B9E937  \
	BIT sprite.oam_property,x		;$B9E93A   |
	BVC CODE_B9E941				;$B9E93C   |
	LDA #$0280				;$B9E93E   |
CODE_B9E941:					;	   |
	STA sprite.x_speed,x			;$B9E941   |
	EOR #$FFFF				;$B9E943   |
	INC A					;$B9E946   |
	CMP #$8000				;$B9E947   |
	ROR A					;$B9E94A   |
	STA sprite.max_x_speed,x		;$B9E94B   |
	INC sprite.movement_sub_state,x		;$B9E94D   |
	RTS					;$B9E94F  /

CODE_B9E950:
	LDA #$FC80				;$B9E950  \
	STA sprite.y_speed,x			;$B9E953   |
	INC sprite.movement_sub_state,x		;$B9E955   |
	RTS					;$B9E957  /

CODE_B9E958:
	DEC sprite.general_purpose_50,x		;$B9E958  \
	BPL CODE_B9E96B				;$B9E95A   |
	LDA sprite.sub_state,x			;$B9E95C   |
	AND #$00FF				;$B9E95E   |
	CMP #$0001				;$B9E961   |
	BNE CODE_B9E96B				;$B9E964   |
	INC sprite.sub_state,x			;$B9E966   |
	BRL process_anim_script			;$B9E968  /

CODE_B9E96B:
	BRL CODE_B9D12B				;$B9E96B  /

CODE_B9E96E:
	INC $19BC				;$B9E96E  \
	RTS					;$B9E971  /

turn_kutlass_if_needed:
	LDY #$016A				;$B9E972  \
	JMP turn_sprite_if_needed		;$B9E975  /

CODE_B9E978:
	LDA sprite.general_purpose_48,x		;$B9E978  \
	DEC sprite.unknown_5C,x			;$B9E97A   |
	BMI CODE_B9E987				;$B9E97C   |
	CMP #$8000				;$B9E97E   |
	ROR A					;$B9E981   |
	CLC					;$B9E982   |
	ADC sprite.general_purpose_48,x		;$B9E983   |
	BRA CODE_B9E988				;$B9E985  /

CODE_B9E987:
	ASL A					;$B9E987  \
CODE_B9E988:					;	   |
	BPL CODE_B9E98E				;$B9E988   |
	EOR #$FFFF				;$B9E98A   |
	INC A					;$B9E98D   |
CODE_B9E98E:					;	   |
	BIT sprite.oam_property,x		;$B9E98E   |
	BVC CODE_B9E996				;$B9E990   |
	EOR #$FFFF				;$B9E992   |
	INC A					;$B9E995   |
CODE_B9E996:					;	   |
	STA sprite.max_x_speed,x		;$B9E996   |
	STA sprite.x_speed,x			;$B9E998   |
	LDA sprite.current_graphic,x		;$B9E99A   |
	SEC					;$B9E99C   |
	SBC #$0F54				;$B9E99D   |
	CMP #$0020				;$B9E9A0   |
	BCS CODE_B9E9AF				;$B9E9A3   |
	LSR A					;$B9E9A5   |
	STA $32					;$B9E9A6   |
	LSR A					;$B9E9A8   |
	ADC $32					;$B9E9A9   |
	ADC sprite.animation_address,x		;$B9E9AB   |
	STA sprite.animation_address,x		;$B9E9AD   |
CODE_B9E9AF:					;	   |
	RTS					;$B9E9AF  /

CODE_B9E9B0:
	LDA sprite.general_purpose_4B,x		;$B9E9B0  \
	AND #$00FF				;$B9E9B2   |
	CMP #$0001				;$B9E9B5   |
	BCC CODE_B9E9BC				;$B9E9B8   |
	DEC sprite.general_purpose_4B,x		;$B9E9BA   |
CODE_B9E9BC:					;	   |
	RTS					;$B9E9BC  /

CODE_B9E9BD:
	LDA sprite.display_mode_high,x		;$B9E9BD  \> This variable serves a different purpose for krook
	AND #$00FF				;$B9E9BF   |
	DEC A					;$B9E9C2   |
	CMP #$8000				;$B9E9C3   |
	BCS CODE_B9E9CA				;$B9E9C6   |
	DEC sprite.display_mode_high,x		;$B9E9C8   |> This variable serves a different purpose for krook
CODE_B9E9CA:					;	   |
	RTS					;$B9E9CA  /

turn_krook_if_needed:
	LDY #$01EF				;$B9E9CB  \
	JMP turn_sprite_if_needed		;$B9E9CE  /

spawn_krooks_hook:
	LDY #!special_sprite_spawn_id_000A	;$B9E9D1  \
	JSL spawn_special_sprite_index		;$B9E9D4   | Spawn krook's hook
	LDY alternate_sprite			;$B9E9D8   | Get hook sprite
	LDX current_sprite			;$B9E9DA   | Get krook sprite
	STY sprite.general_purpose_50,x		;$B9E9DC   | Store pointer of hook into krook
	STX sprite.general_purpose_50,y		;$B9E9DE   | Vice versa
	RTS					;$B9E9E0  / Return

CODE_B9E9E1:
	LDA sprite.general_purpose_50,x		;$B9E9E1  \
	BNE CODE_B9E9EE				;$B9E9E3   |
namespace animation
	LDA #DATA_F95EF2			;$B9E9E5   |
namespace off
	STA sprite.animation_address,x		;$B9E9E8   |
	STZ sprite.animation_timer,x		;$B9E9EA   |
	STZ sprite.animation_routine,x		;$B9E9EC   |
CODE_B9E9EE:					;	   |
	RTS					;$B9E9EE  /

CODE_B9E9EF:
	LDA sprite.terrain_attributes,x		;$B9E9EF  \
	AND #$00FF				;$B9E9F1   |
	BEQ CODE_B9EA05				;$B9E9F4   |
	DEC sprite.terrain_attributes,x		;$B9E9F6   |
	SEC					;$B9E9F8   |
	RTS					;$B9E9F9  /

CODE_B9E9FA:
	LDA sprite.terrain_attributes_high,x	;$B9E9FA  \
	AND #$00FF				;$B9E9FC   |
	BEQ CODE_B9EA05				;$B9E9FF   |
	DEC sprite.terrain_attributes_high,x	;$B9EA01   |
	SEC					;$B9EA03   |
	RTS					;$B9EA04  /

CODE_B9EA05:
	CLC					;$B9EA05  \
	RTS					;$B9EA06  /

CODE_B9EA07:
	LDA sprite.animation_speed,x		;$B9EA07  \
	CMP #$0280				;$B9EA09   |
	BCC CODE_B9EA14				;$B9EA0C   |
	LDA #CODE_B9EA1A			;$B9EA0E   |
	STA sprite.animation_routine,x		;$B9EA11   |
	RTS					;$B9EA13  /

CODE_B9EA14:
	ADC #$0004				;$B9EA14  \
	STA sprite.animation_speed,x		;$B9EA17   |
	RTS					;$B9EA19  /

CODE_B9EA1A:
	LDA sprite.animation_speed,x		;$B9EA1A  \
	CMP #$0140				;$B9EA1C   |
	BCS CODE_B9EA24				;$B9EA1F   |
	STZ sprite.animation_routine,x		;$B9EA21   |
	RTS					;$B9EA23  /

CODE_B9EA24:
	SBC #$0004				;$B9EA24  \
	STA sprite.animation_speed,x		;$B9EA27   |
CODE_B9EA29:					;	   |
	LDA sprite.animation_routine,x		;$B9EA29   |
	CMP #$0001				;$B9EA2B   |
	RTS					;$B9EA2E  /

CODE_B9EA2F:
	STZ sprite.movement_state,x		;$B9EA2F  \> This variable serves a different purpose for cat o 9 tails
	RTS					;$B9EA31  /

CODE_B9EA32:
	LDA #$018C				;$B9EA32  \
	STA sprite.movement_state,x		;$B9EA35   |> This variable serves a different purpose for cat o 9 tails
	RTS					;$B9EA37  /

CODE_B9EA38:
	LDA #$018D				;$B9EA38  \
	STA sprite.movement_state,x		;$B9EA3B   |> This variable serves a different purpose for cat o 9 tails
	RTS					;$B9EA3D  /

spawn_cat_o9_tails_dizzy_stars:
	LDY #!special_sprite_spawn_id_0094	;$B9EA3E  \
	JSL spawn_special_sprite_index		;$B9EA41   | Spawn stars
	LDX current_sprite			;$B9EA45   | Get cat sprite
	BCS .return				;$B9EA47   | If spawn failed, return
	LDA alternate_sprite			;$B9EA49   | Else get stars sprite
	STA sprite.general_purpose_4E,x		;$B9EA4B   | Store pointer to it in cat
.return:					;	   |
	RTS					;$B9EA4D  / Return

CODE_B9EA4E:
	LDY sprite.general_purpose_4E,x		;$B9EA4E  \
	LDA #$0001				;$B9EA50   |
	STA.w sprite.general_purpose_42,y	;$B9EA53   |
	STZ sprite.general_purpose_4E,x		;$B9EA56   |
	RTS					;$B9EA58  /

DATA_F9EA59:
	db $06 : dw $2514
	db $06 : dw $2518
	db $06 : dw $251C
	db $06 : dw $2520
	db $03 : dw $251C
	db $03 : dw $2518
	db !animation_command_92, $50

DATA_F9EA6D:
	db $06 : dw $2514
	db $06 : dw $2524
	db $06 : dw $2528
	db $08 : dw $252C
	db $08 : dw $2530
	db $08 : dw $2534
	db !animation_command_92, $50

DATA_F9EA81:
	db !animation_command_8E, $26, $05
	db $03 : dw $24C4
	db $03 : dw $24C8
	db $03 : dw $24CC
	db $03 : dw $24D0
	db $03 : dw $24D4
	db $03 : dw $24D8
	db $03 : dw $24DC
	db $03 : dw $24E0
	db !animation_command_92, $50

CODE_B9EA9E:
	INC sprite.animation_speed,x		;$B9EA9E   |
	LDA sprite.animation_speed,x		;$B9EAA0   |
	CMP #$00C0				;$B9EAA2   |
	BCC CODE_B9EAAE				;$B9EAA5   |
	INC sprite.state,x			;$B9EAA7   |
	LDA #CODE_B9EAAF			;$B9EAA9   |
	STA sprite.animation_routine,x		;$B9EAAC   |
CODE_B9EAAE:					;	   |
	RTS					;$B9EAAE  /

CODE_B9EAAF:
	DEC sprite.animation_speed,x		;$B9EAAF  \
	LDA sprite.animation_speed,x		;$B9EAB1   |
	CMP #$0055				;$B9EAB3   |
	BCS CODE_B9EAAE				;$B9EAB6   |
	LDA #$0004				;$B9EAB8   |
	STA sprite.state,x			;$B9EABB   |
	RTS					;$B9EABD  /

turn_kloak_if_needed:
	LDA sprite.terrain_attributes,x		;$B9EABE  \
	BEQ .turn				;$B9EAC0   |
	LDA #$02D2				;$B9EAC2   | If shouldn't turn, set throw animation
	JSL set_sprite_animation		;$B9EAC5   |
	RTS					;$B9EAC9  /
	
.turn:
	LDY #$02D1				;$B9EACA  \
	JMP turn_sprite_if_needed		;$B9EACD  /

CODE_B9EAD0:
	LDA sprite.general_purpose_4E,x		;$B9EAD0  \
	LSR A					;$B9EAD2   |
	BCC CODE_B9EAFB				;$B9EAD3   |
namespace animation
	LDA #DATA_F96266			;$B9EAD5   |
namespace off
	STA sprite.animation_address,x		;$B9EAD8   |
	STZ sprite.animation_routine,x		;$B9EADA   |
	RTS					;$B9EADC  /

CODE_B9EADD:
	LDA sprite.general_purpose_4E,x		;$B9EADD  \
	LSR A					;$B9EADF   |
	BCS CODE_B9EAFB				;$B9EAE0   |
namespace animation
	LDA #DATA_F962EC			;$B9EAE2   |
namespace off
	STA sprite.animation_address,x		;$B9EAE5   |
	STZ sprite.animation_routine,x		;$B9EAE7   |
	RTS					;$B9EAE9  /

CODE_B9EAEA:
	BIT sprite.y_speed,x			;$B9EAEA  \
	CLC					;$B9EAEC   |
	BMI CODE_B9EAFB				;$B9EAED   |
	LDA sprite.movement_state,x		;$B9EAEF   |
	AND #$00FF				;$B9EAF1   |
	SEC					;$B9EAF4   |
	SBC #$0004				;$B9EAF5   |
	CMP #$0001				;$B9EAF8   |
CODE_B9EAFB:					;	   |
	RTS					;$B9EAFB  /

CODE_B9EAFC:
	LDA sprite.general_purpose_44,x		;$B9EAFC  \
	BEQ CODE_B9EB23				;$B9EAFE   |
	STZ sprite.animation_routine,x		;$B9EB00   |
namespace animation
	LDA #DATA_F96320			;$B9EB02   |
namespace off
	STA sprite.animation_address,x		;$B9EB05   |
	RTS					;$B9EB07  /

CODE_B9EB08:
	CPX $0D58				;$B9EB08  \
	BEQ CODE_B9EB23				;$B9EB0B   |
	CLC					;$B9EB0D   |
	RTS					;$B9EB0E  /

CODE_B9EB0F:
	LDY sprite.general_purpose_4E,x		;$B9EB0F  \
	JSL spawn_special_sprite_address	;$B9EB11   |
	BCS CODE_B9EB23				;$B9EB15   |
	LDY alternate_sprite			;$B9EB17   |
	LDA #$0854				;$B9EB19   |
	STA.w sprite.current_graphic,y		;$B9EB1C   |
	LDX current_sprite			;$B9EB1F   |
	STY.b sprite.general_purpose_48,x	;$B9EB21   |
CODE_B9EB23:					;	   |
	RTS					;$B9EB23  /

set_krochead_bounce_interaction:
	LDA #!player_interaction_09		;$B9EB24  \
	JSL set_player_interaction_global	;$B9EB27   |
	RTS					;$B9EB2B  /

CODE_B9EB2C:
	LDA sprite.general_purpose_4E,x		;$B9EB2C  \
	AND #$0003				;$B9EB2E   |
	CMP #$0003				;$B9EB31   |
	BNE CODE_B9EB93				;$B9EB34   |
	BRL CODE_B9EB90				;$B9EB36  /

CODE_B9EB39:
	LDA.l active_kong_sprite		;$B9EB39  \
	TAX					;$B9EB3D   |
	LDA sprite.state,x			;$B9EB3E   |
	LDX current_sprite			;$B9EB40   |
	CMP #$0055				;$B9EB42   |
	BNE CODE_B9EB90				;$B9EB45   |
	PHB					;$B9EB47   |
	PHK					;$B9EB48   |
	PLB					;$B9EB49   |
	LDA #!player_interaction_0E		;$B9EB4A   |
	JSL set_player_interaction_global	;$B9EB4D   |
	PLB					;$B9EB51   |
	BCS CODE_B9EB93				;$B9EB52   |
	%lda_sound(5, krockhead_brown)		;$B9EB54   |
	JSL queue_sound_effect			;$B9EB57   |
	LDX current_sprite			;$B9EB5B   |
	LDA sprite.general_purpose_46,x		;$B9EB5D   |
	STA.l interaction_RAM_0A86		;$B9EB5F   |
	LDA sprite.general_purpose_48,x		;$B9EB63   |
	STA.l interaction_RAM_0A88		;$B9EB65   |
	BRL process_anim_script			;$B9EB69  /

CODE_B9EB6C:
	LDA sprite.general_purpose_4A,x		;$B9EB6C  \
	INC sprite.general_purpose_4A,x		;$B9EB6E   |
	INC sprite.general_purpose_4A,x		;$B9EB70   |
	STA current_sprite_constants		;$B9EB72   |
	LDA [current_sprite_constants]		;$B9EB74   |
	BMI CODE_B9EB7F				;$B9EB76   |
	STA sprite.general_purpose_42,x		;$B9EB78   |
	LDA sprite.constants_address,x		;$B9EB7A   |
	STA current_sprite_constants		;$B9EB7C   |
	RTS					;$B9EB7E  /

CODE_B9EB7F:
	LDY #$0002				;$B9EB7F  \
	LDA [current_sprite_constants],y	;$B9EB82   |
	STA sprite.general_purpose_4A,x		;$B9EB84   |
	BRA CODE_B9EB6C				;$B9EB86  /

CODE_B9EB88:
	LDA sprite.general_purpose_42,x		;$B9EB88  \
	BEQ CODE_B9EB93				;$B9EB8A   |
	DEC sprite.general_purpose_42,x		;$B9EB8C   |
	BNE CODE_B9EB93				;$B9EB8E   |
CODE_B9EB90:					;	   |
	BRL process_anim_script			;$B9EB90  /

CODE_B9EB93:
	BRL CODE_B9D12B				;$B9EB93  /

CODE_B9EB96:
	LDA sprite.general_purpose_42,x		;$B9EB96  \
	BEQ CODE_B9EB93				;$B9EB98   |
	BRL process_anim_script			;$B9EB9A  /

CODE_B9EB9D:
	LDA sprite.general_purpose_42,x		;$B9EB9D  \
	BNE CODE_B9EB93				;$B9EB9F   |
	BRL process_anim_script			;$B9EBA1  /

CODE_B9EBA4:
	LDA sprite.general_purpose_4E,x		;$B9EBA4  \
	AND #$0003				;$B9EBA6   |
	CMP #$0003				;$B9EBA9   |
	BNE CODE_B9EBD6				;$B9EBAC   |
	%lda_sound(6, krockhead_green)		;$B9EBAE   |
	JSL queue_sound_effect			;$B9EBB1   |
	LDX current_sprite			;$B9EBB5   |
	LDA sprite.animation_address,x		;$B9EBB7   |
	STA sprite.unknown_2C,x			;$B9EBB9   |
	LDA sprite.current_graphic,x		;$B9EBBB   |
	SEC					;$B9EBBD   |
	SBC #$22D0				;$B9EBBE   |
	BCC CODE_B9EBD6				;$B9EBC1   |
	CMP #$000C				;$B9EBC3   |
	BCC CODE_B9EBCB				;$B9EBC6   |
	LDA #$000C				;$B9EBC8   |
CODE_B9EBCB:					;	   |
	LSR A					;$B9EBCB   |
	TAY					;$B9EBCC   |
	LDA DATA_B9EBD7,y			;$B9EBCD   |
	STA sprite.animation_address,x		;$B9EBD0   |
	STZ sprite.animation_routine,x		;$B9EBD2   |
	STZ sprite.animation_timer,x		;$B9EBD4   |
CODE_B9EBD6:					;	   |
	RTS					;$B9EBD6  /

DATA_B9EBD7:
	dw DATA_B9EBDF
	dw DATA_B9EBF0
	dw DATA_B9EC01
	dw DATA_B9EC12

DATA_B9EBDF:
	db $03 : dw $22CC
	db $03 : dw $22C8
	db $03 : dw $22CC
	db $03 : dw $22D0
	db !animation_command_84 : dw CODE_B9EBA4
	db !animation_command_92, $2C

DATA_B9EBF0:
	db $03 : dw $22D0
	db $03 : dw $22CC
	db $03 : dw $22D0
	db $03 : dw $22D4
	db !animation_command_84 : dw CODE_B9EBA4
	db !animation_command_92, $2C

DATA_B9EC01:
	db $03 : dw $22D4
	db $03 : dw $22D0
	db $03 : dw $22D4
	db $03 : dw $22D8
	db !animation_command_84 : dw CODE_B9EBA4
	db !animation_command_92, $2C

DATA_B9EC12:
	db $03 : dw $22D8
	db $03 : dw $22D4
	db $03 : dw $22D8
	db $03 : dw $22DC
	db !animation_command_84 : dw CODE_B9EBA4
	db !animation_command_92, $2C

CODE_B9EC23:
	LDA sprite.general_purpose_46,x		;$B9EC23  \
	LSR A					;$B9EC25   |
	BCC CODE_B9EC2D				;$B9EC26   |
	INC sprite.general_purpose_46,x		;$B9EC28   |
	BRL process_anim_script			;$B9EC2A  /

CODE_B9EC2D:
	BRL CODE_B9D12B				;$B9EC2D  /

CODE_B9EC30:
	LDA sprite.general_purpose_4E,x		;$B9EC30  \
	CMP #$0001				;$B9EC32   |
	RTS					;$B9EC35  /

CODE_B9EC36:
	LDA sprite.general_purpose_4A,x		;$B9EC36  \
	BIT #$0080				;$B9EC38   |
	BEQ CODE_B9EC4A				;$B9EC3B   |
namespace animation
	LDA #DATA_F9654C			;$B9EC3D   |
namespace off
	STA sprite.animation_address,x		;$B9EC40   |
	STZ sprite.animation_timer,x		;$B9EC42   |
	LDA #CODE_B9EC4A			;$B9EC44   |
	STA sprite.animation_routine,x		;$B9EC47   |
	RTS					;$B9EC49  /

CODE_B9EC4A:
	LDA sprite.general_purpose_48,x		;$B9EC4A  \
	CMP #$0009				;$B9EC4C   |
	BEQ CODE_B9EC66				;$B9EC4F   |
namespace animation
	LDA #DATA_F96504			;$B9EC51   |
namespace off
	STA sprite.animation_address,x		;$B9EC54   |
	STZ sprite.animation_routine,x		;$B9EC56   |
	STZ sprite.animation_timer,x		;$B9EC58   |
	LDA sprite.general_purpose_48,x		;$B9EC5A   |
	AND #$00FF				;$B9EC5C   |
	ASL A					;$B9EC5F   |
	ASL A					;$B9EC60   |
	ADC #$2B08				;$B9EC61   |
	STA sprite.current_graphic,x		;$B9EC64   |
CODE_B9EC66:					;	   |
	RTS					;$B9EC66  /

CODE_B9EC67:
	STZ sprite.animation_routine,x		;$B9EC67  \
	LDA sprite.general_purpose_48,x		;$B9EC69   |
	CMP #$0009				;$B9EC6B   |
	BNE CODE_B9EC88				;$B9EC6E   |
	PHX					;$B9EC70   |
	LDA sprite.general_purpose_42,x		;$B9EC71   |
	TAX					;$B9EC73   |
	LDA sprite.ground_distance,x		;$B9EC74   |
	PLX					;$B9EC76   |
	CMP #$0000				;$B9EC77   |
	BNE CODE_B9EC88				;$B9EC7A   |
	LDA sprite.general_purpose_4A,x		;$B9EC7C   |
	BIT #$0100				;$B9EC7E   |
	BNE CODE_B9ECDC				;$B9EC81   |
	BIT #$0080				;$B9EC83   |
	BNE CODE_B9EC9C				;$B9EC86   |
CODE_B9EC88:					;	   |
	LDA sprite.general_purpose_48,x		;$B9EC88   |
	AND #$00FF				;$B9EC8A   |
	ASL A					;$B9EC8D   |
	ASL A					;$B9EC8E   |
	ADC #$2B08				;$B9EC8F   |
	STA sprite.current_graphic,x		;$B9EC92   |
	LDA #!default_animation_speed		;$B9EC94   |
	STA sprite.animation_timer,x		;$B9EC97   |
	BRL CODE_B9D12B				;$B9EC99  /

CODE_B9EC9C:
	LDA sprite.x_position,x			;$B9EC9C  \
	CMP sprite.x_sub_position,x		;$B9EC9E   |
	BCS CODE_B9ECCE				;$B9ECA0   |
	LDA.l active_kong_sprite		;$B9ECA2   |
	TAX					;$B9ECA6   |
	LDA sprite.x_position,x			;$B9ECA7   |
	SEC					;$B9ECA9   |
	LDX current_sprite			;$B9ECAA   |
	SBC sprite.x_position,x			;$B9ECAC   |
	CLC					;$B9ECAE   |
	ADC #$0040				;$B9ECAF   |
	CMP #$FF00				;$B9ECB2   |
	BCC CODE_B9EC88				;$B9ECB5   |
	LDA sprite.general_purpose_42,x		;$B9ECB7   |
	TAX					;$B9ECB9   |
	LDA sprite.animation_routine,x		;$B9ECBA   |> This probably isnt used as a code pointer in this case
	AND #$FF7F				;$B9ECBC   |
	STA sprite.animation_routine,x		;$B9ECBF   |> This probably isnt used as a code pointer in this case
	LDX current_sprite			;$B9ECC1   |
	TYA					;$B9ECC3   |
	CLC					;$B9ECC4   |
	ADC #$0003				;$B9ECC5   |
	TAY					;$B9ECC8   |
	STZ sprite.animation_timer,x		;$B9ECC9   |
	BRL process_anim_script			;$B9ECCB  /

CODE_B9ECCE:
	LDA sprite.general_purpose_42,x		;$B9ECCE  \
	TAX					;$B9ECD0   |
	LDA sprite.animation_routine,x		;$B9ECD1   |> This probably isnt used as a code pointer in this case
	AND #$FF7F				;$B9ECD3   |
	STA sprite.animation_routine,x		;$B9ECD6   |> This probably isnt used as a code pointer in this case
	LDX current_sprite			;$B9ECD8   |
	BRA CODE_B9EC88				;$B9ECDA  /

CODE_B9ECDC:
	PHX					;$B9ECDC  \
	LDA sprite.general_purpose_42,x		;$B9ECDD   |
	TAX					;$B9ECDF   |
	LDA sprite.animation_routine,x		;$B9ECE0   |> This probably isnt used as a code pointer in this case
	AND #$FEFF				;$B9ECE2   |
	STA sprite.animation_routine,x		;$B9ECE5   |> This probably isnt used as a code pointer in this case
	PLX					;$B9ECE7   |
	STZ sprite.animation_timer,x		;$B9ECE8   |
	BRL process_anim_script			;$B9ECEA  /

CODE_B9ECED:
	LDY sprite.general_purpose_42,x		;$B9ECED  \
	LDA.w sprite.ground_distance,y		;$B9ECEF   |
	BNE CODE_B9ECF5				;$B9ECF2   |
	RTS					;$B9ECF4  /

CODE_B9ECF5:
	STZ sprite.animation_routine,x		;$B9ECF5  \
	LDY sprite.general_purpose_42,x		;$B9ECF7   |
	STZ sprite.general_purpose_42,x		;$B9ECF9   |
	LDA sprite.general_purpose_44,x		;$B9ECFB   |
	BEQ CODE_B9ED06				;$B9ECFD   |
	LDA #$0000				;$B9ECFF   |
	STA.w sprite.general_purpose_48,y	;$B9ED02   |
	RTS					;$B9ED05  /

CODE_B9ED06:
	STA.w sprite.general_purpose_46,y	;$B9ED06  \
	RTS					;$B9ED09  /

turn_screech_if_needed:
	LDA sprite.unknown_22,x			;$B9ED0A  \
	EOR sprite.oam_property,x		;$B9ED0C   |
	AND #$4000				;$B9ED0E   |
	BEQ CODE_B9ED1B				;$B9ED11   |
	LDA #$02DE				;$B9ED13   |
	JSL set_sprite_animation		;$B9ED16   |
	RTS					;$B9ED1A  /

CODE_B9ED1B:
	LDA sprite.unknown_28,x			;$B9ED1B  \
	STA sprite.animation_speed,x		;$B9ED1D   |
	RTS					;$B9ED1F  /

CODE_B9ED20:
	LDA sprite.x_speed,x			;$B9ED20  \
	CLC					;$B9ED22   |
	ADC sprite.max_x_speed,x		;$B9ED23   |
	BPL CODE_B9ED2B				;$B9ED25   |
	EOR #$FFFF				;$B9ED27   |
	INC A					;$B9ED2A   |
CODE_B9ED2B:					;	   |
	LSR A					;$B9ED2B   |
	CMP #$0018				;$B9ED2C   |
	BCS CODE_B9ED34				;$B9ED2F   |
	LDA #$0018				;$B9ED31   |
CODE_B9ED34:					;	   |
	STA sprite.animation_speed,x		;$B9ED34   |
	LDA sprite.animation_address,x		;$B9ED36   |
namespace animation
	CMP #DATA_F96600			;$B9ED38   |
namespace off
	ROR A					;$B9ED3B   |
	EOR sprite.max_x_speed,x		;$B9ED3C   |
	BPL CODE_B9ED6E				;$B9ED3E   |
	BIT sprite.max_x_speed,x		;$B9ED40   |
	BPL CODE_B9ED5B				;$B9ED42   |
	LDA sprite.current_graphic,x		;$B9ED44   |
	SEC					;$B9ED46   |
	SBC #$2CAC				;$B9ED47   |
	LSR A					;$B9ED4A   |
	STA $5E					;$B9ED4B   |
	LSR A					;$B9ED4D   |
	CLC					;$B9ED4E   |
	ADC $5E					;$B9ED4F   |
	EOR #$FFFF				;$B9ED51   |
	SEC					;$B9ED54   |
namespace animation
	ADC #DATA_F96609			;$B9ED55   |
namespace off
	STA sprite.animation_address,x		;$B9ED58   |
	RTS					;$B9ED5A  /

CODE_B9ED5B:
	LDA sprite.current_graphic,x		;$B9ED5B  \
	SEC					;$B9ED5D   |
	SBC #$2CAC				;$B9ED5E   |
	LSR A					;$B9ED61   |
	STA $5E					;$B9ED62   |
	LSR A					;$B9ED64   |
	CLC					;$B9ED65   |
	ADC $5E					;$B9ED66   |
	CLC					;$B9ED68   |
namespace animation
	ADC #DATA_F965F4			;$B9ED69   |
namespace off
	STA sprite.animation_address,x		;$B9ED6C   |
CODE_B9ED6E:					;	   |
	RTS					;$B9ED6E  /

CODE_B9ED6F:
	LDA sprite.state,x			;$B9ED6F  \
	CMP #$0001				;$B9ED71   |
	BEQ CODE_B9ED77				;$B9ED74   |
	CLC					;$B9ED76   |
CODE_B9ED77:					;	   |
	RTS					;$B9ED77  /

CODE_B9ED78:
	LDA #$00D0				;$B9ED78  \
	STA sprite.render_order,x		;$B9ED7B   |
	LDA sprite.general_purpose_44,x		;$B9ED7D   |
	BIT sprite.oam_property,x		;$B9ED7F   |
	BVC CODE_B9ED87				;$B9ED81   |
	EOR #$FFFF				;$B9ED83   |
	INC A					;$B9ED86   |
CODE_B9ED87:					;	   |
	STA sprite.max_x_speed,x		;$B9ED87   |
	ASL A					;$B9ED89   |
	STA sprite.x_speed,x			;$B9ED8A   |
	RTS					;$B9ED8C  /

CODE_B9ED8D:
	LDA #$0147				;$B9ED8D  \
	JSL set_anim_handle_animal		;$B9ED90   |
	RTS					;$B9ED94  /

CODE_B9ED95:
	LDA sprite.terrain_interaction,x	;$B9ED95  \
	AND #$1001				;$B9ED97   |
	BEQ CODE_B9EDA1				;$B9ED9A   |
	LDA #$FD00				;$B9ED9C   |
	STA sprite.y_speed,x			;$B9ED9F   |
CODE_B9EDA1:					;	   |
	RTS					;$B9EDA1  /

CODE_B9EDA2:
	LDA #$00D0				;$B9EDA2  \
	STA sprite.render_order,x		;$B9EDA5   |
	LDA sprite.general_purpose_44,x		;$B9EDA7   |
	LSR A					;$B9EDA9   |
	LSR A					;$B9EDAA   |
	BIT sprite.oam_property,x		;$B9EDAB   |
	BVC CODE_B9EDB3				;$B9EDAD   |
	EOR #$FFFF				;$B9EDAF   |
	INC A					;$B9EDB2   |
CODE_B9EDB3:					;	   |
	STA sprite.max_x_speed,x		;$B9EDB3   |
	ASL A					;$B9EDB5   |
	STA sprite.x_speed,x			;$B9EDB6   |
	RTS					;$B9EDB8  /

CODE_B9EDB9:
	LDA #$00D0				;$B9EDB9  \
	STA sprite.render_order,x		;$B9EDBC   |
	LDA sprite.general_purpose_44,x		;$B9EDBE   |
	BIT sprite.oam_property,x		;$B9EDC0   |
	BVC CODE_B9EDC8				;$B9EDC2   |
	EOR #$FFFF				;$B9EDC4   |
	INC A					;$B9EDC7   |
CODE_B9EDC8:					;	   |
	STA sprite.max_x_speed,x		;$B9EDC8   |
	ASL A					;$B9EDCA   |
	STA sprite.x_speed,x			;$B9EDCB   |
	RTS					;$B9EDCD  /

CODE_B9EDCE:
	LDA #$00F0				;$B9EDCE  \
	STA sprite.render_order,x		;$B9EDD1   |
	RTS					;$B9EDD3  /

CODE_B9EDD4:
	LDA #$0001				;$B9EDD4  \
	TSB firework_flicker_flag		;$B9EDD7   |
	RTS					;$B9EDDA  /

CODE_B9EDDB:
	DEC $092E				;$B9EDDB  \
	LDA $092E				;$B9EDDE   |
	AND #$00FF				;$B9EDE1   |
	BNE CODE_B9EDE9				;$B9EDE4   |
	STA $0929				;$B9EDE6   |
CODE_B9EDE9:					;	   |
	JMP anim_delete_sprite			;$B9EDE9  /

CODE_B9EDEC:
	DEC sprite.general_purpose_44,x		;$B9EDEC  \
	BMI CODE_B9EDF1				;$B9EDEE   |
	RTS					;$B9EDF0  /

CODE_B9EDF1:
	LDA #$02A8				;$B9EDF1  \
	JSL set_sprite_animation		;$B9EDF4   |
	RTS					;$B9EDF8  /

CODE_B9EDF9:
	LDA #$0002				;$B9EDF9  \
	STA sprite.state,x			;$B9EDFC   |
	RTS					;$B9EDFE  /

CODE_B9EDFF:
	LDA sprite.general_purpose_46,x		;$B9EDFF  \
	STA sprite.general_purpose_44,x		;$B9EE01   |
	LDA #$02A6				;$B9EE03   |
	JSL set_sprite_animation		;$B9EE06   |
	RTS					;$B9EE0A  /

CODE_B9EE0B:
	DEC sprite.general_purpose_44,x		;$B9EE0B  \
	BMI CODE_B9EE10				;$B9EE0D   |
	RTS					;$B9EE0F  /

CODE_B9EE10:
	LDA #$02A9				;$B9EE10  \
	JSL set_sprite_animation		;$B9EE13   |
	RTS					;$B9EE17  /

CODE_B9EE18:
	LDA #$0003				;$B9EE18  \
	STA sprite.state,x			;$B9EE1B   |
	RTS					;$B9EE1D  /

CODE_B9EE1E:
	LDA sprite.general_purpose_48,x		;$B9EE1E  \
	STA sprite.general_purpose_44,x		;$B9EE20   |
	LDA #$02A7				;$B9EE22   |
	JSL set_sprite_animation		;$B9EE25   |
	RTS					;$B9EE29  /

turn_glimmer_if_needed:
	LDY active_kong_sprite			;$B9EE2A  \
	LDA.w sprite.oam_property,y		;$B9EE2D   |
	EOR sprite.oam_property,x		;$B9EE30   |
	AND #$4000				;$B9EE32   |
	BNE .turn				;$B9EE35   |
	RTS					;$B9EE37  /

.turn:
	LDA #$02CF				;$B9EE38  \
	JSL set_sprite_animation		;$B9EE3B   |
	RTS					;$B9EE3F  /

CODE_B9EE40:
	JSR CODE_B9E019				;$B9EE40  \
	LDA #$02CE				;$B9EE43   |
	JSL set_sprite_animation		;$B9EE46   |
	RTS					;$B9EE4A  /

play_dk_barrel_sound_if_one_kong:
	LDA game_state_flags			;$B9EE4B  \
	AND #$4000				;$B9EE4E   |
	BEQ .one_kong				;$B9EE51   |
	RTS					;$B9EE53  /

.one_kong:
	LDX.w #sound(5, !sound_kong_dk_barrel)	;$B9EE54  \
	LDY.w #sound(5, !sound_kong_dk_barrel)	;$B9EE57   |
	JSL play_kong_dependant_sound_global	;$B9EE5A   |
	RTS					;$B9EE5E  /

CODE_B9EE5F:
	LDA sprite.display_mode,x		;$B9EE5F  \
	CMP #$C000				;$B9EE61   |
	BEQ CODE_B9EE98				;$B9EE64   |
	LDA sprite.general_purpose_48,x		;$B9EE66   |
	TAY					;$B9EE68   |
	LDA.w sprite.type,y			;$B9EE69   |
	CMP #!sprite_dkbarrel_letters		;$B9EE6C   |
	BNE CODE_B9EE72				;$B9EE6F   |
	RTS					;$B9EE71  /

CODE_B9EE72:
	LDY.w #DATA_FF164E			;$B9EE72  \
	JSL spawn_special_sprite_address	;$B9EE75   |
	BCS CODE_B9EE98				;$B9EE79   |
	LDY alternate_sprite			;$B9EE7B   |
	LDA current_sprite			;$B9EE7D   |
	STA.w sprite.general_purpose_42,y	;$B9EE7F   |
	TYA					;$B9EE82   |
	LDX current_sprite			;$B9EE83   |
	STA sprite.general_purpose_48,x		;$B9EE85   |
	TXA					;$B9EE87   |
	CMP alternate_sprite			;$B9EE88   |
	BMI CODE_B9EE98				;$B9EE8A   |
	LDY alternate_sprite			;$B9EE8C   |
	LDA.w sprite.animation_timer,y		;$B9EE8E   |
	SEC					;$B9EE91   |
	SBC #$0100				;$B9EE92   |
	STA.w sprite.animation_timer,y		;$B9EE95   |
CODE_B9EE98:					;	   |
	RTS					;$B9EE98  /

CODE_B9EE99:
	LDA #$0002				;$B9EE99  \
	STA sprite.state,x			;$B9EE9C   |
	RTS					;$B9EE9E  /

spawn_animal_crate_smoke:
	LDY #!special_sprite_spawn_id_0048	;$B9EE9F  \
	JSL spawn_special_sprite_index		;$B9EEA2   |
	RTS					;$B9EEA6  /

spawn_animal_crate_animal:
	JSL delete_sprite_handle_deallocation	;$B9EEA7  \ Delete crate sprite
	LDA animal_type				;$B9EEAB   | Check if we're already an animal
	BNE .return				;$B9EEAD   | If yes, return
	LDX current_sprite			;$B9EEAF   | Else get crate sprite
	LDA sprite.state,x			;$B9EEB1   | Get crate state
	CMP #$0004				;$B9EEB3   | Check if in state 04 (which points to state 02)
	BEQ .return				;$B9EEB6   | If yes, return
	LDA sprite.general_purpose_42,x		;$B9EEB8   | Get animal animation ID (copied from $36,x)
	SEC					;$B9EEBA   |
	SBC #$02AC				;$B9EEBB   | Subtract base animal anim ID to get table index
	ASL A					;$B9EEBE   | *2
	TAX					;$B9EEBF   | Transfer to X
	LDA.l DATA_FF0D64,x			;$B9EEC0   | Get animal buddy spawn script address from table
	TAY					;$B9EEC4   | Transfer to Y
	JSL spawn_special_sprite_address	;$B9EEC5   | Spawn animal buddy
.return:					;	   |
	RTS					;$B9EEC9  / Return

;Unreferenced
CODE_B9EECA:
	RTL					;$B9EECA  |

CODE_B9EECB:
	LDA #$0080				;$B9EECB  \
	TSB RAM_0B02				;$B9EECE   |
	STZ sprite.x_speed,x			;$B9EED1   |
	STZ sprite.max_x_speed,x		;$B9EED3   |
	LDA #$0001				;$B9EED5   |
	STA sprite.state,x			;$B9EED8   |
	RTS					;$B9EEDA  /

CODE_B9EEDB:
	LDA #$0080				;$B9EEDB  \
	TRB RAM_0B02				;$B9EEDE   |
	RTS					;$B9EEE1  /

CODE_B9EEE2:
	LDA #$0003				;$B9EEE2  \
	STA sprite.state,x			;$B9EEE5   |
	RTS					;$B9EEE7  /

CODE_B9EEE8:
	JMP anim_delete_sprite			;$B9EEE8  |

CODE_B9EEEB:
	LDY #!special_sprite_spawn_id_001C	;$B9EEEB  \
	JSL spawn_special_sprite_index		;$B9EEEE   | Spawn rain cloud (loop)
	RTS					;$B9EEF2  /

anim_delete_sprite_2:
	JSL delete_sprite_handle_deallocation	;$B9EEF3  \
	RTS					;$B9EEF7  /

set_rattly_idle_no_player_animation:
	LDA #$014B				;$B9EEF8  \
	JSL set_sprite_animation		;$B9EEFB   |
	RTS					;$B9EEFF  /

set_clapper_water_timer:
	LDA sprite.general_purpose_44,x		;$B9EF00  \
if !version == 1				;	   |
	CMP $0915				;$B9EF02   |
	BCC .play_clapper_breath_sound		;$B9EF05   |
endif						;	   |
	STA $0915				;$B9EF07   |
.play_clapper_breath_sound:			;	   |
	%lda_sound(6, clapper_spit)		;$B9EF0A   |
	JSL queue_sound_effect			;$B9EF0D   |
	RTS					;$B9EF11  /

spawn_clapper_breath:
	LDY #!special_sprite_spawn_id_00DE	;$B9EF12  \
	JSL spawn_no_gfx_special_sprite_index	;$B9EF15   |
	RTS					;$B9EF19  /

set_clapper_idle_animation:
	LDA #$01B7				;$B9EF1A  \
	JSL set_sprite_animation		;$B9EF1D   |
	RTS					;$B9EF21  /

CODE_B9EF22:
	STZ sprite.x_speed,x			;$B9EF22  \
	RTS					;$B9EF24  /

set_hit_by_klubba_interaction:
	LDA #!player_interaction_26		;$B9EF25  \
	JSL set_player_interaction_global	;$B9EF28   |
	RTS					;$B9EF2C  /

CODE_B9EF2D:
	JSL CODE_B68CC2				;$B9EF2D  \
	RTS					;$B9EF31  /

CODE_B9EF32:
	JSL CODE_B68CE1				;$B9EF32  \
	RTS					;$B9EF36  /

CODE_B9EF37:
	JSL CODE_B68CB7				;$B9EF37  \
	RTS					;$B9EF3B  /

CODE_B9EF3C:
	JSL CODE_B69D96				;$B9EF3C  \
	RTS					;$B9EF40  /

CODE_B9EF41:
	INC sprite.state,x			;$B9EF41  \
	RTS					;$B9EF43  /

CODE_B9EF44:
	LDA.l $0006A3				;$B9EF44  \
	ORA #$4000				;$B9EF48   |
	STA $0006A3				;$B9EF4B   |
	RTS					;$B9EF4F  /

CODE_B9EF50:
	JSL CODE_B69043				;$B9EF50  \
	RTS					;$B9EF54  /

CODE_B9EF55:
	JSL CODE_B6A838				;$B9EF55  \
	RTS					;$B9EF59  /

;Related to krool on final hit (both versions)
CODE_B9EF5A:
	LDX active_kong_sprite			;$B9EF5A  \
	LDA sprite.state,x			;$B9EF5D   |
	CMP #$0005				;$B9EF5F   |
	BEQ CODE_B9EF76				;$B9EF62   |
	LDY #$0004				;$B9EF64   |
	LDA level_number			;$B9EF67   |
	CMP #!level_krocodile_kore		;$B9EF69   |
	BNE CODE_B9EF71				;$B9EF6C   |
	LDY #$000B				;$B9EF6E   |
CODE_B9EF71:					;	   |
	TYA					;$B9EF71   |
	JSL kong_cutscene_handler		;$B9EF72   |
CODE_B9EF76:					;	   |
	RTS					;$B9EF76  /

CODE_B9EF77:
	JSL CODE_B6A3F4				;$B9EF77  \
	RTS					;$B9EF7B  /

CODE_B9EF7C:
	JSL CODE_B6A40F				;$B9EF7C  \
	RTS					;$B9EF80  /

CODE_B9EF81:
	JSL CODE_B6A41C				;$B9EF81  \
	RTS					;$B9EF85  /

CODE_B9EF86:
	JSL CODE_B6A421				;$B9EF86  \
	RTS					;$B9EF8A  /

CODE_B9EF8B:
	JSL CODE_B6A7E8				;$B9EF8B  \
	RTS					;$B9EF8F  /

CODE_B9EF90:
	JSL CODE_B6AD8B				;$B9EF90  \
	RTS					;$B9EF94  /

CODE_B9EF95:
	JSL CODE_B6AD93				;$B9EF95  \
	RTS					;$B9EF99  /

CODE_B9EF9A:
	INC $0A,x				;$B9EF9A  \
	RTS					;$B9EF9C  /

CODE_B9EF9D:
	JSL CODE_B6ACD8				;$B9EF9D  \
	RTS					;$B9EFA1  /

CODE_B9EFA2:
	JSL CODE_B6A3CE				;$B9EFA2  \
	RTS					;$B9EFA6  /

CODE_B9EFA7:
	JSL CODE_B6A8E0				;$B9EFA7  \
	RTS					;$B9EFAB  /

CODE_B9EFAC:
	JSL CODE_B6ABCB				;$B9EFAC  \
	RTS					;$B9EFB0  /

CODE_B9EFB1:
	JSL CODE_B69493				;$B9EFB1  \
	RTS					;$B9EFB5  /

CODE_B9EFB6:
	JSL CODE_B69499				;$B9EFB6  \
	RTS					;$B9EFBA  /

CODE_B9EFBB:
	JSL CODE_B6A892				;$B9EFBB  \
	RTS					;$B9EFBF  /

CODE_B9EFC0:
	JSL CODE_B6AD0D				;$B9EFC0  \
	RTS					;$B9EFC4  /

CODE_B9EFC5:
	JSL CODE_B6AA9B				;$B9EFC5  \
	RTS					;$B9EFC9  /

CODE_B9EFCA:
	JSL CODE_B6AAA4				;$B9EFCA  \
	RTS					;$B9EFCE  /

CODE_B9EFCF:
	JSL CODE_B6AA8B				;$B9EFCF  \
	RTS					;$B9EFD3  /

CODE_B9EFD4:
	JSL CODE_B6AA93				;$B9EFD4  \
	RTS					;$B9EFD8  /

CODE_B9EFD9:
	JSL CODE_B6AABE				;$B9EFD9  \
	RTS					;$B9EFDD  /

CODE_B9EFDE:
	JSL CODE_B6AAAA				;$B9EFDE  \
	RTS					;$B9EFE2  /

CODE_B9EFE3:
	JSL CODE_B6AC99				;$B9EFE3  \
	RTS					;$B9EFE7  /

CODE_B9EFE8:
	JSL CODE_BA9751				;$B9EFE8  \
	RTS					;$B9EFEC  /

CODE_B9EFED:
	JSL CODE_BA943A				;$B9EFED  \
	RTS					;$B9EFF1  /

CODE_B9EFF2:
	JSL CODE_BA9450				;$B9EFF2  \
	RTS					;$B9EFF6  /

CODE_B9EFF7:
	JSL CODE_B6DD63				;$B9EFF7  \
	RTS					;$B9EFFB  /

CODE_B9EFFC:
	JSL CODE_B6DBF6				;$B9EFFC  \
	RTS					;$B9F000  /

CODE_B9F001:
	JSL CODE_B6DBBA				;$B9F001  \
	RTS					;$B9F005  /

CODE_B9F006:
	JSL CODE_B6DBD2				;$B9F006  \
	RTS					;$B9F00A  /

CODE_B9F00B:
	JSL CODE_B6DB36				;$B9F00B  \
	RTS					;$B9F00F  /

CODE_B9F010:
	JSL CODE_B6DBE0				;$B9F010  \
	RTS					;$B9F014  /

CODE_B9F015:
	JSL CODE_B6DC2B				;$B9F015  \
	RTS					;$B9F019  /

CODE_B9F01A:
	JSL CODE_B6DC3F				;$B9F01A  \
	RTS					;$B9F01E  /

CODE_B9F01F:
	JSL CODE_B6CA9E				;$B9F01F  \
	RTS					;$B9F023  /

CODE_B9F024:
	JSL CODE_B6CF91				;$B9F024  \
	RTS					;$B9F028  /

CODE_B9F029:
	JSL CODE_B6CF96				;$B9F029  \
	RTS					;$B9F02D  /

CODE_B9F02E:
	JSL CODE_B6CF9B				;$B9F02E  \
	RTS					;$B9F032  /

CODE_B9F033:
	JSL CODE_B6CFA0				;$B9F033  \
	RTS					;$B9F037  /

CODE_B9F038:
	JSL CODE_B6CFA5				;$B9F038  \
	RTS					;$B9F03C  /

CODE_B9F03D:
	JSL CODE_B6CFAA				;$B9F03D  \
	RTS					;$B9F041  /

CODE_B9F042:
	JSL CODE_B6CFCA				;$B9F042  \
	RTS					;$B9F046  /

CODE_B9F047:
	JSL CODE_B6CFD1				;$B9F047  \
	RTS					;$B9F04B  /

CODE_B9F04C:
	JSL CODE_B6CFD8				;$B9F04C  \
	RTS					;$B9F050  /

CODE_B9F051:
	JSL CODE_B6F753				;$B9F051  \
	RTS					;$B9F055  /

CODE_B9F056:
	JSL CODE_B6F71A				;$B9F056  \
	RTS					;$B9F05A  /

CODE_B9F05B:
	JSL CODE_B6F725				;$B9F05B  \
	RTS					;$B9F05F  /

CODE_B9F060:
	JSL CODE_B6F72F				;$B9F060  \
	RTS					;$B9F064  /

CODE_B9F065:
	JSL CODE_B6F746				;$B9F065  \
	RTS					;$B9F069  /

CODE_B9F06A:
	JSL CODE_B6F904				;$B9F06A  \
	RTS					;$B9F06E  /

set_celebrate_interaction_and_clear_level:
	JSL set_current_level_as_cleared	;$B9F06F  \
	LDA #!player_interaction_27		;$B9F073   |
	JSL set_player_interaction_global	;$B9F076   |
	RTS					;$B9F07A  /

;Unreferenced, would have set level end states on kongs and triggered fadeout.
CODE_B9F07B:
	JSL set_level_end_exit_state_global	;$B9F07B  \
	RTS					;$B9F07F  /

CODE_B9F080:
	JSL CODE_BAC256				;$B9F080  \
	RTS					;$B9F084  /

launch_barrel_rotation_frames:
	dw $08CC
	dw $08D0
	dw $08D4
	dw $08D8
	dw $08DC
	dw $08E0
	dw $08E4
	dw $08E8
	dw $08AC
	dw $08B0
	dw $08B4
	dw $08B8
	dw $08BC
	dw $08C0
	dw $08C4
	dw $08C8

auto_barrel_rotation_frames:
	dw $0318
	dw $031C
	dw $0320
	dw $0324
	dw $0328
	dw $032C
	dw $0330
	dw $0334
	dw $02F8
	dw $02FC
	dw $0300
	dw $0304
	dw $0308
	dw $030C
	dw $0310
	dw $0314

;level end target/chest reward frames
item_reward_frames_table:
	dw $1944	;00 Banana Bunch
	dw $23B0	;01 K
	dw $23D0	;02 O
	dw $23F0	;03 N
	dw $2410	;04 G
	dw $088C	;05 Barrel
	dw $0868	;06 TNT Barrel
	dw $0960	;07 Invincibility Barrel
	dw $10E4	;08 Life Balloon
	dw $10E4	;09 Life Balloon
	dw $10E4	;0A Life Balloon
	dw $10E4	;0B Life Balloon
	dw $3180	;0C Banana Coin
	dw $3180	;0D Banana Coin
	dw $3180	;0E Banana Coin
	dw $3180	;0F Banana Coin
	dw $0FCC	;10 Krem Coin
	dw $1128	;11 DK Coin
	dw $0984	;12 Cannonball
	dw $095C	;13 Chest
	dw $1944	;14 Banana Bunch
	dw $1944	;15 Banana Bunch
	dw $1944	;16 Banana Bunch
	dw $1944	;17 Banana Bunch
	dw $1944	;18 Banana Bunch
	dw $3308	;19 Banana
	dw $0FCC	;1A Krem Coin
	dw $0FCC	;1B Krem Coin
